...
`max_items` is 192 here and 64 in v1. Nothing else differs: the method, the
required sections, the allowed source roles, the required claims, the renderer,
the character bound and every word of the business definition are the v1 bytes.

The bound moved because it was limiting block count rather than disclosure.
Measured over the nine annual filings this repository holds, an excerpt carries
between 341 and 1,422 characters on average once the renderer's separators are
counted the way this bound counts them, so a filing using the whole 64,000
character budget needs between 45 and 187 items depending only on how its filer
breaks paragraphs. At 64 the item bound decided the outcome for a filing well
inside the character budget: Pfizer's Item 3 incorporates Note 16A, and that
sub-note's legal proceedings are 92 excerpts and 41,860 characters - 65 percent
of the character budget and 144 percent of the item bound. Southwest discloses
35,545 characters in 25 excerpts and Ford 18,077 in 53; the same metric, the
same sections, three very different block counts.

192 is derived rather than chosen to fit Pfizer: it is the 187 items the
sparsest density observed here needs at the full character budget, rounded up
to three times the original bound. The character bound stays the operative
limit on how much text a result may carry, and the item bound keeps refusing
genuinely pathological fragmentation.
