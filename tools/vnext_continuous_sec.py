...
def main(argv=None):
    ...
    if args.command=='plan':
        if args.control_id is None:
            discovered=discover_saved_source_requirements(repo_root=ROOT,company_id=args.company)
        else:
            from vnext.r6_historical_controls import discover_control_sources
            discovered=discover_control_sources(repo_root=ROOT,company_id=args.company,control_id=args.control_id)
