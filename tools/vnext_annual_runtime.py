...
def main(argv=None):
    ...
    try:
        with redirect_stdout(sys.stderr):
            if args.command=='initialize':
                ...
                result=runtime.stage_proposal(data_root=args.data_root,stage_root=args.stage_root,
                    baseline_run=args.baseline_run,review_path=args.review_file)
            else:
                result=runtime.run_update(approval_url=args.approval_url)
    except (ValueError,OSError,KeyError,TypeError,RuntimeError) as error:
        result={'status':'STAGE_STOPPED','error':str(error),'error_type':type(error).__name__,
            'execution':'NOT_CONFIRMED','provider_paid_sec_calls':[None,None,0],
    ...
    return 0 if result.get('status') in {'SAVED_INPUTS_COPIED','CANDIDATE_UPDATE_SUCCEEDED','NO_NEW_ANNUAL_FILING'} or args.command=='stage-proposal' and 'stage_id' in result else 2
