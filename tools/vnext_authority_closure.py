...
* modules imported inside a function by authority-named code. A parent's own
  unused routes belong here as information, not as a defect.
* of those, the ones this generation's own new rule files import directly. That
  is not information: this generation wrote the importing line, so the
  authority has to name what it imports or the Run records less than it ran.
  One hop, not the transitive closure - the new rule files reach 206 of the 215
  modules through the parent's own code, so a transitive test says nothing. The
  distinction had to be added after a D02 Run executed
  `historical_text_input.py` on every call while neither the rule set nor the
  inherited authority named it. It was in the second list all along, under a
  heading that said absence there was not a defect.
...
def measure(*, repo_root, requirement_id):
    ...
    own_seeds = sorted(path for path in manifest.get("new_rule_files", {})
                       if path.startswith(PACKAGE) and path.endswith(".py"))
    own_direct = {_module_path(name) for seed in own_seeds
                  for name in _imports(repo_root=repo_root, relative=seed,
                                       module_scope_only=False)}
    own_unnamed = sorted(own_direct - authority)
    return {"record_type": "EXECUTION_AUTHORITY_IMPORT_CLOSURE", "schema_version": 2,
            ...
            "new_rule_modules": len(own_seeds),
            "new_rule_direct_imports": len(own_direct),
            "required_to_import_but_not_named": required,
            "reachable_on_some_path_but_not_named": sorted(
                reachable - authority - set(required)),
            "imported_by_this_generation_but_not_named": own_unnamed,
            "authority_is_import_complete": not required,
            "authority_names_this_generation_routes": not own_unnamed,
...
def main(argv=None):
    ...
    parser.add_argument("--require-own-routes", action="store_true",
                        help="Exit non-zero when this generation's own code reaches "
                             "a module the authority does not name.")
    ...
    if arguments.require_own_routes and not report["authority_names_this_generation_routes"]:
        raise SystemExit("this generation's own code reaches unnamed modules: "
                         + ", ".join(report["imported_by_this_generation_but_not_named"]))
    return 0
