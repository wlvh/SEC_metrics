...
HUMAN reviewer 必须亲自执行 decide；Agent、模型、fixture helper 与 acceptance runner 都没有自动批准权限。只有 ReviewUnit、rendered context、Spec/source/Evidence exact 且 decision 仍为 effective tip，旧决定才可复用。错误 supersedes 会显示 Run ID、ReviewUnit hash、requested/current tip、chain 摘要与恢复命令。qualification 只接受有效 HUMAN `APPROVE`、disclosure Spec exact Result set全部`PUBLISHED`且Run validation=`PASSED`的receipt；`REJECT`或WITHHELD必须保留为审计结果，不能冻结为第二布局或holdout的Cutover资格。qualification CLI 对意外错误也只给结构化`QUALIFICATION_COMMAND_FAILED`，只有`--debug`显示traceback。
