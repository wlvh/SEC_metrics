...
Stage C-A新增的`vnext_table_context_measurement.py plan`只离线重建并保存一个exact lodging usage-measurement plan，显示392447 estimated-input、普通qualification 200000门仍阻断、Marriott/task/serializer/prompt/schema/request/provider/protected-closure binding及`repository_head_binding=REQUIRED_AT_EXTERNAL_AUTHORIZATION`。`execute`不会接受family/task/source/serializer/provider override；只有命令中同时提供exact `AUTHORIZE_ONE_TOKEN_MEASUREMENT`、当前clean HEAD和UTC授权时间才会签发opaque capability。provider socket/mock egress立即前的marker是互斥claim：任何既有marker（即使bytes相同）都返回`AUTHORIZATION_CONSUMED`，只有`O_EXCL`唯一创建者在file+directory fsync后继续。确定性双进程测试以相同authorization/clock和callback barrier证明总opener=1；顺序第二次同样稳定拒绝。marker前失败保留未消费状态但不会自动继续。terminal measurement evidence只显示provider raw usage、HTTP/transport状态、request/response hash、token/cache字段与egress计数；缺prompt/input usage显示`FAILED_USAGE_UNAVAILABLE`，绝不从bytes/token历史比例猜值。该evidence明确`qualification_credit=false`、`publication_eligible=false`、`response_reuse_for_qualification=false`。Stage C-A没有运行`execute`，真实model/paid/SEC计数为0/0/0；这只是已封存的pre-egress历史状态。
