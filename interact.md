...
table transport/scope/task freeze继续是离线开发证据：expanded grid不变，本地Evidence Authority始终materialize全文档；模型传输只可用可逆serializer-v2 compact payload。lodging仍把完整compact parent放入一个schema-v3 request。financial schema v4只在request边界按exact envelope大小贪心形成连续shards：每个shard携带完整ReaderInputManifest和exact set/index/count/range/table IDs；`NO_CANDIDATE_IN_SHARD`只表示本片未找到目标，只能形成WITHHELD Result。Candidate、Evidence、Review、attempt、Run和WB-3 authorization均绑定同一shard identity；全部shards FROZEN、679个table IDs逐一且只出现一次、至少一个candidate Evidence PASS且候选不冲突以前，用户不得看到qualification/publication credit。一个scope locator仍可机械支持多个raw dimension；Evidence逐字节重取locator和raw value，系统只按MetricSpec exact enum alias规范化。未知alias继续为`REVIEW_REQUIRED`，不能变成新的quality或SYSTEM auto approval。

每个catalog task仍只拥有一个role和一组MetricSpec scope/schema/prompt identity，matrix task ID实际进入formal Workflow、Run manifest、prepared Reader request、provider envelope、attempt audit、Review、FROZEN validation和replay。catalog LIVE task不能由普通 Workflow 参数直接执行：唯一qualification executor从current matrix、freeze、Stage-A snapshot、immutable source、parent task plan及child shard plans重建opaque authorization；shared Workflow在读source或进入WB-3前重验。成功attempt同时成为qualification evidence并写入cycle-owned ledger。freeze以shared engine closure保护共用语义，以matrix/task/MetricSpec fragments保护family-local语义；另一family的local失败不阻断requested family。current exact JPM离线结果为8个task各27 shards、每task全覆盖679表，最大单片199912；materialization resource gate已PASS。该结果不等于live qualification：current freeze/Stage-A、D-07 concrete scope和exact-head review完成前仍不得调用provider。
...
Stage-B financial grid census receipt是pre-policy历史证据：它显示JPM exact HTML binding、全部679张表的order/table-id候选/shape/source/rectangular/blank/span/text/size、top 50及旧production 100000门的exact触发表。工具得到124761个rectangular coordinates，但当时不创建完整expanded dict/list，OPTION-A/B/C均保持`selected_option=null`。后续D-35引用并消费该immutable census与独立JPM Linux benchmark完成历史100000→124761，不重签Stage-B receipt；BOA second-layout又以200229 cells完成同边界characterization，current A把cap最小提高到200229，仍不做parser/materialization shards或source replacement。连续provider request shards发生在完整materialization之后，不改变两段local resource选择。
...
Stage C financial benchmark界面同时保留Darwin `NOT_RUN_RSS_GUARD_UNAVAILABLE`、一次Linux `FAILED_CHILD`诊断及JPM `COMPLETED` immutable receipt。JPM run在pinned Linux image、read-only source、512 MiB cgroup-v2、120秒wall和`network=none`下显示679 tables/124761 cells、process RSS 282877952、cgroup peak 288043008、wall 5.10483s、canonical 22174348 bytes与DerivedAsset ID，支撑历史cap=124761。current successor另显示BOA 369 tables/200229 cells、process RSS 366530560、cgroup peak 371380224、wall 4.330166s、canonical 34204694 bytes与DerivedAsset ID，支撑current cap=200229；二者模型/paid/SEC egress均为0/0/0。界面必须把local resource policy与provider request shards分开显示，也不得把历史NOT_RUN改写成completed。
