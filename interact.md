...
Stage C-A新增的`vnext_table_context_measurement.py plan`只离线重建并保存一个exact lodging usage-measurement plan，显示392447 estimated-input、普通qualification 200000门仍阻断、Marriott/task/serializer/prompt/schema/request/provider/protected-closure binding及`repository_head_binding=REQUIRED_AT_EXTERNAL_AUTHORIZATION`。`execute`不会接受family/task/source/serializer/provider override；只有命令中同时提供exact `AUTHORIZE_ONE_TOKEN_MEASUREMENT`、当前clean HEAD和UTC授权时间才会签发opaque capability。provider socket/mock egress立即前出现的marker永久消费capability，第二次命令稳定拒绝；marker前失败保留未消费状态但不会自动继续。terminal measurement evidence只显示provider raw usage、HTTP/transport状态、request/response hash、token/cache字段与egress计数；缺prompt/input usage显示`FAILED_USAGE_UNAVAILABLE`，绝不从bytes/token历史比例猜值。该evidence明确`qualification_credit=false`、`publication_eligible=false`、`response_reuse_for_qualification=false`。本Stage C-A没有运行`execute`，所以真实model/paid/SEC计数仍为0/0/0；这不授权qualification、R3/R4或publication。
...
Stage C-A packet界面按`OWNER_APPROVED`、`IMPLEMENTED_NOT_EXECUTED`、`MEASURED_OFFLINE`、`STILL_UNAUTHORIZED`、`BLOCKERS`显示，不把实现或NOT_RUN写成live evidence。当前measurement只显示plan ID，`authorization_id=NOT_ISSUED`；JPM显示guard blocker及四个null measurement；active publication、309 rows、root before/after和0/0/0 egress必须逐byte可复算。snapshot checker的Stage C-A PASS表示历史R2非source artifacts与当前source overlay同时闭合，不表示benchmark completed、真实measurement、qualification或publication。
