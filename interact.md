...
latest D-07已把live qualification限定到lodging两个current task。`vnext_qualification.py table-plan`显示matrix-owned phase/source/request：second layout=Marriott FY2024、holdout=Hyatt、fresh=Marriott FY2025；CLI不接收source/company/period override。FY2024替代source由既有`SecHttpClient`一次获取，offline proof要求唯一目标表同时包含全部冻结scope literals并与FY2025目标表geometry/grid hash不同；source-only fixture不含可复用模型response。每个`table-execute`仍生成新的WB-3 execution、provider ledger row、qualification Evidence、Review、Result与FROZEN Run。FY2024 second-layout和Hyatt holdout的estimated requests都超过200000，只可在独立exact-head review逐plan/request绑定后执行，并由各自新response的provider usage terminal判断；缺usage或actual prompt超限立即terminal、零重试并停止后续lodging plans。Marriott FY2025 fresh继续走各task exact attestation。两个second-layout task都FROZEN后，`table-freeze`用既有`PRODUCTION_SEMANTIC_FREEZE`类型绑定production tree和ledger prefix；holdout只能在freeze后，三个fresh ordinals只能在两个holdout task FROZEN后。该顺序不新增measurement、不复用历史response，也不自动授权R3 publication。
