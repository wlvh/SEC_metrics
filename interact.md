...
- 运行负责人维护 registry、提供环境凭据并控制 clean checkout 运行。SEC organization 固定 `axaxl`，contact email 只从 `SEC_CONTACT_EMAIL` 读取；OpenAI key 只从 `OPENAI_API_KEY` 读取。缺失、畸形或 example/reserved-domain email 会在联网前失败，secret 不写入 artifact。
...
仓库已包含同一套 recorded/live vNext operator：Requirement/Spec、table-grid Reader input、固定 Responses adapter、机械 Evidence、整单 Review、freeze/replay、Spec-driven Calculator、Projector、qualification、formal publication/rollback 与 pinned consumers。recorded 只替换 transport/source acquisition并强制 socket=0；live 必须显式授权，且不能建立第二套业务逻辑。它面向开发者、运行负责人和 reviewer；当前没有 active pointer，所以仍不是业务结果的新入口。
<!-- capability-anchor: CAPABILITY.vnext_recorded_shadow -->

正式 CLI 支持 prepare/init、status、review list/show/decide、resume/finalize、freeze/replay、project、publish、rollback、restore 与 acceptance。默认输出稳定错误且不显示 traceback，`--debug` 才显示 traceback，`--json` 提供机器可读结果。首次缺 HUMAN decision 时返回 `HUMAN_REVIEW_REQUIRED`，同时显示 review path、ReviewUnit hash 与可复制 decide 命令；Run 保持 OPEN，不能创建伪 publication。

冷启动 recorded operator 不要求阅读源码或 tests。`fixture list`/`fixture show --fixture-id ...` 会从仓库 catalog 返回 byte-verified fixture binding、SEC source/provenance identity，以及不含 caller business override 的可复制 prepare/Cutover 命令。运行 `tools/vnext_cutover.py --fixture-id ... --workspace-dir artifacts/vnext/recorded-<workspace>` 时，第一次调用从同一 release plan 创建 structured FROZEN Runs 与需要审核的 OPEN Run；若缺决定，JSON error=`HUMAN_REVIEW_REQUIRED`，每个 `pending_reviews` 项必须包含绝对 `review.md` 路径、ReviewUnit hash、Run directory 与可复制的 HUMAN decide 命令，且 `<workspace>/recorded-publication` 尚不存在。reviewer 阅读 exact rendered context 并亲自决定后，重跑完全相同的 Cutover 命令会 resume 原 Run，完成 finalization、validation、freeze、无网络 replay、complete BatchManifest、Projector、recorded validation，再只在固定 `<workspace>/recorded-publication` 内执行 pointer CAS、root-mirror materialization 与 pinned PublicationView read-back。
...
HUMAN reviewer 必须亲自执行 decide；Agent、模型、fixture helper 与 acceptance runner 都没有自动批准权限。只有 ReviewUnit、rendered context、Spec/source/Evidence exact 且 decision 仍为 effective tip，旧决定才可复用。错误 supersedes 会显示 Run ID、ReviewUnit hash、requested/current tip、chain 摘要与恢复命令。
...
acceptance JSON保留logical/executed argv、return code、duration及stdout/stderr digest，但持久化时把repository/output/runtime locator递归替换为`$REPO_ROOT`、`$ACCEPTANCE_OUTPUT`、`$PYTHON_CURRENT`、`$PYTHON39`和`$SANDBOX_EXEC`；`runtime_bindings`用executable name与binary SHA-256证明实际runtime，本机绝对路径不进入可提交receipt。`--output-dir`与正式authority存在equal/ancestor/descendant关系时在首次写入前返回`ACCEPTANCE_OUTPUT_DIR_OVERLAPS_FORMAL_AUTHORITY`；caller Python 3.9只接受在完整正式snapshot/backup后、network+formal-file sandbox中通过exact structured probe的native CPython 3.9，任何authority漂移恢复后仍失败。该portable化只改变定位表达，不删除NOT_RUN、error或command语义。

### 8.4 当前不能执行的承诺

effective D-01已由R3批准，固定SEC Stage00/01/02/03/05 acquisition/inventory、legacy migrated producer退出、PublicationView consumers、qualification、fault/rollback primitives和new/rollback/restore终态编排也已实现。但本轮因`OPENAI_API_KEY`、`SEC_CONTACT_EMAIL`、qualification与HUMAN前提缺失，live acquisition未执行且无receipt；第二真实布局/holdout合格bytes、live三轮、十公司formal staging、committed active/previous publication、rollback/restore与full receipt也不存在。因此：

- `tools/run_acceptance.py --scope recorded` 强制离线且不修改 pointer/root mirrors，最高状态是 `PASSED_RECORDED_ONLY`；
- `--scope full` 未带 `--execute-live` 返回 `LIVE_EXECUTION_NOT_AUTHORIZED`；带授权但凭据/qualification/predecessor缺失时返回 BLOCKED且不开始 Cutover；
