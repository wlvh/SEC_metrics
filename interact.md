...
Stage C financial benchmark界面同时保留三代事实：Darwin `NOT_RUN_RSS_GUARD_UNAVAILABLE`、一次Linux `FAILED_CHILD`诊断，以及current `COMPLETED` receipt。current run在pinned Linux image、read-only source、512 MiB cgroup-v2、120秒wall和`network=none`下执行完整parser/materialization/canonical serialization；显示679 tables/124761 cells、process RSS 282877952、cgroup peak 288043008、wall 5.10483s、canonical 22174348 bytes与DerivedAsset ID。benchmark时production 100000 bytes和active/root before-after不变，模型/paid/SEC egress为0/0/0。D-35随后把current production materialization cap设为124761；用户界面必须把这项local resource policy与随后新增的provider request shards分开显示，也不得把历史NOT_RUN改写成completed。
