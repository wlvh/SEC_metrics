...
Stage D复用上述同一plan/executor/marker/evidence边界，把当前唯一未消费目标固定为`lodging_revpar_table_v2`；历史occupancy authorization、marker、raw response、evidence与attestation均不改写。RevPAR plan必须继续使用Marriott SHA-256 `c372495a…2547`、serializer v2、全部table grids原序和no-selector/no-prefilter，并显示exact provider request SHA。独立review绑定exact HEAD和request SHA后才可执行；任一egress永久消费且retry=0。若authoritative prompt usage缺失或actual prompt>200000，operator必须显示`OWNER_DECISION_REQUIRED`并停止；若不超过200000，则同一`TABLE_CONTEXT_FEASIBILITY_ATTESTATION` record type直接绑定plan/marker/raw/evidence，不新增packet、pointer或第二套state machine。measurement response仍不得计入qualification ordinal或publication。
