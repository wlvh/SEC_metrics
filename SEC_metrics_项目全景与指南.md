...
**版本**：v2.1（以本文件为底稿，合并 `SEC_metrics_项目全景与专家指南.md` 的当前代码考证与专家审计内容）  
...
- **scale（缩放属性）**：iXBRL 数字标签可声明 10 的幂缩放（正文显示 294,804 百万，标签值 294804 + scale=6）。v1.0 曾把本项目误判为“缺少 scale 处理”，后续复核撤销：代码中已有 `scaled_inline_value`。当前版本进一步修掉了 parse-route 风险：`parse_instance_with_fallback()` 会先识别 inline XBRL，再走 `InlineFactParser`，避免 XML 主路径吞掉 `scale/sign`；测试夹具与 JPM CET1 capital 完整 evidence crosscheck 均已覆盖。新增金额类维度事实进入派生公式前，仍必须新增金额级 golden 或 companyfacts/表格交叉核验。
...
全部逻辑住在 `scripts/sec_pipeline.py`（约 14,000 行）这一个单体模块里；`00_smoke_test_sec_access.py` 到 `12_validate_repair.py` 十三个编号脚本各约 20 行，只做一件事：`run_stage(stage_name="...")`。调度表在单体尾部的 `STAGES` 字典。这个形态是 spike 阶段的务实选择，产品化时应拆分——但拆分时**必须保留的**是下面的逻辑架构，那才是五轮迭代真正沉淀的资产。
...
一份让你成为专家的文档，最不可省略的就是这一节。当前工作区 `python3 scripts/12_validate_repair.py` 返回全 P0 PASS，`python3 -m unittest tests/test_sec_pipeline_validation.py` 跑通 17 个测试。下面不再把旧审计项混作当前风险，而是分成**已关闭风险**与**仍开放风险**。
...
4 看 concept_or_section/context_or_dimension：确认取数口径是不是你要的口径。
