# 05｜SEC_metrics 项目全景与专家指南（融合完整版本）

**版本**：v2.0（融合版，基于 round3 / 20260707T080136Z）  
**用途**：让项目负责人、审计者、开发者完整掌握 SEC_metrics spike 的目标、架构、计算逻辑、证据链、准确性防线、验收方法和扩展路径。  
**读者目标**：读完后，你应该能独立审计任意一个指标、判断一个 status 是否诚实、识别伪成功、给 Codex 下可泛化的修复指令，并设计新增公司/行业/指标的扩展方案。
...
6. 准确性防线：证据链、golden、validation、tamper test
7. 残余风险登记簿
...
| 完整 SEC evidence 验收 | 不能仅凭轻量包判 ACCEPT | 轻量包刻意剔除了 `evidence/`、完整 `outputs/concept_inventory/` 等大文件；正式验收仍要完整 evidence 包或 live SEC rerun。 |
| scale-ready 生产化 | 尚未最终完成 | 还需要修风险登记簿里的 R1/R2/R3/R4 等真实世界扩展风险，并用 Hilton / Citi / GM / ServiceNow 这类真实第 11 家试点跑全流程。 |
...
- **scale（缩放属性）**：iXBRL 数字标签可声明 10 的幂缩放（正文显示 294,804 百万，标签值 294804 + scale=6）。**本项目的 instance 解析器未处理 scale**——比率类事实是 pure 不受影响，但金额类维度事实存在 100 倍级错位风险（[实测]：第一轮包中 JPM 的 CET1 资本金额在 inventory 里是 29.5 亿，真实约 2,950 亿）。这是残余风险 R4，规则是：**维度金额勿直接消费，比率类才安全**。

### 2.3 HTTP 访问纪律

[考证+实测] `sec_http.py` 实现：所有请求带 `User-Agent: <组织> <邮箱>`（SEC 硬性要求，缺失会 403）；全局限速 ≤5 请求/秒（sleep 节流）；403/429/5xx 指数退避重试（第一轮的请求日志里能看到 403→重试→200 的完整链）；每次请求写入 `evidence/requests_log.csv`；原始响应落盘。第一轮审计对 592 条请求做过域名普查：**只有 www.sec.gov 和 data.sec.gov 两个域名**，零第三方数据源。
...
**R1 [实测确认未修] 10-K/A 修正案无 instance 回退。** 修正案（10-K/A）常只含封面级 iXBRL，完整事实在原始 10-K 里。管道没有"解析出的事实数过少时回退抓原始 10-K"的逻辑（终版代码 grep 确认该路径不存在）。第一轮实证：Southwest/Paramount 的 instance 清单仅 36/117 行（正常 1,200–8,400），导致 crosscheck 出现 NOT_FOUND。触发条件：任何 target 是 10-K/A 的公司。修法：`若 form==10-K/A 或解析事实数 < 500 → 定位同财年原始 10-K → 解析其完整 instance，role 标 target_original_full_instance`。这是扩展前**必修**项。
...
**R4 [实测] iXBRL scale 属性未处理，维度金额有 100 倍级风险。** 第一轮 inventory 中 JPM CET1 资本金额 29.5 亿（真实约 2,950 亿）。比率类 pure 事实不受影响（当前 A01/A02 只消费比率，安全）；铁律：**在修好 scale 之前，禁止用维度金额自行计算比率**。修法：解析 `ix:nonFraction` 时应用 `value × 10^scale`，并处理 sign/format/括号负数，加 crosscheck 回归。
...
这一节是实操入口。前面的章节解释了系统为什么这么设计；这里解释你拿到一个交付包以后，应该按什么顺序打开文件、看什么字段、如何判断一个数字是否可信。

### 9.1 `metrics_matrix.csv`：主交付物，不是唯一证据

`metrics_matrix.csv` 是所有指标的主矩阵。每一行代表一个 `company × metric_id`。你最先应该看这些字段：
...
给 Codex 下修复指令时，避免公司名主体：

```text
错误说法：修 Salesforce B12。
正确说法：修 RpoCrpoExtractor，使其对所有存在 RevenueRemainingPerformanceObligation instance fact 的公司优先输出 DIM_XBRL_OK，并新增第 11 家 subscription fixture。

错误说法：修 JPM A02。
正确说法：修 BaselCapitalRatioExtractor，使 CET1 actual ratio 与 regulatory threshold 分离，threshold 不进入 primary candidate。

错误说法：修 Marriott RevPAR。
正确说法：修 LodgingKpiExtractor 的表头映射和 RevPAR=ADR×Occupancy 恒等式，适用于 lodging profile。
```
...
scale              iXBRL 数字的 10 幂缩放声明（本项目未处理，见 R4）
...
## 附录 A：五轮演化史速查（每条机制背后的伤疤）

```text
R1 原始交付   核心数值层三重验证干净；文本层 12 错值戴 OK 状态；
              JPM 资本比率/审计师名/CEO 薪酬的答案已在自家解析产物中未消费
              → 催生：quote 支撑性铁律、inventory 优先规则、量纲区间断言
R2 P0 修复    错值全部降级或用结构化数据重算
              → 暴露：11 处公司名业务派发（SIC 在册未用）、公司×年度级专属正则
R3 去特例化   注册表/YAML/profile 派发落地，字面量归零
              → 暴露：Basel 词表过拟合 JPM 命名、Marriott 召回归零、captive 探针
                恒真、挂载级假行为测试 → 催生：AST 扫描、SIC 规则、召回棘轮
R4 二轮修复   表头映射+恒等式、探针收窄、行为夹具带值、六项全真
              → 暴露：阈值混入候选池（同维度对决被打穿）、轻量 golden 循环自证
                → 催生：审计并集流程（GPT 与 Claude 清单合并进指令）
R5 最终硬化   阈值双层排除+角色分离保全、三态 marker 模式、五类完整性重算、
              7 篡改回归；对抗火力首次全部被接住（除 R2 理论字缝）
              → 判定：静态防线到顶，毕业条件 = R1..R4 登记簿修复 + live 试点
```
