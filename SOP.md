...
| 7 | PR-3阶段A冻结 table transport / scope / task contracts | `scripts/vnext/table_payload.py`；`scripts/vnext/scope_contract.py`；`catalog/table_task_contracts.json`；`config/table_qualification_matrix.json`；`tools/freeze_table_qualification.py` | 只对既有本地SEC bytes执行11组compact round-trip与token/context测量；生成content-addressed freeze receipt，三种real egress=0，active/root不变。结果为`D07_DECISION_REQUIRED`时停止，不加入selector；无论结果如何不得自行启动capture或qualification。 |
