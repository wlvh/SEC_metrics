...
| 13 | 生成并验证Stage C-A packet/source overlay后主动停止 | `tools/create_stage_c_a_packet.py`；`tools/check_validation_snapshot.py` | packet严格分区且authorization=`NOT_ISSUED`、JPM NOT_RUN/null、active/root不变、0/0/0 egress；historical R2只允许source drift，current clean source tree由packet exact绑定。存在`JPM_RSS_GUARD_UNAVAILABLE` blocker，仍须创建Draft并停止，不得进入Stage C-B |
