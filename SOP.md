...
| 15 | Stage D在同一one-shot边界实施RevPAR exact plan并停在独立审核前 | `tools/vnext_table_context_measurement.py plan`；`scripts/vnext/table_context_measurement.py`；latest same-ID D-07 tip | task必须机械解析为`lodging_revpar_table_v2`；source/serializer/prompt/schema/provider/model/API、full table set原序与no-selector/no-prefilter exact；plan输出provider request SHA且measurement response无qualification/publication/reuse credit。`execute`必须同时收到授权词、clean exact HEAD、review绑定的request SHA、top-level PR comment URL与UTC时间。审核前不得执行；任一marker后永久消费且retry=0。actual usage缺失或>200000时停止并输出`OWNER_DECISION_REQUIRED`；否则用同一attestation record type生成RevPAR exact proof，不新增packet、pointer或state machine。 |
