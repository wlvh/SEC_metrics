...
| 7 | 冻结 table transport / scope / task/request context authority | `scripts/vnext/table_payload.py`；`scripts/vnext/scope_contract.py`；`catalog/table_task_contracts.json`；`config/table_qualification_matrix.json`；`scripts/vnext/table_qualification_freeze.py`；`tools/freeze_table_qualification.py`；`tools/create_stage_a_validation_snapshot.py` | 继续验证全表可逆compact transport、scope/task/MetricSpec与shared/family-local closure。D-35 cap=124761后，exact JPM production materialization必须679表/124761 cells且不再产生`EXPANDED_GRID_RESOURCE_LIMIT`。financial schema-v4在request边界把同一完整compact parent按原序贪心切成连续shards；每个task当前离线重建为27个exact provider envelopes，679个table IDs必须逐一且只出现一次，任何缺片、重复、乱序、semantic selector或candidate后提前停止均fail closed。全部shards完成前无qualification/publication credit；旧freeze保持历史blocked状态，current freeze/Stage-A/plans必须重建。financial真实qualification仍需current D-07 scope、exact-head审核、新execution和usage terminal；旧response不得复用。 |
...
| 9 | 保留JPM pre-policy census并验证D-35 resource transition | `tools/investigate_jpm_financial_grid.py`；`tools/benchmark_jpm_full_materialization.py --validate` | Stage-B 100000触发点/OPTION-A-B-C receipt原字节不改；current Linux benchmark为COMPLETED且679/124761、RSS/time/canonical/DerivedAsset闭合；D-35的local materialization仍只选择minimum cap=124761且不做parser shards/selector/filter/source replacement，另一个同ID successor只授权provider request连续分包；三类benchmark egress为0 |
