...
| 7 | 冻结 table transport / scope / task/request context authority | `scripts/vnext/table_payload.py`；`scripts/vnext/scope_contract.py`；`catalog/table_task_contracts.json`；`config/table_qualification_matrix.json`；`scripts/vnext/table_qualification_freeze.py`；`tools/freeze_table_qualification.py`；`tools/create_stage_a_validation_snapshot.py` | 继续验证全表可逆compact transport、scope/task/MetricSpec与shared/family-local closure。schema-v4 freeze必须为每个development source×task重建完整SourceReference与exact provider request SHA，并持久化`readiness_by_task_request`；普通`estimated<=200000`走`ESTIMATED_BOUND`，超限只在所有exact binding等于有效attestation且actual prompt<=200000时走`PROVIDER_REPORTED_EXACT_BINDING`。attested request PASS不得传播给sibling；family overall只有全部required task/request通过才为true。当前attested lodging request context PASS、sibling BLOCK，因此lodging family=false；financial仍resource BLOCK，`live_ready_family_ids=[]`。qualification仍需独立授权，新execution response usage缺失或超限必须terminal/no retry/stop later ordinal；active/root不变且本轮三种egress为0。 |
...
| 16 | 按matrix phase完成lodging qualification并形成R3前置证据 | `tools/vnext_qualification.py` 的 table-plan、table-execute、table-freeze、table-freeze-status；latest D-07 live qualification scope | 两个task依次完成Hilton second-layout新execution→FROZEN、production semantic tree/ledger-prefix freeze、Hyatt post-freeze holdout新execution→FROZEN、每task三个Marriott fresh ordinals。所有plan绑定source/request SHA/context basis；Hyatt over-estimate request必须exact-head review且terminal actual usage<=200000。每个sample必须有new WB-3 execution、provider ledger、Evidence、Review、Result、PASSED/FROZEN closure；measurement response禁止复用。financial仍未授权。 |
| 17 | 修订lodging prompt后重新建立两个exact context proof | latest same-ID D-07；`tools/vnext_table_context_measurement.py plan --task-contract-id ...`与`tools/vnext_table_context_measurement.py execute --task-contract-id ...`；`tools/create_table_context_feasibility_attestation.py --task-contract-id ...` | 保留既有schema/Evidence失败terminal；只改两个lodging system prompt，caption必须来自selected target table自身非空caption raw text，否则从同一目标表的一格复制八字段locator与exact raw text，禁止跨表或借邻近正文。schema-v3/MetricSpec/source/serializer/provider不变。由独立review评论逐head/tree/plan/request SHA签发Occupancy与RevPAR各一次one-shot；retry=0、任一marker永久消费、usage-only/no-credit。任一usage缺失或actual>200000即在本PR输出OWNER_DECISION_REQUIRED；两份新attestation都被D-07接受前不得回到步骤16。 |
