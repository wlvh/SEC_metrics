...
| 17 | 修订lodging prompt后重新建立两个exact context proof | latest same-ID D-07；`tools/vnext_table_context_measurement.py plan/execute --task-contract-id ...`；`tools/create_table_context_feasibility_attestation.py --task-contract-id ...` | 三条Hilton schema terminal保持失败证据；只改两个lodging system prompt，schema/MetricSpec/source/serializer/provider不变。先生成blocked-current freeze/Stage-A，再由独立review评论逐head/tree/plan/request SHA签发occupancy与RevPAR各一次one-shot；retry=0、任一marker永久消费、usage-only/no-credit。任一usage缺失或actual>200000即在本PR输出OWNER_DECISION_REQUIRED；两份新attestation都被D-07接受前不得回到步骤16。 |
