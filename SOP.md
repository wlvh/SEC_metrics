...
| 2 | 加载自包含 Decision 和 WB-1 receipts | `requirements/issue_15_v1/decision_register.json`；`requirements/issue_15_v1/legacy_semantic_producer_inventory.json`；`requirements/issue_15_v1/source_strategy_baseline_receipt.json`；`requirements/issue_15_v1/foundation_verification_receipt.json` | `load_requirement_snapshot(issue_15_v1)` 通过；D-01 与 post-freeze D-36、D-35、D-26、D-07 effective tips 精确。D-07保留全表/原序/无selector、200000 inclusive context门与no-response-reuse；D-35先以exact JPM Linux benchmark把financial local cap提高到124761，后以BOA 200229-cell hard-guard characterization选择A并把current cap最小提高到200229，两次均明确不做parser/materialization shards；provider request边界另按exact envelope大小形成确定性连续shards。所有policy均不删表、不筛表、不改变D-07业务语义。current runtime authority绑定resource/tool bytes，历史R3 bundle仍按pre-resource D-35 closure验证。 |
...
| 7 | 冻结 table transport / scope / task/request context authority | `scripts/vnext/table_payload.py`；`scripts/vnext/scope_contract.py`；`catalog/table_task_contracts.json`；`config/table_qualification_matrix.json`；`scripts/vnext/table_qualification_freeze.py`；`tools/freeze_table_qualification.py`；`tools/create_stage_a_validation_snapshot.py` | 继续验证全表可逆compact transport、scope/task/MetricSpec与shared/family-local closure。current D-35 cap=200229后，JPM/BOA/Citi production materialization必须分别完整保留679/369/330表与124761/200229/95463 cells，不得产生`EXPANDED_GRID_RESOURCE_LIMIT`或local shards。financial schema-v4只在request边界把各自完整compact parent按原序贪心切成连续shards；table IDs必须逐一且只出现一次，任何缺片、重复、乱序、semantic selector或candidate后提前停止均fail closed。全部shards完成前无qualification/publication credit；旧freeze保持历史状态，current freeze/Stage-A/plans必须重建。financial真实qualification仍需current D-07 scope、exact-head审核、新execution和usage terminal；旧response不得复用。 |
...
| 9 | 保留JPM pre-policy census并验证D-35 resource transition | `tools/investigate_jpm_financial_grid.py`；`tools/benchmark_jpm_full_materialization.py --validate` | Stage-B 100000触发点/OPTION-A-B-C receipt原字节不改；JPM Linux benchmark仍以679/124761、RSS/time/canonical/DerivedAsset闭合历史100000→124761 transition；current successor另绑定BOA 369/200229、371380224-byte cgroup peak与4.330166s hard-guard结果，选择A完成124761→200229且不做local shards。provider request连续分包保持独立；三类characterization egress为0 |
...
| 11 | 保留historical token measurement并验证JPM Linux hard-guard benchmark | `scripts/vnext/table_context_measurement.py`；`tools/benchmark_jpm_full_materialization.py --validate`；effective D-35 and D-07 | measurement历史authorization均已消费且无qualification/publication credit。JPM current receipt必须在pinned Linux、512 MiB cgroup、120秒wall、network-none下COMPLETED，process RSS 282877952、cgroup peak 288043008、wall 5.10483s、canonical 22174348 bytes；benchmark commit/tree与post-policy resource/tool authority双重验证，real model/paid/SEC=0/0/0。 |
