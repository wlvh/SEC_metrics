...
class AnnualRuntimeTest(unittest.TestCase):
    def setUp(self):
        ...
        def api(path):
            self.assertEqual(
                "repos/wlvh/SEC_metrics/issues/comments/999901",
                path,
                "Runtime must not query PR state",
            )
    ...
    def test_complete_runtime_and_same_input_new_process_no_pr(self):
        watched = [
            ...
            ROOT / "outputs/metrics_matrix.csv",
            ROOT / "evidence/requests_log.csv",
            OLD_RUN / "records.jsonl",
        ]
        ...
        self.assertEqual(
            "2024-12-31", result["old_successful_candidate"]["filing"]["period_end"]
        )
        self.assertEqual(
            "2025-12-31", result["current_published"]["filing"]["period_end"]
        )
        self.assertEqual("0.693", result["new_candidate"]["results"][0]["value"])
        self.assertEqual(
            "ISOLATED_HISTORICAL_START_NOT_PRODUCTION", result["baseline_mode"]
        )
        self.assertEqual(before, [p.read_bytes() for p in watched])
        capture = self.root / "comment.json"
        capture.write_text(json.dumps(self.comment))
...
"""
    ...
    def test_failure_retains_old_success_and_consumes_stage(self):
        ...
        with provider_boundary(plan, usage="excess") as opened:
            ...
            self.assertEqual(1, opened.call_count)
        self.assertFalse(
            (Path(self.stage["stage_root"]) / "successful-candidate.json").exists()
        )
        self.assertEqual(
            "2024-12-31", failed["old_successful_candidate"]["filing"]["period_end"]
        )
    ...
    def test_authority_input_and_owner_tamper_fail_before_provider(self):
        plan = self.plan()
        path = self.data / "catalog/table_task_contracts.json"
        ...
        source = (
            self.data
            / plan["prepared_input"]["source_proofs"][1]["request_repo_relative_path"]
        )
    ...
    def test_stage_slot_is_global_to_input_plan_and_restart(self):
        plan = self.plan()
        runtime._exclusive_slot(self.stage, plan)
        with self.assertRaisesRegex(ValueError, "ALREADY_CONSUMED"):
            runtime._exclusive_slot(self.stage, plan)
        changed = copy.deepcopy(plan)
        changed["plan_id"] = "sha256:" + "a" * 64
        with self.assertRaisesRegex(ValueError, "ALREADY_CONSUMED"):
            runtime._exclusive_slot(self.stage, changed)
        result = runtime.run_update(approval_url=URL)
    ...
    def test_uncommitted_new_input_bytes_do_not_change_code_identity(self):
        ...
        inventory = update.saved_source(
            repo_root=self.data,
            url="https://data.sec.gov/submissions/CIK0001048286.json",
        )
        payload = json.loads(inventory["raw"])
        ...
        client = sec_http.SecHttpClient(
            workdir=self.data,
            config_path=self.data / "config/sec_config.json",
            log_path=self.data / "evidence/requests_log.csv",
        )
        client.config = {**client.config, "max_retries": 0}
        with mock.patch.dict(
            os.environ, {"SEC_CONTACT_EMAIL": "runtime-tests@secmetrics.org"}
        ), mock.patch.object(
            sec_http, "urlopen", return_value=Response(json.dumps(payload).encode())
        ):
            client.fetch(
                url=inventory["proof"]["source_url"],
                purpose="SIMULATED_HTTP_BOUNDARY",
                local_path=self.data / "evidence/test/new-list.json",
            )
        binding = runtime.verify_stage(approval_url=URL)
        plan = runtime.prepare_plan(binding=binding)
        self.assertNotEqual(
            inventory["proof"]["content_sha256"],
            plan["prepared_input"]["source_proofs"][0]["content_sha256"],
        )
        self.assertEqual(before, runtime.code_identity())
        self.assertFalse((self.data / ".git").exists())
        self.assertEqual(
            "2025-12-31",
            plan["prepared_input"]["table_input"]["target_period"]["period_end"],
        )
