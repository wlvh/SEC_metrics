...
from vnext.continuous_semantic_calls import prepare_requests, build_plan, execute_d04_assessment, execute_feasibility


class D04NativeAssessmentMaterialTest(unittest.TestCase):
    def test_actual_factory_native_acceptance_and_omitted_unit_failure(self):
        requested = os.environ.get('D04_ASSESSMENT_MATERIAL_ROOT')
        if requested:
        ...
        with patch.object(socket.socket, 'connect', side_effect=AssertionError('NETWORK_FORBIDDEN')), \
            ...
            def wire(value):
                return canonical_json_bytes(value={'id': 'd04-offline-native', 'model': 'deepseek-flash',
            ...
            class FakeResponse(io.BytesIO):
                headers = {'x-request-id': 'd04-offline-native'}
            ...
            with self.assertRaisesRegex(ValueError, 'D04_NATIVE_REQUEST_REQUIRES_NATIVE_ENTRY'):
                execute_feasibility(prepared=prepared, ledger=ledger, recorded_wire=raw)
            path, outcome = execute_d04_assessment(prepared=prepared, ledger=ledger, recorded_wire=raw)
            ...
            _, bad = execute_d04_assessment(prepared=prepared, ledger=failed,
            ...
            summary = {'status': 'OFFLINE_NATIVE_REQUEST_ACCEPTANCE_PASS', 'network_disabled': True,
                ...
                'real_calls': [0, 0, 0], 'complete_d04_result': False}
