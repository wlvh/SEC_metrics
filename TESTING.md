...
PR-3阶段A及financial request-shard的必跑离线定向证据为：`tests.vnext.test_compact_table_payload`（原有11组`decode(encode(expanded))`逐字段相等、compact篡改负例，以及连续shard的缺表/重复/乱序/单表超限/maximal packing负例）、`tests.vnext.test_financial_table_shards`（完整manifest保留、schema-v4 Candidate/no-candidate、shard ID/range/examined IDs内容哈希、Evidence/Review闭包、无候选只能生成`WITHHELD / SHARD_NO_CANDIDATE`、全部child Runs完成前无aggregate credit、candidate冲突→WITHHELD）、`tests.vnext.test_scope_contract`（D-31共享`99% one-day VaR` locator的多维raw proof、exact enum唯一自动规范化、unknown alias→`REVIEW_REQUIRED`且SYSTEM拒绝）、`tests.vnext.test_table_task_contracts`（SourceStrategy SHA-256绑定fallback representation schema的exact table family/metric set；lodging schema/prompt保持v3，financial只使用schema v4 shard contract）、`tests.vnext.test_table_qualification_authorization`（public/shared catalog LIVE callee缺opaque authorization时在source/transport前失败；test-only real-builder authority把exact binding写入Run/attempt/evidence/ledger/replay；target period、media、Run/terminal、family/task/ordinal/source/freeze/cycle/prompt/schema/shard以及evidence/ledger/request/transport mutation全部在opener或formal freeze前拒绝；WB-3与Run recovery/HTTP terminal矩阵保持原语义）和`tests.vnext.test_table_qualification_freeze`（200000/200001 inclusive boundary、传递shared/family-local closure、11组round-trip、每个development source×task envelope与split-cost receipt；financial每task必须是679 tables、当前27个连续shards、最大exact estimate<=200000，伪造live-ready或incomplete coverage即拒绝）。冻结后运行`tools/freeze_table_qualification.py --freeze-commit <sha> --frozen-at-utc <UTC>`与`tools/create_stage_a_validation_snapshot.py --frozen-at-utc <UTC>`；`python3 tools/check_validation_snapshot.py`必须同时验证历史R2 artifact bytes与current Stage-A source overlay。所有freeze工具只跑本地WB-3 mock回归和已有SEC bytes；它们必须记录`actual_prompt_tokens=NOT_RUN`、three real egress counts=0、R3 active/root before-after equality。effective D-07继续以`utf8_byte_upper_bound` v1执行inclusive 200000门；financial packing另为最终set-ID替换保留71 UTF-8 token机械上界并再次验证每个最终provider envelope。family-local failure只阻断该family，shared protected-closure drift阻断全部依赖family。lodging historical attestation blocker继续按已批准qualification-usage-only路径处理；financial materialization resource gate已PASS，request shard context gate离线PASS，但current freeze/Stage-A、concrete D-07 scope和exact-head review完成前仍不得真实调用。
...
Stage-A的post-snapshot SEC证据追加还必须运行`tests.vnext.test_stage_a_post_snapshot_append`：历史request ledger bytes必须是current ledger的精确前缀，current manifest必须覆盖完整追加集，新增immutable body/header exact set必须与新增ledger行逐项对应且digest/size/HEAD bytes一致；缺失任一文件或改变历史前缀都必须fail closed。该overlay不得重写历史validation sidecar。

financial pre-egress terminal控制必须同时运行`test_retryable_failure_can_be_authorized_one_shot`、`test_retryable_terminal_is_one_shot_and_binds_cycle_evidence`与`test_next_financial_task_stops_after_prior_task_terminal`：qualification policy的retry=0必须在共享WB-3实际产生至多一个marker；429/TIMEOUT/recoverable-5xx直接形成单attempt `FAILED_RETRYABLE_FINAL`。任一prior financial parent/child的FAILED、batch terminal、UNKNOWN、PENDING、缺Run materialization或乱序namespace都必须在next child authorization/provider opener前阻断；只有完整FROZEN prior parent和SUCCEEDED WB-3 exact set可推进。
...
Stage-B financial grid调查现在作为immutable pre-policy evidence读取：`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_table_stage_b_financial_grid_census -v`重算receipt content ID并锁定exact JPM 12927325 bytes、679 tables、60348 origins/source cells、124761 rectangular cells、62748 span duplicates、1665 synthetic blanks、0.01334552/0.50294563 ratios及table_000588的99975→100050旧gate crossing。不得在D-35 cap生效后重签该历史receipt或把其`selected_option=null`改写成OPTION-A。

current financial resource gate运行`PYTHONDONTWRITEBYTECODE=1 python3 tools/benchmark_jpm_full_materialization.py --validate`及`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_table_stage_c_financial_materialization -v`。测试继续重验pinned Linux image、read-only mount、512 MiB cgroup-v2、120秒wall、network-none route closure、JPM 679/124761完整materialization、process/cgroup peak、canonical bytes、DerivedAsset、0/0/0 egress与历史100000→124761 D-35 transition；current D-35 successor另要求BOA 369/200229 hard-guard characterization逐字段绑定、production cap精确为200229且local shards=false，并用current production parser实际重建同一JPM DerivedAsset。historical Darwin NOT_RUN与FAILED_CHILD receipts保留，不得被current pointer覆盖后删除。
