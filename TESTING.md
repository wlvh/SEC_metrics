...
|---|---|---:|---:|---|---|
| vNext 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4`；同一命令由 `.github/workflows/vnext-fast.yml` 在 PR 上执行 | 否 | 否；二十个隔离子进程条目只读当前工作树 | 二十个直接边界条目并发返回 0，输出标记 `FAST_LOCAL_ONLY`；Requirement smoke只加载parent/child authority，publication smoke只绑定active R3 pointer/manifest edge并完整验证exact R2 predecessor，不在30秒tier内重放portable R3 closure；Stage-C继续验证历史packet、exact attestation、usage terminal、sibling no-bound evidence与post-attestation packet | Issue #15 full evidence、portable R3 closure、AI live/Cutover |
...
1. 先运行 `python3 tools/run_fast_tests.py --jobs 4`；再把 Issue #15 全authority历史闭包、active R3→exact R2 chain和R3 formal read-back作为integration gates运行。fast tier保留Requirement loader与active-edge/R2 bundle smoke，但不重放portable R3 closure；不得把全量 `tests/vnext/`、Python 3.9 双跑、隔离仓库或其他长串行套件作为开发、PR 或 final acceptance 必跑项。Issue #15 D-26 明列的短小确定性不变量测试按受影响范围执行。
