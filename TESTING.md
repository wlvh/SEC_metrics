...
| WB-2B deterministic router | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_deterministic_router tests.vnext.test_zero_ai_release.ZeroAiReleaseTest.test_r2_financial_producer_has_no_legacy_semantic_input -v` | 否；socket constructor 是立即失败 canary | 否；只写系统临时 fixture | 五adapter、统一`sources[]`、SourceSetManifest、14财务producer无legacy row/evidence/expected-value入参、XBRL context维度、C01/E03共用claim、E02/E04完整set及E01事后parity通过 | 正式R2 publication、WB-3 invocation control |
