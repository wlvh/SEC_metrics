...
- immutable R2、exact R3 Addendum 与 Decision Register 共同形成 Requirement closure；历史 D-01 pending record 保留，但唯一 effective D-01 已由R5固定为DeepSeek OpenAI-compatible Chat Completions、`deepseek-v4-flash`、`api.deepseek.com`、temperature=0、thinking disabled、JSON output、strict Reader schema和无工具，并从模块固定 repository root 构造；调用方不能注入 policy/root/transport。D-06使HUMAN review可选，但SYSTEM decision只能使用固定身份、reason与完整claims。每次失败重试必须形成独立 attempt/Run，最多 initial + 2 retries；凭据只可注入本次进程，不能伪造 live evidence。
...
明确尚无本轮真实运行证据：live SEC acquisition/inventory receipt、第二个真实lodging filing、实现冻结后加入的独立holdout、remote live Reader连续三轮稳定性、全十家公司formal staging parity、active publication、真实rollback→report→Stage12→checker→restore，以及clean committed checkout的full acceptance。固定SEC阶段编排、旧producer退出、PublicationView consumers、fault/rollback primitives与new/rollback/restore终态编排已有代码/测试；实现测试不能替代现场receipts，缺项必须保持NOT_RUN/BLOCKED。首次A→B会创建previous publication，故其预先不存在不是blocker。
...
- migrated legacy writers/resolvers已fail closed，Stage10/11/12已具备active `PublicationView`分支，固定live SEC acquisition/inventory与publication/fault/rollback/new→rollback→restore终态编排也已有动态测试；但本轮尚未执行live acquisition，仓库当前也没有对应receipt、committed active pointer、十公司formal staging、真实rollback/restore terminal cycles或full receipt，root outputs仍是业务读取入口。首次initial A→B会创建predecessor，故其预先缺失不是blocker。
