...
历史期间安装与冷重放：`HISTORICAL_PACKAGE_MATERIAL_ROOT=/absolute/new/root PYTHONPATH=scripts python3 -m unittest -v tests.vnext.test_historical_package_material`，独立 CI 作业 `historical-period-package`。覆盖异目录安装后在数据根内重建出相同绑定、新进程冷重放恢复相同绑定/期间/数值/选择身份且新增调用为0、两个期间并存不互相覆盖、未知绑定被拒。该测试不创建原生 Run，也不冒称自包含包：重放进程是新的、无网络、输入与来源全部来自已安装数据根，但代码来自开发 checkout。这不是偷懒——`_external` 以 `B06_EXTERNAL_CANDIDATE_ROOT_REQUIRED` 明确拒绝把数据根同时当作代码根，自包含包被现有不变量禁止。原生 Run 的接缝成本由 `tools/vnext_requirement_seam.py` 实测，见 `docs/evidence/issue47_history/requirement-seam-2026-09-18/`：已安装的包不受影响，受影响的是接缝改动之后新安装的包，需要 #28 在其下一次 Requirement 世代里一并重记两个哈希。不在此处以放宽冻结合同的方式绕过。

Requirement 接缝成本实测：`python3 tools/vnext_requirement_seam.py --data-root <已安装的历史包> --work <新的外部目录> --binding-id <绑定> --company-id <公司> --metric-id <指标>`。它复制该包、在副本上加一条引擎登记与一条 Run 授权分支，然后分进程测量同一个已安装包分别被原运行时和改动后的运行时读取的结果。它不写开发 checkout、不写 #28 的工作现场、不创建 Run、不发任何请求。这是一次测量而不是一个测试：它的结论会随 `issue_28_v13`／`issue_28_v14` 的 manifest 变化，所以不进任何测试层，只在需要重新核对接缝成本时运行并把输出存进证据目录。

历史期间 CI 作业待人工添加：本会话的 GitHub App 没有 `workflows` 权限，无法提交 `.github/workflows/vnext-fast.yml`。需要在该文件中新增一个与既有原生作业同形的作业，仓库维护者可直接照抄：作业名 `historical-period-package`，`runs-on: ubuntu-latest`，`timeout-minutes: 20`，步骤为 checkout、`actions/setup-python` 3.14，然后以 `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts HISTORICAL_PACKAGE_MATERIAL_ROOT=${{ runner.temp }}/historical-period-${{ github.run_id }}-${{ github.run_attempt }}` 运行 `python3 -m unittest -v tests.vnext.test_historical_package_material`。在该作业加入前，这条材料测试只有本地执行记录，不能写成 CI PASS。
