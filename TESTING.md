...
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；十个子进程只读当前工作树 | 十个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY` | CI、full evidence、Golden、live/Cutover |
...
| WB-2 SourceStrategy | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_source_strategy_registry -v` | 否 | 否；负例只篡改系统临时副本 | 39 metric exact set/四种 source mode、family literal、ReleasePlan state separation、authority hashes和 root matrix SHA-256 不变均通过；`ai_event_text`、当前迁移状态、通用禁词与 metric-level state 负例 fail closed | WB-2B adapters、provider 零调用、active publication |
| WB-2B deterministic router | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_deterministic_router -v` | 否；socket constructor 是立即失败 canary | 否；只写系统临时 fixture | 五 adapter、统一 `sources[]`、submissions-derived SourceSetManifest、DeterministicVerifiedClaim→Observation→Result/Trace、C01/E03 共用 claim、E02/E04 CLOSED_WORLD 零值、E01 key-set legacy exact parity 和无 metric-specific branch 全部通过 | 真实十公司 R2 publication、WB-3 invocation control |
| WB-3 invocation control | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_invocation_control -v` | 否；只有 injected `MOCK` transport | 只写系统临时 invocation namespace | 三层身份、exact success reuse、`O_EXCL` single-flight、immutable attempts、terminal/retry、402 停批、ABANDONED/UNKNOWN、usage/cache/cost 非阻断观测、payload/context hard limit、structured-only 三计数全 0 均通过 | 真实模型 egress、AI qualification、active publication |
...
本轮真实运行证据已包括 live SEC acquisition/inventory receipt、Hilton第二真实布局、freeze后Hyatt独立holdout及qualification `PASSED`。但remote live Reader三次均因provider `Insufficient Balance`失败，因此尚无全十家公司formal staging parity、active publication、真实rollback→report→Stage12→checker→restore或clean committed checkout的full acceptance。固定SEC阶段编排、旧producer退出、PublicationView consumers、fault/rollback primitives与new/rollback/restore终态编排已有代码/测试；实现测试不能替代现场receipts，缺项必须保持NOT_RUN/BLOCKED。首次A→B会创建previous publication，故其预先不存在不是blocker。
...
`config/validation_source_policy.json`把`fixtures/`纳入runtime source，并把`artifacts/vnext/qualification`、`evidence/request_attempts`、`outputs/failure_first_receipts`、`outputs/publication_fault_receipts`与`outputs/vnext_cutover_audits`列为full artifact directories。full正例必须真实创建这些目录的本轮content-addressed bytes；缺失、symlink、extra/tamper或只引用临时文件均失败。
...
- migrated legacy writers/resolvers已fail closed，Stage10/11/12已具备active `PublicationView`分支，固定live SEC acquisition/inventory与publication/fault/rollback/new→rollback→restore终态编排也已有动态测试；本轮SEC acquisition及qualification receipts已生成，但live Reader三次均因provider `Insufficient Balance`失败，故仍没有committed active pointer、十公司formal staging、真实rollback/restore terminal cycles或full receipt，root outputs仍是业务读取入口。首次initial A→B会创建predecessor，故其预先缺失不是blocker。
