...
- `tools/run_acceptance.py --scope recorded` 的离线边界覆盖整个子进程树：当前 macOS 支持路径必须经 `/usr/bin/sandbox-exec` 执行 `(deny network*)`，Python `sitecustomize` audit hook 只作为第二道诊断保护。`sandbox-exec` 缺失时稳定返回 `OFFLINE_PROCESS_SANDBOX_REQUIRED`，不得降级成较弱的同进程 socket monkeypatch；recorded gates 及 full 的 terminal-validation 子进程都会剥离 `OPENAI_API_KEY` 和 `SEC_CONTACT_EMAIL`。
- immutable R2、exact R3 Addendum 与 Decision Register 共同形成 Requirement closure；历史 D-01 pending record 保留，但唯一 effective D-01 已由 R3 批准。remote adapter 固定为 OpenAI Responses API、`gpt-5.6-terra`、`api.openai.com`、`store=false`、strict Structured Output、无工具，并从模块固定 repository root 构造；调用方不能注入 policy/root/transport。每次失败重试必须形成独立 attempt/Run，最多 initial + 2 retries。本轮环境的 `OPENAI_API_KEY` 与 `SEC_CONTACT_EMAIL` 均缺失，因此 live/full 只能稳定 BLOCKED，不能伪造 live evidence。
...
明确尚无本轮真实运行证据：live SEC acquisition/inventory receipt、第二个真实lodging filing、实现冻结后加入的独立holdout、remote live Reader连续三轮稳定性、具名HUMAN Decision、全十家公司formal staging parity、active/previous publication、真实rollback→report→Stage12→checker→restore，以及clean committed checkout的full acceptance。固定SEC阶段编排、旧producer退出、PublicationView consumers、fault/rollback primitives与new/rollback/restore终态编排已有代码/测试，但本轮因secrets/qualification/HUMAN缺失未执行live path；实现测试不能替代现场receipts，缺项必须保持NOT_RUN/BLOCKED。
