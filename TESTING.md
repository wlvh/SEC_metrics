...
|---|---|---:|---:|---|---|
| vNext 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4`；同一命令由 `.github/workflows/vnext-fast.yml` 在 PR 上执行 | 否 | 否；23个子进程条目只读当前工作树 | 原20条加B0原生Evidence/零调用class/UNKNOWN-stop三个短smoke，全部并发返回0并标记`FAST_LOCAL_ONLY`；不在30秒tier内重放portable R3 closure，未移除原case或提高timeout | 真实来源R4离线资格、10倍性能、live/full acceptance |
