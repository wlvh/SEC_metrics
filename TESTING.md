...
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；十三个子进程只读当前工作树 | 十三个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY`；其中R2用例读取已提交formal active artifacts，Stage-C只验证mock与honest guard terminal | CI、Issue #15 full evidence、AI live/Cutover |
