...
current financial resource gate运行`PYTHONDONTWRITEBYTECODE=1 python3 tools/benchmark_jpm_full_materialization.py --validate`及`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_table_stage_c_financial_materialization -v`。测试同时重验pinned Linux image、read-only mount、512 MiB cgroup-v2、120秒wall、network-none route closure、679/124761完整materialization、process/cgroup peak、canonical bytes、DerivedAsset、0/0/0 egress、benchmark-time 100000未改、effective D-35 transition与current production 124761；还用current production parser实际重建同一679-table DerivedAsset。historical Darwin NOT_RUN与FAILED_CHILD receipts保留，不得被current pointer覆盖后删除。
