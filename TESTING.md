...
它并发执行直接、非隔离边界用例，每例硬上限 30 秒；Requirement/WB-2/WB-2B/WB-3/R1及Stage-C/D用例覆盖 authority closure、producer scope、39指标registry、E01 parity、invocation故障矩阵、JPM guard terminal、Stage C-A/C-B历史packet content identity、consumed occupancy measurement/no-credit边界、RevPAR exact plan/one-shot marker、exact context attestation、usage terminal、sibling no-bound evidence、post-attestation packet与formal active read-back。recorded acceptance 的任一快速/静态 gate 硬上限 60 秒。不得把全仓 discover、Python 3.9 双跑、临时 Git repository/worktree 或长串行套件列为必跑项。金额 budget preflight 已从必测集删除；仍必须覆盖 single-flight、HTTP 402 一次调用后停批、UNKNOWN_REMOTE_OUTCOME 不自动重试、frozen replay/rollback/restore 零网络和 structured-only 零模型调用。receipt 的最高测试状态仍是 `PASSED_FAST_LOCAL_ONLY`，绝不等同 CI 或Issue #15 full acceptance。
...
one-shot measurement回归运行`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_table_context_measurement -v`和`PYTHONDONTWRITEBYTECODE=1 python3 tools/vnext_table_context_measurement.py plan`。当前plan必须解析为`lodging_revpar_table_v2`、Marriott exact SHA、serializer v2、392438-byte exact provider request和普通200000 blocker，绝不从occupancy usage外推。mock矩阵覆盖exact review/head/request authorization→一次调用、第二次零调用拒绝、HTTP 402一次后停止、UNKNOWN不重试、缺usage→`FAILED_USAGE_UNAVAILABLE`、marker前失败不消费但不自动继续，以及family/task/source/request/prompt/schema/head/review URL篡改在opener前失败。并发正例仍要求两个进程总mock opener严格为1。独立measurement evidence必须被通用record/qualification/replay/publication边界拒绝；真实`execute`只能在独立top-level review comment绑定clean exact HEAD和provider request SHA后运行一次。
...
Stage D RevPAR真实命令同样不是回归测试：先提交并推送plan/executor exact head，由独立reviewer在本PR top-level comment同时写明head、tree、plan ID和provider request SHA；随后用同一评论URL、request SHA、head、UTC与授权词运行唯一一次`execute`。任何marker后绝不重跑。若terminal为`FAILED_USAGE_UNAVAILABLE`或actual prompt>200000，只保存原始terminal并输出`OWNER_DECISION_REQUIRED`；若actual prompt<=200000，运行`python3 tools/create_table_context_feasibility_attestation.py --task-contract-id lodging_revpar_table_v2`和其`--validate`，不得创建新packet。之后重建schema-v4 freeze并要求occupancy与RevPAR两个required task/request均以各自exact attestation通过；measurement response仍不能成为qualification ordinal。

post-attestation PR只运行离线命令：`python3 tools/create_table_context_feasibility_attestation.py --validate`、`python3 tools/investigate_sibling_table_context.py --validate`、重建freeze/Stage-A/schema-v4 owner packet后运行`python3 tools/create_stage_c_context_attestation_packet.py --validate`，并运行四个对应test modules。禁止运行measurement `execute`或qualification executor。必须逐项证明exact attested request context PASS、source/task/prompt/schema/serializer/provider/model/API/request drift在opener前BLOCK、sibling不能借用、measurement response/evidence不能qualification reuse、usage missing/excess terminal/no retry/no later ordinal、family overall仍false、financial F3、active/root不变和本PR0/0/0 egress。offline comparison只允许exact byte/component结论；若无仓库授权的严格跨task上界，必须保留`EXACT_CONTEXT_EVIDENCE_REQUIRED / NO_SOUND_CROSS_TASK_TOKEN_BOUND`。
...
- `requirements/issue_15_v1/` 是后续开发 authority；其 loader 显式验证 exact Contract、parent closure、13 条历史 Decision、D-01 与 post-freeze D-36/D-35/D-26/D-07 effective tips、D-07 same-ID parent hash、D-30–D-38 roots、producer/matrix/foundation receipts。现有 inherited Reader/Cutover runtime 在对应 WB 实施前不得被 Requirement-only 证据写成已切换 production semantics。

## 3. 真实测试与验证层级

| 层级 | 命令 / 入口 | 网络 | 仓库写入 | 通过条件 | 不能替代 |
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；十九个子进程只读当前工作树 | 十九个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY`；其中R2读取已提交formal active artifacts，Stage-C验证历史packet、exact attestation、usage terminal、sibling no-bound evidence与post-attestation packet | CI、Issue #15 full evidence、AI live/Cutover |
...
| WB-2 SourceStrategy | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_source_strategy_registry -v` | 否 | 否；负例只篡改系统临时副本 | 39 metric exact set/四种source mode、family literal、不可变R1→R2 parent/delta/cumulative-key chain、reader versions、historical plan closure/current Requirement separation与authority hashes通过；删除B01、parent key或parent retired producer并完整重签plan/content/index仍由no-removal语义门拒绝；伪造parent/closure及旧单一可覆盖plan均fail closed | WB-2B adapters、provider零调用、active publication |
...
| Issue #15 zero-AI R2 active | 同上 | 否 | 否；独立projection canary写系统临时Run，formal证据只读 | 22指标、220坐标、141×20 strict-compatible字段、79 structural新增、309-row exact union、submissions+acquisition完整event set、事后event-key parity、projection-bound retirement/read-back与三种零调用计数通过 | WB-4以后、AI Reader、39指标最终Cutover、full acceptance |
...
本轮真实运行证据包括Issue #15 zero-AI R1 A→B→A→B历史和R2 cumulative formal active：22指标、220坐标、141 legacy替换、79 structural新增、309行matrix，model egress/paid call均为0。remote AI Reader三次仍因provider `Insufficient Balance`失败，因此WB-4以后、AI指标、39指标最终Cutover与full acceptance尚未完成；R1/R2 receipts不能替代这些后续证据。
...
1. 先运行 Issue #15 authority 定向测试，再运行 `python3 tools/run_fast_tests.py --jobs 4`；不得把全量 `tests/vnext/`、Python 3.9 双跑、隔离仓库或长串行套件作为开发、PR 或 final acceptance 必跑项。Issue #15 D-26 明列的短小确定性不变量测试按受影响范围执行。
...
- 本轮已有 Marriott 真实 filing 的 recorded production-path fixture及cold-start sandbox publication测试，但第二个 materially different lodging layout 和 production freeze 后的独立 holdout 尚无合格真实 bytes/receipts；资格receipt必须绑定有效 HUMAN 或D-06 SYSTEM `APPROVE`、全量`PUBLISHED` Result和`PASSED` validation，`REJECT`/WITHHELD只能形成审计Run；sandbox active只属于临时 `<workspace>/recorded-publication`，不是正式active证据。
- D-01 已由R5形成唯一effective DeepSeek决定，D-06使HUMAN可选；尚无remote live Reader三轮稳定性证据。
- Issue #15 R1/R2已在正式root形成A→B→A→B历史与R2 successor；`tests/vnext/test_zero_ai_release.py`重验完整predecessor chain、309-row key set、141 strict compatibility、79 structural additions、event parity、retirement与逐byte root mirror read-back，real model egress和paid call均为0。这只证明zero-AI R2，不能写成WB-4以后、39指标最终Cutover或full acceptance。
