...
- registered qualification fixture回归必须使用历史FY2024 OPEN Run证明：即使`marriott_international`存在于production registry，manifest含LIVE `qualification_authorization`时traits/CIK仍由authorization点名fixture重建；普通production Run保持registry-first，无authorization外部fixture保持registry-miss fallback。历史159479/562/160041 success、Evidence/Review/Result不得因本地finalization bug获得qualification credit或被新cycle复用。
