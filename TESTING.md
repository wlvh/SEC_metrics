...
Form 10-K 不编号项的章节边界：`PYTHONPATH=scripts python3 -m unittest tests.vnext.test_historical_text_boundary`（saved-source 层，实测 44 秒，读 Pfizer 与 Macy's 两份年报原件）。四个用例的预期值都从文档自身的块读出，不由被测边界规则产生：Pfizer 的 ITEM_3 必须结束在标题块自己的下标；修正后的块集合必须是冻结集合的**真子集**，且差集恰好等于冻结集合与高管章节的交集（"没有少抓"的强形式，不是章节数比较）；Macy's 第 774 块在 Item 8 内点名首席执行官却不是标题，它的整套记录必须与冻结模块逐字节相等——语料自带这个反例，不需要构造；路由只把 D02 接到后继模块，C02 仍回父模块。另两个用例覆盖被引用附注：Lumen 的 Note 17 由 Item 3 并入，必须`EXACT_NOTE`解析且其范围内每个实体块都进入结果（预期由附注自身的块算出），冻结集合一块不少；Pfizer 的 Note 16A 无同号标题、解析为`WIDER_PARENT_NOTE`，必须不被整取。同一份语料的另一半是回归护栏：边界修复后九家里八家逐字节不变、Pfizer 58 条 22,982 字降到 32 条 18,373 字；附注修复后四家增加而零块丢失（Lumen 15→58、Paramount 16→59、Marriott 10→30、Salesforce 9→15），其余五家逐字节不变。六个用例实测 91.7 秒。
