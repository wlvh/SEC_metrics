...
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；九个子进程只读当前工作树 | 九个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY` | CI、full evidence、Golden、live/Cutover |
...
- formal operator/review CLI 覆盖命令面、稳定错误、JSON、默认无 traceback、TOCTOU、Decision 单链与恢复命令；模型、fixture 与 runner均不能伪装为HUMAN，缺HUMAN时仅D-06固定SYSTEM decision可APPROVE。Cutover orchestrator 覆盖 effective D-01 派生 retry、每次失败独立封存、initial+2 exhaustion、三次成功稳定性、qualification gate、review/compatibility blocker与 commit read-back。
