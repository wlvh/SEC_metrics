...
| Issue #15 R3 offline prepare | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_ratchet_release -v` | 否 | 只写系统临时workspace与publication root；正式pointer/root逐byte不变 | 十个committed qualification terminal重跑DerivedAsset/Evidence/Review/Calculator；20-coordinate delta Batch、18个零AIstructural Runs、legacy-A strict oracle、active-R2 assembly、327-row key union、portable bundle closure与tamper负例通过 | R3 active commit、R4、full acceptance |
