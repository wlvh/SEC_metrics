...
one-shot measurement当前只运行terminal read-back：`PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.vnext.test_table_context_measurement.TableContextMeasurementTerminalTest -v`。它重验RevPAR plan/marker/raw/evidence、160928/535/161463 usage、1/1/0计数、non-credit flags与永久consumption；任何`plan`或`execute`现在都必须稳定返回`AUTHORIZATION_CONSUMED`。历史mock/并发/tamper矩阵保留在其reviewed Git head，不得为测试再次打开真实authorization。
...
lodging qualification source/phase回归运行`PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tests.vnext.test_table_qualification_samples -v`。它必须证明Hilton second-layout、Hyatt post-freeze holdout、Marriott fresh source都从matrix/fixture manifest解析到immutable attempt；两个task的provider request SHA/estimates exact且plan阶段socket=0。外部layout issuer不得写入production company registry；adapter第二次source replay只能从matrix点名fixture的exact manifest取得唯一CIK，并继续重验SEC URL/accession/document与immutable request ledger，错误CIK必须fail closed。真实顺序固定为：两个`table-plan --phase SECOND_LAYOUT --ordinal 1`经exact-head review后分别`table-execute`并FROZEN → `table-freeze`绑定production semantic tree和provider-ledger prefix → 两个holdout plan/execute/FROZEN → 每task三个fresh ordinals。Hyatt estimated超200000的两个request必须在review comment逐SHA绑定，并由新response actual usage terminal裁决；缺usage/超限停止后续样本。每个Run都要重验qualification ledger/Evidence、Review、Result、validation/freeze，不得复用measurement raw response。
