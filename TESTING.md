...
|---|---|---:|---:|---|---|
| vNext 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；二十个隔离子进程条目只读当前工作树 | 二十个直接边界条目并发返回 0，输出标记 `FAST_LOCAL_ONLY`；Requirement smoke只加载parent/child authority，publication smoke只绑定active R3 pointer/manifest edge并完整验证exact R2 predecessor，不在30秒tier内重放portable R3 closure；Stage-C继续验证历史packet、exact attestation、usage terminal、sibling no-bound evidence与post-attestation packet | CI、Issue #15 full evidence、portable R3 closure、AI live/Cutover |
