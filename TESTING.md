...
最新same-ID D-07 tip只批准实现一个exact lodging actual-token measurement exception；authority定向测试必须重算三段D-07单链、exact exception字段、owner comment证据、D-01 8 MiB payload护栏及DeepSeek provider/model/API/1000000 context authority。`live_measurement_authorized=false`仍禁止普通调用；唯一Stage C-B execution必须另由独立review绑定exact head。普通qualification继续以392447>200000在provider opener前阻断。
...
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；十五个子进程只读当前工作树 | 十五个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY`；其中R2用例读取已提交formal active artifacts，Stage-C验证mock、honest guard terminal、历史Stage C-A packet与只读Stage C-B terminal overlay | CI、Issue #15 full evidence、AI live/Cutover |
