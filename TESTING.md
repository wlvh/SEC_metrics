...
PR-3阶段A新增的必跑离线定向证据为：`tests.vnext.test_compact_table_payload`（Marriott、Hilton七组、Hyatt三组的`decode(encode(expanded))`逐字段相等及compact篡改负例）、`tests.vnext.test_scope_contract`（D-31共享`99% one-day VaR` locator的多维raw proof、exact enum唯一自动规范化、unknown alias→`REVIEW_REQUIRED`且SYSTEM拒绝）、`tests.vnext.test_table_task_contracts`（SourceStrategy SHA-256绑定fallback representation schema的exact table family/metric set、legacy qualification prepare 在D-07前停止、catalog task以Issue #15 retry-zero remote observation完成`Run→SYSTEM review→freeze→fresh replay`，以及catalog task+parent Requirement hashes fail closed）、`tests.vnext.test_table_qualification_authorization`（public/shared catalog LIVE callee缺opaque authorization时在source/transport前失败；test-only no-D07 authority以mock provider只形成一次cycle-bound canary，并将同一binding写入Run/attempt/evidence/ledger/replay；family/task/ordinal/source/freeze/cycle/prompt/schema和root/source drift全部在opener前拒绝）和`tests.vnext.test_table_qualification_freeze`（传递shared engine closure、family-local matrix/task/MetricSpec mutation、11组round-trip、每个本地development source×task envelope与split-cost receipt）。冻结后运行`tools/freeze_table_qualification.py --freeze-commit <sha> --frozen-at-utc <UTC>`与`tools/create_stage_a_validation_snapshot.py --frozen-at-utc <UTC>`；`python3 tools/check_validation_snapshot.py`必须同时验证历史R2 artifact bytes与current Stage-A source overlay。所有工具只跑本地WB-3 mock回归和已有SEC bytes；它们必须记录`actual_prompt_tokens=NOT_RUN`、three real egress counts=0、R2 active/root before-after equality。若任一estimated input token超过100000，或完整expanded grid/模型context遇resource硬限，结果只能是`D07_DECISION_REQUIRED`；resource-limited request必须显式记录`NOT_AVAILABLE_RESOURCE_LIMIT`，不得加入selector或继续qualification。
