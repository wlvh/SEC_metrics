...
| R4 短引用选择接口 | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=scripts python3 -m unittest tests.vnext.test_r4_cell_selection -v` | provider/SEC/socket 禁止 | 当前源码副本，独立 offline 实现绑定；九个明确 synthetic 新响应 | 原始九例分层只读核对；全部窗口/引用映射；值/位置/期间/单位/95-99/标签/未决负例；九项正常控制器接受，BAC A12 Run FROZEN 后新进程按保存版本回放 | 不修补旧 JSON，不重签旧证书，不给历史响应新 credit；无真实复测、通道切换、正式认证或发布 |
