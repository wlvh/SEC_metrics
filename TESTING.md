...
|---|---|---:|---:|---|---|
| R4 快速回归 | `PYTHONDONTWRITEBYTECODE=1 python3 tools/run_fast_tests.py --jobs 4` | 否 | 否；十一个子进程只读当前工作树 | 十一个直接边界用例并发返回 0，输出标记 `FAST_LOCAL_ONLY`；其中R1用例读取已提交formal active artifacts | CI、Issue #15 full evidence、AI live/Cutover |
...
本轮真实运行证据包括Issue #15 zero-AI R1十公司B01/B03 formal active、rollback/restore/read-back receipts，以及既有live SEC acquisition、Hilton第二布局和Hyatt holdout qualification。remote AI Reader三次仍因provider `Insufficient Balance`失败，因此R2、AI指标、39指标最终Cutover与clean committed checkout的full acceptance尚未完成。R1 receipts不能替代这些后续证据，缺项必须保持NOT_RUN/BLOCKED。
...
- Issue #15 R1 已在正式root以immutable Company Facts attempts完成十公司B01/B03 FROZEN Runs、verified legacy A导入、formal B提交、rollback到A与restore到B；`tests/vnext/test_zero_ai_release.py`重验active pointer、switch chain、232-row key set、strict compatibility、retirement与逐byte root mirror read-back，三种provider计数均为0。这只证明zero-AI R1，不能写成R2、39指标最终Cutover或full acceptance；历史live Reader三次`Insufficient Balance`仍说明AI档没有成功live证据。
