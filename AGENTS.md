...
- `scripts/vnext/ordinary_source_session.py`：源码外的测试来源会话，复用SEC原生持久化与日志追加，核对旧前缀、意图、终态及新请求证明；基础年度读取器可读新测试记录，输入比较忽略纯请求身份变化。只有测试信用，无真实获取、预算、普通Run或生产接线；见`docs/recorded_source_session.md`。
