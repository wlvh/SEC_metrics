...
- `scripts/vnext/stage_c_packet.py` / `tools/create_stage_c_a_packet.py`：Stage C-A answer-first packet与current-source overlay；严格分开approved/implemented/not-run/unauthorized/blocker，token authorization保持`NOT_ISSUED`。validator先要求historical R2仅source drift，再重建clean current source tree、new Requirement/D-07、plan/benchmark/root；不得重签PR #19历史对象。
