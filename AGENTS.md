...
- `scripts/vnext/table_context_measurement.py`：Stage C专用、与qualification隔离的一次性actual-token measurement边界；离线重建exact Marriott/lodging occupancy/serializer-v2/provider envelope plan，只有外部`AUTHORIZE_ONE_TOKEN_MEASUREMENT`绑定clean exact HEAD后才签发opaque capability。marker以专用`O_EXCL` claim实现进程间互斥，任何existing marker均返回`AUTHORIZATION_CONSUMED`，winner fsync文件与父目录后才进入opener；marker前失败不消费，任一marker后永久消费且零重试。独立evidence永不获得qualification/publication credit。Stage C-A只实现并mock测试；Stage C-B已在review 5014622571绑定的exact head上执行唯一一次真实请求并终态完成，authorization现已消费，禁止第二次调用。
...
- `scripts/vnext/table_context_attestation.py` / `table_context_comparison.py` / `stage_c_context_packet.py`：前者从Stage C-B immutable plan/marker/raw response/evidence/packet机械生成exact-request context attestation；中者离线比较attested与sibling完整provider bodies的hash/length/difference ranges及component hashes，禁止ratio外推；后者把attested request feasible、sibling evidence required、family overall false、measurement无qualification credit、financial F3、active R2/root不变和本PR 0/0/0 egress汇总为content-addressed Stage-C packet。
