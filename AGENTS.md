...
Issue #15 / `issue_15_v1` 是全部未来开发与验收的唯一入口。父快照继续提供不可变R2/R3、既有实现和历史Decision链；child snapshot以same-ID tips和D-30–D-38形成自包含authority。post-freeze D-36/D-35/D-26禁用仓库金额门禁，外部API账户余额是花费权威，cost/token/usage/cache只作非阻断observability；HTTP 402仍零自动重试并停止execution/batch，payload/context/resource仍是独立fail-closed安全类。D-07链保留全表、原序、无selector/prefilter、`utf8_byte_upper_bound` v1 inclusive 200000普通门与family-scoped failure domain。其one-shot measurement tip绑定Marriott development source、单一lodging task与serializer v2；独立review 5014622571随后只对exact head `451dd693175bea6c1196a09989c60017e96d63e7`授权一次执行，Stage C-B记录160937 prompt、576 completion、161513 total与historical real model/paid/SEC=1/1/0。authorization已永久消费，measurement无qualification/publication credit；JPM F3、R3/R4与active R2均未改变。WB-1/Decision/attestation只冻结和验证authority，不自动切换Reader、transport、publication或业务语义。

最新 owner exact-binding Decision 又追加同 ID D-07 successor：普通 `estimated_input_tokens <= 200000` 路径不变；仅当一个 content-addressed context attestation 对 provider request body、family/task/source、serializer/task/prompt/schema/provider/model/API、Requirement 与 protected closure 全部 exact 相等，且 Stage C-B provider-reported actual prompt 不超过 200000 时，才允许该一个 task/request 以 `PROVIDER_REPORTED_EXACT_BINDING` 通过 context gate。当前 Marriott attested request context feasible，但 sibling task 仍为 `EXACT_CONTEXT_EVIDENCE_REQUIRED / NO_SOUND_CROSS_TASK_TOKEN_BOUND`，所以 lodging family overall 仍未 ready、`live_ready_family_ids=[]`。measurement response继续禁止qualification reuse；authorization已永久消费，additional measurement与live qualification仍未授权。本PR的所有新freeze/packet/analysis均为0/0/0 real model/paid/SEC egress，不改变financial F3、R3/R4、active R2或四个root业务文件。
...
- `scripts/vnext/table_context_measurement.py`：与qualification隔离的同一one-shot actual-token measurement边界；Stage C-B occupancy历史authorization/marker/evidence已消费且不可改写，最新same-ID D-07 additive successor只把当前未消费目标固定为Marriott `lodging_revpar_table_v2`。plan继续绑定serializer v2、full table set原序、no selector/prefilter与exact provider body；opaque capability要求授权词、clean exact HEAD、独立PR top-level review comment URL和其绑定的request SHA。marker仍以`O_EXCL`跨进程互斥并fsync文件/父目录，任一egress永久消费且零重试；usage缺失稳定失败。schema-v2 marker/evidence记录review/head/task binding但仍无qualification/publication/reuse credit，不建立第二套executor、packet或state machine。
- `scripts/vnext/table_payload.py` / `scope_contract.py` / `table_task_contracts.py` / `table_qualification_freeze.py`：分别实现expanded grid可逆compact transport、多维shared-locator exact-enum scope、单角色catalog task与无网络qualification freeze。schema-v4 freeze在既有shared engine、family-local matrix/task/MetricSpec、11组round-trip、R2 root和zero-egress closure之上新增`readiness_by_task_request`；每个本地development source×task绑定完整SourceReference与exact provider request SHA，再以estimated bound或exact attestation逐request判定context，family `live_ready`只可由全部required task/request通过派生。当前attested request以`PROVIDER_REPORTED_EXACT_BINDING`通过，sibling request以`EXACT_CONTEXT_BINDING_MISMATCH`阻断，故lodging family overall仍false；financial仍为`EXPANDED_GRID_RESOURCE_LIMIT`，`live_ready_family_ids=[]`。唯一qualification executor继续重建matrix/freeze/Stage-A/source/task/provider opaque authorization，并新增measurement-response reuse prohibition与provider usage terminal policy；context PASS不授权qualification。shared drift仍传播全部dependent family，family-local drift只阻断owner family，Stage-A完整source tree仍由无family scope checker负责。
- `scripts/vnext/table_context_attestation.py` / `table_context_comparison.py` / `stage_c_context_packet.py`：前者保留并机械重验原occupancy attestation，additive RevPAR successor只允许四个明确authority/executor文件变化且provider request必须逐字段未变；RevPAR成功terminal复用同一attestation record type直接绑定plan/marker/raw/evidence，不新增packet或第二pointer。中者保留历史两request exact comparison与禁止ratio外推事实；后者仍是历史post-occupancy packet，不因RevPAR delivery改写。
