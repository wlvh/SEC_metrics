...
当前lodging authority在owner批准的compact prompt、same-target-table八字段locator、Marriott FY2024 second layout、Marriott FY2023 post-freeze holdout和Marriott FY2025 fresh source上冻结。Occupancy与RevPAR context均由各自provider-reported actual usage证明不超过200000；qualification没有复用measurement response。SECOND_LAYOUT、POST_FREEZE_HOLDOUT和三个FRESH ordinals按ordinal-major顺序形成十个独立provider execution，全部Evidence PASS、D-06 SYSTEM APPROVE、Result PUBLISHED、validation PASSED且usage terminal通过。任何新exact-head push不会重签这些已提交的无关family证据。financial先以exact JPM Linux benchmark把local cap从100000提高到124761；随后BOA second-layout在同一512 MiB/120s/network-none硬边界下以200229 cells、371380224 cgroup peak和4.330166s证明安全，owner选择A把current production `max_total_cells`最小提高到200229且不做local materialization shards。BOA/Citi immutable SEC attempts已分别进入second-layout/holdout matrix；provider request仍按task机械连续分片、全部shards完成前无qualification/publication credit。current freeze/Stage-A/plans和exact-head执行审核仍须在新head重建。
...
- financial qualification把authorization内`automatic_retry_count=0`传入同一WB-3 executor，因此retryable terminal也只能形成一个egress marker并直接`FAILED_RETRYABLE_FINAL`；普通非qualification路径仍保留D-35最多一次retry。每个financial child授权及egress前还按D-07 phase→task→shard顺序扫描current cycle Runs与plan-owned WB-3 namespaces：prior parent必须全shard FROZEN，prior sibling必须完整materialize且remote success；任一FAILED/batch-terminal/UNKNOWN/PENDING/缺Run或乱序状态阻断所有later financial plans。
...
- `scripts/vnext/table_payload.py` / `scope_contract.py` / `table_task_contracts.py` / `table_qualification_freeze.py`：分别实现expanded grid可逆compact transport、多维shared-locator exact-enum scope、单角色catalog task与无网络qualification freeze。lodging继续走完整schema-v3 request和已批准qualification-usage context path；financial schema v4在同一完整compact parent之上按exact provider envelope大小形成连续shards，每片绑定set/index/count/range/examined IDs与candidate/no-candidate disposition。离线JPM proof为每task 27片/679表/最大199912；local materialization对JPM/BOA/Citi仍各自一次完整完成，current统一cap为最大来源BOA的200229 cells，不做local shards。Candidate、Evidence、ReviewUnit、attempt、Run、WB-3 authorization和cycle aggregate共同闭合shard identity；缺片/重复/乱序、semantic selector、candidate后提前停止、无Evidence-pass candidate或candidate冲突均无qualification/publication credit。qualification executor仍沿同一task plan处理matrix-owned `SECOND_LAYOUT` / `POST_FREEZE_HOLDOUT` / `FRESH_STABILITY`，caller不能覆盖source/shard；usage缺失或超限零重试并停止后续shards/plans，measurement response仍禁止复用。
...
- `tools/freeze_table_qualification.py` / `tools/create_stage_a_validation_snapshot.py`：前者仅从现有本地SEC bytes、WB-3 mock regression和当前代码生成content-addressed table qualification freeze receipt；后者绑定当前clean Stage-A source tree与未变R2 root/历史provenance。历史sidecar之后新增的SEC证据只在current ledger精确以历史bytes为前缀、manifest覆盖完整追加集、每个新增body/header与追加行digest/size一一对应且全部已提交时，才作为overlay append接受；绝不重写历史sidecar。两者不得调用SEC/provider或进入qualification。
...
- `tools/investigate_jpm_financial_grid.py`：历史只读、interval-based JPM完整grid census；复用production raw parser/text transform但不构造完整expanded dict/list，输出679表逐表矩形/blank/span/text/canonical-size、旧100000门首次触发点和A/B/C decision-neutral option matrix。该immutable receipt不因后续D-35选择OPTION-A而重签。
- `tools/benchmark_jpm_full_materialization.py`：Stage C隔离benchmark入口；先保留Darwin `NOT_RUN_RSS_GUARD_UNAVAILABLE`与一次Linux child代码失败，再在pinned Python Linux镜像、read-only mount、512 MiB cgroup-v2、120秒wall、network-none下完成679表/124761 cells。receipt记录process RSS 282877952、cgroup peak 288043008、wall 5.10483s、canonical 22174348 bytes与零模型/SEC egress；历史D-35据此把cap从100000最小提高到124761。后继D-35以同样硬边界下的BOA 369表/200229 cells characterization把current cap最小提高到200229；两次选择都明确不做parser/materialization shards。provider request shards是另一边界，不能反写两项local resource结论。
