...
- `scripts/vnext/table_payload.py` / `scope_contract.py` / `table_task_contracts.py` / `table_qualification_freeze.py`：分别实现expanded grid的可逆compact transport、支持多维共享locator的Spec exact-enum scope v2、由SourceStrategy SHA-256绑定fallback representation schema机械派生的单角色table task catalog、以及无网络qualification freeze；catalog task实际进入Reader/adapter/attempt audit，金融/酒店各自绑定MetricSpec scope closure；unknown alias不由SYSTEM批准，freeze只绑定未来资格前提，不是live qualification。
