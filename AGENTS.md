...
Issue #47 C02的代理在获取门上根本不算依赖，以及一次由构建器自己挡下的提交：`plan_historical_sources`声明四个依赖类，没有治理类，所以C02第二级读的每一份DEF 14A都会被门以`HISTORICAL_URL_IS_NOT_A_DECLARED_DEPENDENCY`按名拒绝——**这条成立时，任何许可都解不开那些坐标**。这和事件类是同一个洞、晚一个指标，修法也同形：规划器是`issue_47_v1`的`NEW_RULE_FILE`，改它会移动上一批374个冻结Run所依据的closure，所以新增非规则文件`scripts/vnext/historical_governance_sources.py`，由`declared_frame`取并集。**声明不是重抄规则**：它问`prepare_historical_business_text_input`本身要读哪一份，和事件声明问`_event_sources`是同一条纪律——另写一遍proxy规则就是本模块要避免的漂移。十家实测，Marriott声明三份代理（2024-03-27与2025-03-27未存、2026-03-27已存），年报正文未存的期间什么都不声明并带具名限制。

**构建器第一次真正拦下了我**：`tools/vnext_historical_wiring.py`第一阶段报`ISSUE_47_WIRING_FIRST_PHASE_DID_NOT_PASS`、`artifact_released:false`，什么都没安装。挂的是元数据刷新链的`a refresh must not change what the frame declares`：115 != 116。**先测再判**——探针逐URL做集合差：**恰好多一行、零行消失**，是JPMorgan的FY2025代理`jpm-20260402.htm`，而治理限制由两条降为一条，少掉的正是2025-12-31那条。也就是说刷新解除了那个期间的选择阻塞，声明这才第一次推得出它要读哪一份；FY2024仍被挡住，因为它要的分片从未保存、派生索引不会凭空造出来。

**那条等式既太严又太松，两个方向都测了**。太严：它把"清掉冲突的方式是不再声明那些行"这个真故障，和"阻塞解除后声明得出更多"这个正常增长，禁在一起。太松：**换一行进换一行出，计数纹丝不动**。所以改成按URL做集合差——`test_the_refresh_drops_nothing_it_was_declaring`要求一行都不能消失、且当初带冲突的那些行仍在声明里；`test_anything_the_refresh_adds_is_a_period_it_unblocked`要求新增行必须服务于一个"之前被挡、之后不被挡"的期间，并要求仍有期间被挡着——否则这条链就把缺材料的问题一起糊掉了。**明确不断言**：新增行不需要获取。这一份恰好已保存，但一个刚解除阻塞的期间完全可能点名一份没存的代理，那是对的。

五次注错全部被抓，**其中一次的第一版是我自己写坏的**：丢掉带冲突的分片（catch）、新增行服务于仍被挡的期间（catch）、什么都没解除却多了一行（catch）、以及那个"计数看不见的换行"。换行第一版写成"丢一行加一行"，而真实链本来就是115→116，于是等式**本来就会**失败、证明不了盲点；第二版丢两行加一行让计数正好落回115（115→115，`a_count_equality_would_have_seen_it: false`），**等式会通过、集合差抓住并点名被丢掉的那份Company Facts**。一个够不到目标的注错不是证据，这条本文件已记过一次，这次是它的另一个形状：**注错必须落在被测判据会分歧的那个点上，而不是落在它恰好也会报错的地方**。

**C02的来源缺口是本帧第一个「在准备输入时就缺」的形状**：Marriott FY2023的C02以`HISTORICAL_TEXT_RUN_INPUT_BLOCKED:SOURCE_UNAVAILABLE`点名那份2024年代理的URL并**失败**，而C04同期的来源缺口产出的是FROZEN+WITHHELD的Run。区别不是约定被破坏，而是C02的第二个来源是**建Run所需的输入**：输入建不出来就没有Run，硬造一个等于声明一份不存在的输入。但读帧的人要知道这不是"未解释的尝试失败"而是来源缺口，已按名记录。

**PR #52正文重写**：它停在`638efe6`那一版，说"16/39指标、只跑通一条原生Run、需要142次GET"，全部已被实测推翻。重写时又自查出三条**正文自己说错**的：`SOP.md`和`interact.md`其实都改过（前者加了"已成功结果的业务核查与错误修复闭环"七步，后者写了覆盖汇总CLI的用户可观察输出），而正文的清单写着"未修改"；注册补丁也不是"两文件三处"而是**六文件十四处**（`run_store`8处、`records`2处，其余各1处，`git apply --check`通过）。**一份不再描述当前状态的PR正文，比没有正文更容易让人得出错误结论**。材料见`docs/evidence/issue47_history/governance-source-declaration/measured.json`。

Issue #47 27个没人读过的数值分诊，**C03那四个此前被记成来源缺口是错的**：155个带数值的位置里128个有接受登记，剩下27个逐条问它卡在什么上。B06(4)/E01(6)/D02(7)/B03(1)确有已登记理由（反解组成是拟合不是核对；唯一带关键词分支的路线在任何附件里都命中"transaction"；读过的四份里两份有错所以不能据以接受没读的七份；Salesforce主文件不标那个概念）。**C03那四条写的是`NO_SAVED_PROXY_REPORTS_THE_TARGET_PERIOD`，读起来就是"材料不在"——实测全是在的**：Enphase/Lumen/Pfizer/Salesforce四份钉住的代理**全部`VERIFIED_SAVED_SOURCE`、全部可读、分别带76/89/138/82个ecd事实**，而被接受的Ford作为对照坐在同一张表里、同样的形状、103个事实。**这是本Issue里我自己读取器的第七个缺陷**，形状也和已记过的两个上下文解析缺陷相同：一次查空的查找，和一份什么都没写的文档长得一模一样。**该说的是它们卡在我的读取器上，不是卡在材料上**；错误的不接受理由仍然不是接受，四条保持未接受，欠的是找出为什么Ford的期间能匹配而这四家不能。C04那五条同样重新标为**未分诊**而不是来源缺口——路线根本不读上一年的正文HTML（它读每个accession的索引与`AuditorName`概念），我的读取器挑了更难的路，所以那个`PREVIOUS_YEARS_FILING_NOT_READABLE`很可能也是在说我的路而不是材料，本轮未测。材料见`docs/evidence/issue47_history/content-acceptance/unread-value-triage.json`。

Issue #47 C03那四条的根因是文件名通配，**而修它的过程里我又在同一个形状上栽了三次，逐条记下来比结果重要**。根因：读取器用`*def14a*.htm`在各registrant的accession目录里找代理，**十家里只有Ford把主文件命名成`e26003_f-def14a.htm`**，其余用`<ticker>-<date>.htm`（`enph-20260401.htm`/`lumn-20260401.htm`/`pfe-20260312.htm`/`crm-20260416.htm`），所以那个通配**只可能找到恰好为它命名的那几家**，找不到就报`NO_SAVED_PROXY_REPORTS_THE_TARGET_PERIOD`——读起来是"材料不在"。**三次同一形状**：(1) 第一版修复改去读各accession目录的`index.json`拿form，**那些目录里只有正文和headers、根本没有index.json**，于是对十家都返回零份代理，报出的正是它要纠正的那个"不在"；(2) 诊断该版时我读的是**打补丁之前**启动的那次运行的输出（两个后台任务先后启动），据此断定"新查找也失效"；(3) 产出重算后我又读了一份由旧代码在几分钟前经**副作用import**写出的同名文件，看到`pub=None`/四条read=None，而产出自己的日志写着八条全MATCH。**每一次的表现形式都是"空集"或"旧内容"冒充事实**。

**修对之后实测**：代理改为**按form从已保存submissions块识别**，八家C03全部`MATCH`（Enphase 9601958、Lumen 24936641、Pfizer 27585301、Salesforce 49379252，与发布值逐位相同；Ford/Macy's/Marriott/Southwest原已接受），**0 DIFFERS**。Paramount读不出且**这是对的**——它的C03走年报自己的薪酬表那一级、不是代理，早已由另一次阅读接受。接受登记128→132，C03由5升到**9，九个位置全部被独立读过**。**修的是产出证据的那个读取器本身**而不是另写一份，所以登记由原生成器重算、生成器一行未改。**新增的三态区分正是这次教训的产物**：`THE_REGISTRANT_DECLARES_NO_DEF14A`／`NO_SAVED_PROXY_DOCUMENT_FOR_ANY_DECLARED_DEF14A`／`NO_SAVED_PROXY_REPORTS_THE_TARGET_PERIOD`——塌成一句话正是当初那四条被记错的方式。**顺带量出一个此前没人写下的数**：每家registrant声明**16到33份**DEF 14A而盘上**恰好1份**，所以"更早期间点名一份读不到的代理"是**来源缺口**（与C02那条同源），而这一轮必须被逼着才把它与读取器缺陷分开。C04的五条仍记为**未分诊**而不是来源缺口——路线读每个accession的索引与`AuditorName`概念，我的读取器读上一年的正文HTML，按C03这次的教训，那个`PREVIOUS_YEARS_FILING_NOT_READABLE`很可能也是在说我的路。

**本轮的一条通用纪律**：**报告"不存在"之前，先让那个查找器当场找到一个已知存在的东西**。Ford在这四次里一直是可用的对照，只要每次都先问它一遍，三次误诊一次都不会发生。这条按它**不需要手工清单**的那一半写进了`build_acceptance_register.py`：任一份阅读产出零条接受即`A_READING_CONTRIBUTED_NO_ACCEPTANCES`并**在写出之前**退出，所以一份死掉的阅读不能悄悄把已提交的登记改短。注错实测（把治理阅读的MATCH判据整个站掉）返回1并点名该份阅读、未写出任何登记；**第一次注错本身是坏的**——副本放在`/tmp`使`REPO`解析到别处，失败在读不到`cross-source-read.json`，与被测问题无关，改放回`tools/`才成立。**它抓不到的也写在代码注释里**：一份仍有产出、只是丢了一部分集合的阅读（治理阅读在两次失效中始终保住它那条C04）；抓那个要对已提交登记做ratchet，而ratchet需要为合法下降开口子，故不建。

Issue #47 C04那五条也查清了，**把它标成"未分诊"而不是来源缺口是对的，而查清的过程又连出我自己三个缺陷、外加一个在发出去之前被拦住的**。(1) **读取器只认上一年的主HTML**，而路线的`_auditor_filing`读的是那个accession里**任何带dei事实的文档**；实测Enphase上一期accession只有`enph-20241231_htm.xml`和`index.json`、**没有主HTML**，所以`PREVIOUS_YEARS_FILING_NOT_READABLE`说的是我的路不是材料。(2) 上一期accession是从**我自己截断到140字符的错误串**里正则出来的——URL正好断在accession中间，正则永远匹配不上；**信息在我解析它之前就已经被我自己销毁了**。(3) 上一期期末用"把年份字符串换掉"算，Macy's的2026-01-31变成2025-01-31，而它52/53周的上一期实际结束在**2025-02-01**，那个日期它的日历里从来没有过。**(2)和(3)的修法是同一句话：别重算，问那份selection**——它本来就带着`prior_report_end`与`prior_filing`（含accession与主文档名），两个缺陷都来自把已经递到手上的东西又推导一遍。

**第四个在发出去之前被拦住**：改完之后`text_fact`只匹配`<ix:nonNumeric>`（inline XBRL），而实例文档写的是`<dei:AuditorName contextRef="c-1">`，于是它会返回空集并记成`NO_SAVED_DOCUMENT_OF_THE_PRIOR_ACCESSION_NAMES_AN_AUDITOR`——**本轮第六次假性缺失**。这次没有发出去，只因为我先拿一份**已知含有该事实**的文档去问那个查找器（`instance -> ['DELOITTE & TOUCHE LLP']`）；这是上一条刚写进本文件的纪律第一次真的挡住东西。顺带两侧都做了HTML实体反转义，已接受的Marriott记录里那串由`DELOITTE &amp; TOUCHE LLP`变为`DELOITTE & TOUCHE LLP`，比较仍一致。

**Macy's暴露的是真实观察不是缺陷**：两份申报把同一家事务所写成`KPMG LLP`与`KPMG, LLP`。按原文逐字比会报出一个"更换审计师"，而读过这两份申报的人没有一个会同意，所以**标点与大小写的差异按同一家事务所处理**——这是一个判断，且与路线的规则相同，故**两串原文都记在判断旁边**而不是只留结论。要点名的残余局限有两处：该归一化与路线同规则，所以路线若在这一层过松我会一起错（实测它不至于——`kpmg`与`kpmgadvisory`在同一判据下并不相等）；以及我的`text_fact`取文档里**全部**`dei:AuditorName`事实而不按上下文/主体/维度过滤，比路线更松，本语料每份文档恰好只有一个名字所以无差异。

C04由**1条MATCH升到6条**（Marriott/Ford/Pfizer/Lumen/Enphase/Macy's）；Southwest与Salesforce的C04本就没有已发布值（WITHHELD），`NO_PUBLISHED_VALUE`是对的。

Issue #47 29指标×11期间批次（同一只读运行时树、closure `sha256:d4c4a7ab…`、零调用）：**385个位置全部尝试、383个冻结Run与公共行、2个失败**。两个失败是同一条已命名的来源缺口——Marriott 2023/2024的C02，那两年的代理未保存。三层照旧分开数：**164带数值**（上一批155）、**169结构性不适用**、**50跑了没有值**；`content_acceptance` **140**（上一批125）、`verified_outcome` 329。**+9个数值正是C02接线带来的**：九家当期代理已保存的公司各出一个值。帧的另外三层不变：1950个目标位置里1521仍在等获取。

那50仍是三件事：**答案7**（分母非正3、分子非正2、接续注册人146天首期2）／**已决政策6**／**缺口37**（事件8-K正文24、C04上一期accession索引5、B06来源关系3+1、C03代理未保存2、Company Facts 1、全分支拒绝1）。**四个D02坐标被已登记内容缺陷撤回**（Pfizer条目上限、Lumen与Paramount的关键词代理、Enphase页脚）。

**C02的失败形状已写进证据而不是留在散文里**：帧把失败读成`ROUTE_IMPLEMENTED_ATTEMPT_FAILED`——对"尝试停住了"这是对的名字，但它不说停住的是缺材料还是路线有缺陷。C02是本帧第一个**第二来源用于准备Run输入**的指标：C04缺的索引解析指标时才要，所以Run建得出来、结果WITHHELD；C02缺的代理意味着**根本没有输入**，硬造一个等于声明一份不存在的输入。**欠的也写明**：帧今天分不开这两者，而这条记录不从错误文本里正则——那串是批次恰好写下的Python repr，解析它是把猜测装扮成测量。

**按路线族冷读又抓到我自己检查器的一个缺陷，同一形状**：家族映射是`FAMILY.get(metric_id, "unclassified")`，C02接线后**悄悄落进那个兜底桶**，而`families_covered`把`unclassified`当成一个真实家族列出来，于是检查通过、证据里出现一个并不存在的家族。**一个读起来像正经答案的兜底，和"空集被读成不存在"是同一个形状**：明天接线的指标同样会消失在里面，它的路线没被冷读过而没有任何东西会说。改为无法分类即按名退出（`COLD_READ_CANNOT_CLASSIFY_A_METRIC`），C02归入`text`。实测该守卫对`D01`——恰好是下一个要接的指标——正确退出，所以它会**逼我分类而不是吸收它**。修后16次读、九个真实家族全覆盖、全部通过。

**两棵树**：逐叶比对两份minted manifest，**12个叶子不同，全部落在6个注册补丁文件的`execution_authority`上；`new_rule_files`的哈希一个都不差**——这批跑的是仓库自己的路线字节。

Issue #47 内容验收层两处缺陷（外部审阅对`ab91b1dd`点名，**两条都先复现再修，而第二条是我上一轮自己加的那道检查**）。**(1) 接受绑定的不是被核查的那件事**：`_acceptance_covers`只比公司/指标/期末/值。实测（对照先通过，否则整张表无意义）：值由0改为1**正确拒绝**，而**测量起点移一年、单位换掉、scope_key换掉——三个都仍然被接受**；`period_end`被抓只因为它同时充当坐标。**这在看起来最无害的地方最危险**：C04的值是一个标志，"读了2025两份指定申报、审计师未更换"这条接受，会被任何一个在别的窗口/范围/单位下同样算出0的结果继承。修法是每条接受携带`result_identity`（`period_start`/`period_end`/`unit`/`scope_key`）且逐字段必须相等，**不带该绑定的条目什么都不授予**（失败闭合，旧格式不能蹭过去）。**明确不绑的**：Requirement closure、`spec_closure_hash`、`result_id`——它们随无关代码移动，绑上去等于让一次纯重构要求重读每一份申报；**绑的是业务事实不是字节**。**pin怎么来、以及它不主张什么**：阅读确立公司/指标/测量窗口/值，单位与scope_key从"该阅读所针对的那个结果"pin下来，**是防止事后替换的绑定，不是第二次阅读**；阅读自己记了窗口的，生成器与结果交叉核对，不一致即按名退出而不是pin过去。**对当前帧的影响：没有**——`content_acceptance`仍是140、`verified_outcome`仍是329，因为现有每一条本就pin在它被读的那个结果上；**洞是真的，但没有东西站在洞里**。

**(2) 一份得出"不一致"的阅读没法把这个结论说出来**——而这正是我上一轮写的那道"任一份阅读产出零条接受即退出"。它与它要抓的情况**无法区分**：一份跑完并判定每个值都不一致的阅读贡献零条，于是守卫**在写出之前**退出，**旧登记原样留在盘上，继续授予新阅读刚刚推翻的那些接受**。实测：把治理阅读全部verdict改成DIFFERS→生成器返回1、登记未重写、**6条C04接受一条不少地仍被授予**；修后返回0、登记由140写成126、**C04授予0条**。修法是**计数只报告不设门**：接受数的合法下降是这个登记必须承载的结果，不用单调规则替换它。计数对部分阅读为`null`并写明理由——只有带`per_position`的阅读暴露得出来，其余是扁平标签映射或单坐标，**按这些形状写出来的规则看不见下一种形状**，第一版就是这样在lodging阅读上失败的。
...
Issue #47 D01 内容核查（两个方向，按已批定义读原文）：以前每次只问"取到的这些是不是标题"，那只是一个方向——**一条对取到的每一块都判断正确的选择器，同时可以丢掉一份披露，而只看取到的集合永远看不见它**（D02 那轮的缺陷正是这个形状）。这次把 ITEM_1A 范围内**每个块**连同选择器所依据的属性（`leading_emphasis`、`linked`、item/part 标签、注册人名）与**从申报字节读出的外层 `<span>` 样式**一起列出——样式从字节读而不是从解析读，因为被怀疑的正是解析。
...
Issue #47 **上一条的结论撤回：危险的不是强调规则，是审计报告范围规则**。我写的「只改 D01，不改共享解析器」对"只放宽强调"这件事成立，但**少测了一步**。问那五块到底为什么丢：Southwest 的块 1421 与真正的两个标题（2250/2275）文本完全相同，区别是 **1421 `linked=True`**（Item 8 目录里的蓝色带下划线超链接），真标题 `linked=False`、11pt、`font-weight:700`。而 `audit_report_spans` 只看 `leading_emphasis`，**同一个模块往上三百行的 `_note_heading` 早就在要求 `not linked`**，理由一模一样：一个标题的超链接副本是目录项不是标题。所以这个判据不是我发明的，是邻居规则已经在用的。

**三侧实测**（冻结／承认下划线／承认下划线＋不许链接块开范围；三个指标×十家公司，同一探针进程、磁盘不动）：**二十七个公司—指标对里二十四个逐项等于冻结答案**，Southwest 的 D02 走 25→20→**25**（完全恢复）。移动的只有三处、**三处都是更正**：Marriott 的 D01 由 34→**38**（那四条），Lumen 的 D02 由 41→**42**（块3400 `Blum`，两段集体诉讼之间的案件名小标题）。所以正确的修法是**两半一起**：承认下划线，且要求开范围的块不是链接。

**代价也量了**：链接守卫在 `historical_text_results.audit_report_spans`，本世代自己的规则文件，**一行**。强调那一半在 `text_coverage._Blocks.handle_starttag`，字节被 `issue_28_v11` 点名，要后继。**后继可以很小**，因为章节边界不依赖强调（`_heading` 只读 `linked` 与文本），实测两侧块序列完全相同：后继跑一遍冻结 `build_text_document` 拿全部身份校验与章节，再用一个只重写 `handle_starttag` 的 `_Blocks` 子类重算，**要求块文本与偏移逐项相同**，然后只替换两个强调字段——这条要求就是担保，它让后继在机械意义上"只动了一个字段"，而不是第二个解析器。行数：文档后继约 60、`prepare_text_sources` 后继 37、D01 的 v1 候选链 104（四个函数互相按模块全局查找），合计约 **200**，对照文本协议后继的 155。**本轮不付**：批次正跑在当前 closure 上并且是它的证据，规则文件要等批次按自己的 closure 报告完。**仍未解决的**：下划线非加粗在本仓库没有的申报里会不会标记别的东西——九份、各一个期间、只有一家用这种样式，所以这条规则靠的是**文档内对照**而不是一个总体。

Issue #47 **量那个下划线修法的仪器自己先是坏的，这条比结论重要**。前两次测量是这样承认下划线的：把冻结的强调标志在 `text-decoration:underline` 时置真、在 `text-decoration:none` 时置假。**一个标志分不清它清掉的是哪一份贡献**——加粗祖先下面的一个 `none` 会把加粗一起清掉。事后实测：九份文档因此丢失加粗前缀，最严重的 `crm-20260416.htm` **清掉 1599 个 leading emphasis、缩短 6 个**。它当时之所以报"其余一律未动"，**是运气不是设计**：被弄坏的块恰好都不在三个指标扫描的范围里。**一个做的事和它声称的不一样的测量，即使答案碰巧对，也不是证据**，所以由它得出的每条结论都用下面这个解析器重算了一遍。**去掉 cancel 也不是答案**：忽略 `none` 的单调规则与 cancel 版在九份文档上不同（Salesforce 代理 1605 块、Pfizer 年报 5 块），所以 `none` 确实带信息。正解是**两个栈**，不是一个标志、也不是不 cancel。

**解析器形状**：`text_coverage._Blocks` 的子类，在冻结的强调标志旁边另记一个 bold 栈和一个 underline 栈；bold 栈复述冻结的 font-weight 规则，使继承从加粗分量而不是从合并标志读出——这正是让 `none` 只取消下划线的那一步。**复述的那份规则被机械地锁在原版上**：`admit_underline=False` 时子类必须产出与冻结解析器**逐项相同**的块，全部 >400KB 的已保存申报实测**无一例外相同**。打开下划线后**零清除、零缩短**——单调性是两个栈的**推论**，不是假设。

**三侧重算，结论复现**：Marriott D01 34→**38**，Lumen D02 41→**42**（块3400），Southwest D02 25→20→**25**（链接守卫恢复）；二十七个可读公司—指标对里二十四个三侧完全相同。同样三处移动、同样那五块，只是这次的仪器在机械意义上只往一个方向动。**尚未安装**：批次还跑在当前 closure 上，规则文件不能动。安装意味着：该解析器成为本世代规则文件；D01 需要后继的来源准备与 v1 候选链才够得到它（那四个函数在冻结模块里互相按模块全局查找）；`audit_report_spans` 加 `not linked`，那是本世代已拥有的规则文件里的一行。材料见`docs/evidence/issue47_history/d01-risk-headings/underline-parser.json`。

Issue #47 D01 第三个方向（取到的这些是不是标题）也读了，**又查出一处、只有一处**：前两个方向只能问"拒绝对不对"和"有没有漏下视觉上被标记的块"，**都看不见一条被取走而本不是标题的**。逐条读九份申报的标题列表：七家干净——分类标题加逐条风险标题，正是已批定义要的"标题级摘要或主题清单"，没有页眉、没有引言句、没有审计报告正文（Ford/Macy's/Enphase/Lumen/Salesforce/Southwest/Paramount 的分类标题都在，因为它们加粗；Marriott 的那四条用下划线，就是上面那件事）。**Paramount 有一条被截断**：交付的是 `Failures to comply with or changes in U`，而申报写的是整句 `…in U.S. or foreign laws or regulations…`——这家把 `U.S.` 里的两个句点标成 `font-weight:400`、夹在加粗字母之间，而 `leading_emphasis` 是**首个连续加粗段**，到第一个非加粗部分就停。

**常见情形不是缺陷，这一点量了**：Ford 三十条里二十六条、Marriott 三十四条里三十二条同样"前缀短于整块"，但**没有一条在空隙之后恢复加粗**——那正是冻结文档串里写明的刻意设计（加粗引导句与其后段落分开）。**只有 Paramount 那条恢复了，空隙是一个句点**。**没修**：九份申报一个实例。"只由标点构成的非加粗空隙不终止该段"这条规则背后只有一个例子，而从一个例子发明规则正是本 Issue 反复记录要避免的。已登记为 `D01_PARAMOUNT_2025_HEADING_TRUNCATED_AT_AN_UNBOLDED_PERIOD`。

所以 D01 的内容读法现在是三个方向、九份申报：**七家三向干净**（批次跑完后按各自发布值进接受登记），Marriott 三年因下划线分类标题被撤回，Paramount 因这条截断被撤回。读法与生成器写在`refresh_d01_read.py`（批次后运行，按批次自己的值重建），它还要求"我判读过的标题列表"同时等于选择器今天返回的列表**和发布值自己的行**——判读因此被绑到交付物上，而不是绑到一次运行的记忆上。

Issue #47 30指标×11期间批次（同一只读运行时树、closure `sha256:5c89905a…`、零调用）：**396个位置全部尝试**（30条已接线指标＋各公司结构不适用指标，每期间36个）、**394个冻结Run与公共行、2个失败**。两个失败是同一条已命名来源缺口——Marriott 2023/2024的C02，那两年的代理未保存；这仍是本帧唯一"缺料即建不出Run"的形状（C02的第二来源是**建Run所需的输入**，C04缺的索引是解指标时才要，所以后者产出FROZEN+WITHHELD）。三层照旧分开数：**175带数值**（173 EXACT＋2 APPROX，上一批164）、**169结构性不适用**、**50跑了没有值**——拆成**答案7**（分母非正3、分子非正2、接续注册人146天首期2）／**已决政策6**（Paramount报表类未清除）／**缺口37**（事件8-K正文24、C04上一期accession索引5、B06来源关系3+1、C03代理未保存2、Company Facts 1、全分支拒绝1）。

**+11个数值正是D01接线带来的**：十家里九家当期各一个，Marriott三年各一个（2025由19→21、2024由11→12、2023由9→10——多出来的两个是D01与C02）。**无路线由44降到33**，正好是剩下三条语义路线（B13/D03/D04）×11个期间。帧的另外三层不变：1950个目标位置里1521仍在等获取。**按路线族冷读16次、九族全覆盖、全部通过**；**两棵树逐叶比对12个叶子不同，全部落在6个注册补丁文件的`execution_authority`上，`new_rule_files`哈希一个不差**。

**第三层：`content_acceptance` 147**（上一批140）、`verified_outcome` 336。175个数值里**147个被独立读过、8个被已登记内容缺陷撤回、20个没人读过**。撤回的八个里**四个是本轮新登记的D01/D02内容缺陷**（三年Marriott下划线分类标题、Paramount标题截断），另加此前的Pfizer条目上限、Lumen与Paramount关键词代理、Enphase页脚。

**D01阅读的判据第一版太严，而这也是量出来的**：方向二"有没有漏下视觉上被标记的块"我写成**要求该集合为空**，于是Ford（14块斜体running head）、Pfizer与Salesforce（各1句斜体引言）三家被判`READING_STALE_OR_INCOMPLETE`——**而这三处恰恰是我自己前面量出来"该留在外面"的那17块**。要求为空等于把"本语料的标题标记是下划线、斜体标别的东西"这条实测结论从判据里抹掉。改为**每一块都要带一条记录下来的判断**：没有判断的块使阅读失效，记录了却在申报里找不到的判断同样使它失效（一份点名着申报没有的块的阅读，描述的是别的东西）。修后D01七家`MATCH`、四个坐标`READ_AND_A_DEFECT_WAS_FOUND`、JPMorgan `NO_PUBLISHED_VALUE`。
...
Issue #47 外部审阅（GPT-6 Pro，对`7a583496`）的三项有限修复。**(1) 接受的身份改由阅读记录恢复**。旧生成器按目录顺序`setdefault`从当期结果倒填身份，所以阅读不变、只把结果换成新单位或新范围再重新生成，登记就授予了新的那个。现在每条接受的身份写在阅读自己的`checked_identity`里：申报（`filings`）、期间、主体（`entities`与scope_key）、单位、指标含义（`spec_closure_hash`）与`value_kind`；`tools/build_acceptance_register.py`只从阅读复制、**完全不读Run**——一条用例把每个模块对三个收据读取函数的绑定都换成抛异常仍能生成，另一条用旧的环境变量入口把"只换单位/范围/申报/含义"的批次放到生成器看得见的位置，重新生成的登记必须与已提交的逐字节相同。旧阅读没有记录这些，由`tools/bind_acceptance_readings.py`一次性绑定：复用`collect_run_receipts`与`select_receipt`按指名closure选结果，要求所选结果的值就是阅读比较过的值，再与阅读自己记下的东西交叉核对——打开的申报必须在结果的申报集合内，事件窗口要求阅读逐一列出的8-K集合与结果的集合**完全相等**（七个窗口实测全等），记了窗口的要求窗口一致；通过才写入，标`BOUND_AFTER_THE_READING`并记下来源closure、run、result与跑过的检查。已有身份永不覆盖，批次不一致只报告。实测158个比较过的位置全部通过；同一绑定在29指标与30指标两批下对147个共有位置逐字段相同，所以绑定不依赖阅读当时对着哪一批。重新生成的登记147条与旧的逐条相同，身份在旧四字段上一致并新增四字段。为此收据读取器给每个结果补上申报集合（沿trace→观察→来源引用，跳过submissions伪accession）、实体与`spec_closure_hash`。**纠正上一轮一句话**："`spec_closure_hash`随无关代码移动"不对——它由编译后Spec语义、依赖closure与`SEMANTIC_VERSIONS`的声明版本组成（最后一次变动在8月17日），正是"指标含义"该绑的那一个；Requirement closure与result_id仍不绑。**(2) 阅读哈希表必须覆盖每条接受引用的阅读**。旧检查遍历表本身，表缺失、为空或漏列被引用的阅读时就跳过了。现在每个被引用的阅读都有状态——`GRANTING`、`READING_NOT_HASHED_BY_THE_REGISTER`、`READING_UNREADABLE`、`READING_CHANGED_SINCE_THE_REGISTER`——只有第一种授予，按阅读失效而不是让整张覆盖表抛异常；位置层把"没人读过""读过但阅读不在可授予状态"（`ACCEPTANCE_NOT_GRANTING:<状态>`）和"读过但不是这个结果"（`ACCEPTANCE_DOES_NOT_MATCH_THIS_RESULT:<字段>`，业务不一致）分开，帧新增`acceptance_reading_states`；零接受的阅读是合法表项。九次注错全部被抓，含按旧入口glob倒填的忠实版本。**(3) Marriott三个D01修复结果**：读其Run得result_id `178bc962…`/`2058162f…`/`99c76e50…`、closure `d4cf2c3f…`；执行字节——三个Run记录的baseline就是运行树当前的，26个规则文件在两棵树里与记录逐一相同，两树只差6个注册补丁条目，两树mint --check均通过。**独立内容验收时发现旧D01阅读的"判断清单"就是选择器自己的输出**，分不出修复前后。新写`tools/read_d01_headings.py`，不导入路线任何文本模块：自己的块解析，从样式与标签读粗体/下划线/斜体，按Item 1A自身标题定位（取跨度最大者，因为Salesforce目录里的"Item 1A."是不加链接的粗体；跳过Paramount拆成两块的条目标题），注册人名从申报自己的`dei:EntityRegistrantName`读，相同标题按路线已发布的形态并到首次出现处；逐行按序与已发布值比较，其余视觉标记块逐块要有判断，封面财年结束日必须是所读期间，并机械标出"强调在≤2字符未标记间隙后恢复"的行；每行是否为标题的业务判断逐家记在`d01-risk-headings/heading-judgements-*.json`。对照：对修复前的结果三年各正好报出四个下划线分类，对Paramount标出唯一那条截断行——即已登记的两条缺陷。修复后37/39/38条逐行一致，其余七家同样一致，所以D01接受改由这两份字节级阅读提供，旧阅读恢复原始字节只作记录；读取器输出按行拼接后的摘要逐字节等于每个已接受位置的已发布摘要（不依赖批次目录），六次读取器注错全部被抓。三条缺陷只对各自修复结果与closure释放，`repair_state`改为`RULE_FIXED_RESULT_RECOMPUTED_AND_READ`——上一条写的释放"要等这个closure上的批次"撤回：释放点名的是结果与它的closure，不需要整批重跑；三个Run的小帧三层交付全部成立，30指标批次里同坐标的修复前结果仍被撤回、新接受对它们列为"未适用：数值不同"。旧批次证据未改动、未改签，也不把这三个结果写成新版本的全量验收。登记147→150。同一原则顺带查出两条Pfizer缺陷也写着"未重算"而实际已重算（结果`367fe2ef…`在28/29/30指标三批中相同，高管履历已不在、Item 3句子已在），已更正；该坐标仍被关键词误取那条撤回，交付不变。材料见`docs/evidence/issue47_history/d01-risk-headings/repaired-results.json`与`acceptance-binding/reading-identity-binding.json`。

Issue #47 三处单例文本缺陷修复、B13定义范围外分支与定向重跑（`d5de1684`、`4f1be8d4`与本提交）：**"目前只有一个真实实例"限制的是规则能推广多远，不是永不修复的理由**——三处都按"真实失败＋现有材料＋针对性反例"做最小修复，并在每个已存申报上量它移动了什么。D01：`UnderlineBlocks`在强调恢复处桥接一到两个标点的未加粗空隙，十一个D01位置里只移动Paramount那一行；D02：页脚带页码时按"去掉页码后的文本在至少三页重复且邻块也重复"判为页面装饰，下划线的附注内短标签按申报字节里它自己的span算标题——十一份D02只移动Enphase块755（出）与Lumen块3400 "Blum"（入），D03候选零移动。关键词代理不修，而是写成决策包（`d02-content-read/keyword-proxy-decision.json`：三条确定性规则各丢一条已批来源点名的披露）。B13：已批定义标题只点名Ford/Enphase，其余公司给`N_A_STRUCTURAL/TRAIT_NOT_APPLICABLE`并与普通路线逐字段相同（含result_id）；范围取自定义标题而不是#28的调用政策，那份政策同时承载#28的预算与委托。已提交的注册补丁此前缺B10/B11那几个hunk，现等于运行树的实际diff（18个hunk）。**定向重跑31个Run**（闭包`c7a49b98`，D01/D02各11、B13九个）：全部冻结、公共行、独立进程冷读一致、零调用；两棵树minted manifest只差6个注册补丁文件的12个叶子。D02只移动了探针预测的两块，其余九份与30指标批次逐字节相同。这只是这三个指标的定向重跑，**不是该闭包的全量帧**，30指标批次不改签。

内容接受152条（+2）：D01阅读器对"强调在两字符内空隙后恢复"的行**只标记、同时给出前缀与跨空隙两种读法，由记录的逐行判断选择**——若阅读器也内置路线的桥接规则，一条错误规则会在两边同时错而通过；两次注错（判断失效、缺判断默认跨空隙）都被抓。Enphase D02的17条正是早先双向阅读判为披露的17块、同序、只少页脚，按阅读时记录身份接受。缺陷释放按版本逐个点名：Paramount截断、Enphase页脚对新结果；Marriott三年与Pfizer超链接对同一result_id在新版本下各自点名。**两处登记表自己的错**：(1) Pfizer"条目上限"缺陷只对`1b6d9c87`下92条结果释放，于是覆盖表把这个在96条结果上早已修复的缺陷报成撤回理由（它排在前面），现对96条结果释放，报告理由变为真正起作用的关键词代理；(2) Blum此前在本文件写"保持登记"而登记表里从来没有——与Pfizer超链接Item 3同一种失误，现登记为`D02_LUMEN_2025_UNDERLINED_CASE_LABEL_DROPPED`并对新结果释放，Lumen仍被关键词代理撤回。本轮31个位置：D01 11/11接受、D02 8/11接受（3个被关键词代理撤回）、B13九个结构性答案。材料见`docs/evidence/issue47_history/targeted-round-c7a49b98/`。**操作教训重犯一次**：`pkill -f <pattern>`又杀掉了自己的shell，按pid清理才对。

Issue #47 语义路线（B13适用分支/D03/D04）的请求需求与复用核查：三条路线共用`r6_semantic_source.prepare_d04_semantic_source`，它经两个函数读"最新年报"；把这两个函数指向钉期输入、原样运行#28的请求构造器，零调用即得请求普查。11个已存原件期间共**193个请求**（D04 74、D03 102、B13适用两家17）；**最新年份20个可比位置与#28自己的grouped census逐个相等**，其中五家D04还等于#28的实际调用数——所以最新年份上历史语义路线不增加#28没在做的审阅，#47自己的请求只在更早年份，今天只量得到Marriott 2023/2024的18个，其余39个期间要先有原件，不估。**复用核查的结论是没有可复用的**：同一申报下钉期请求与#28请求的源单元完全相同，但请求身份不同（普通输入带完整submissions行、钉期输入带五字段投影，另带period_selection）；#28已登记的响应在它自己主机checkout的`.git`私有目录里、本checkout只有测试写的两条RECORDED_TEST_ONLY；登记按(semantic_source_id, #28 requirement closure)键控且source id含模块字节；#28自己也记着这些D04结果语义未验收、独立审阅有阻断。拿它们给#47位置用是跨Issue的信用决定，不是工程步骤。仍要做的离线工程：钉期输入准备（已量得可行）、#47自己的模型调用策略与账本（`register_assessment_input`只收#28的`CallLedger`工厂，请求计划只收#28的传输政策）、历史接线与记录响应测试——后两者依赖前者。材料见`docs/evidence/issue47_history/semantic-route-need/`。

Issue #47 E01的8.01关键词分支从未读过8.01正文（`tools/read_e01_eight_o_ones.py`，非规则文件）：已批定义写"8.01 需正文关键词确认"，路线却把关键词拿去和claim的`brief`比，而只要hdr.sgml带item代码（本帧每份都带），`brief`就是固定文字`8-K item 8.01 parsed from hdr.sgml`——**8.01分支一份都没放行过**，已发布E01其实只是1.01与2.01的条数。路由器被`issue_28`各世代冻结、普通路线共用，两条路线同样如此。七个有值窗口的**23份8.01全部整段读过**，逐份判断记在`e01-keyword-branch/eight-o-one-judgements.json`：只有1份报告了交易（Pfizer 2025-11-13完成对Metsera的收购）；8.01正文带关键词的4份里另外3份是融资（Marriott两份募资用途句"may include … acquisitions"、Macy's要约收购的"combined aggregate purchase price"）；另有5份关键词只在别的item里（1.01契约条款、7.01）。**接受规则只在三种读正文口径（正文字面命中、全文字面命中、正文命中且确为交易）都给出已发布值时成立**：Enphase/Ford/Southwest接受（登记152→154），Lumen/Macy's/Marriott取决于尚未做的口径决定，Pfizer在任何读正文的口径下都是1、路线发布0，登记为坐标级缺陷`E01_PFIZER_2025_EIGHT_O_ONE_BRANCH_NEVER_READS_THE_ITEM`。E01从此不再从表头计数那份阅读接受——它不打开8.01。**缺的含义判断**写在`e01-keyword-branch/decision.json`：只在融资语境里出现的关键词算不算一次并购公告；已批的"任意位置子串命中"说算，指标名字说不算，本语料带关键词的4份里两者在3份上相反。推荐只认指向具名交易的用法（在#47自己的后继规则里实现、每处被拒绝的命中都记录），代价是改已批匹配方式、正例只有一个。**这次阅读自己也先错了一次**：第一版只认"Other Events"标题，Pfizer那份写的是"Results of Other Events"，被读成空、零关键词——和路线同形的漏法，只高了一层。顺带记下、不提议：定义让1.01不经确认直接计入，七个窗口13个1.01里11个是借款/债券协议、1个是Southwest与Elliott的合作协议、只有Lumen出售光纤业务给AT&T是并购。五次注错全部被抓（"表头计数重新接受E01"是以接受编号重复粗粒度抓到）。

Issue #47 **撤回本文件早先一句**："Salesforce的B03读出0.288、发布0.230……路线是对的，我的读法够不到"——**错，而且是把一个缺陷写成了路线正确**。当时记的是"主文件没标`DepreciationDepletionAndAmortization`、accession事实里才有"。实测主文件标了（f-792至f-794，物业设备附注里一句"Depreciation and amortization of fixed assets totaled $1.2 billion"）；早先那次阅读的代码从未提交，在会话临时目录里找到的工作副本用`int()`解析数值、失败就静默跳过——"1.2"（scale 9）过不了`int()`，于是看不见。现已由`tools/read_statement_facts.py`从提交的代码重新产出这份阅读：按小数解析、fixed-zero横线读作零、读不出的列出不丢，其余79个（读数，结论）对逐个不变；Salesforce B03现在读出的正是已发布值（读者看见了路线取的同一个事实），结论却是`DIRECT_CANDIDATES_DISAGREE`——两个都按链条走的读者彼此一致，说明不了链条第一个概念覆盖了什么。更要紧的是这个事实**只是固定资产折旧**：申报的现金流量表合计`DepreciationAndAmortization` 3,631,000,000，脚注写明含收购无形资产摊销、固定资产折旧及使用权资产的摊销与减值；利润表脚注给出收购无形资产摊销692+995=1,687百万。已批链条按顺序取第一个出现的直接概念、从不与后面的比较（`calculator._select_structured_fact`），于是B03拿固定资产折旧当了全部D&A——这正是常设规则"不得把小计当完整总额"禁止的。九份B03申报逐份普查（`b03-depreciation-scope/da_census.py`）：**只有这一份的直接候选互相矛盾**（Lumen标了两个直接概念但同为2,749百万；Ford取的是分部附注的合并"Depreciation and tooling amortization"合计15,974百万，含Ford Credit经营租赁车辆折旧）。正确值还不能直接换成3,631百万：它含使用权资产减值，而定义写"不加回 impairment"，当年减值金额又没有单独标注；已批的拆分组合也用不上（该申报没有无维度的`Depreciation`与`AmortizationOfIntangibleAssets`）。登记为坐标级缺陷`B03_SALESFORCE_2026_CHAIN_TAKES_FIXED_ASSET_DEPRECIATION_AS_TOTAL`；推荐的修法是"直接候选互相矛盾时不取任一、按名扣留"，只动这一份申报、不点名公司，但它改了已批选择规则，要走修订机制，写在`b03-depreciation-scope/finding.json`。又一次"没看见当成不存在"，这次还把缺陷翻成了对路线的背书。

Issue #47 授予接受的阅读全部改由已提交代码产出（`docs/evidence/issue47_history/reading-producers/`）：154条接受来自九份阅读，此前只有D01与E01的产出代码在仓库里，报表（75）、事件计数（35）、治理（14）、住宿表（6）、RPO与薪酬表（各1）只有结论JSON，代码在会话临时目录——容器回收即失，也无从审查。报表阅读的临时代码用`int()`解析、失败静默跳过，正是它让Salesforce那条"1.2"（scale 9）的折旧不可见。新工具`tools/read_statement_facts.py`、`read_event_counts.py`、`read_governance_facts.py`、`read_lodging_table.py`、`read_single_facts.py`：不导入路线计算模块，已发布值经收据从点名版本读取，身份在阅读时记录，已提交阅读逐位置可从已保存字节重算；重新产出后登记仍154条、身份字段除`established_by`外不变。**重写时读出两件事**：Salesforce B03修好解析后读数等于已发布值，结论却是`DIRECT_CANDIDATES_DISAGREE`（两个按链条走的读者一致，说明不了链条首个概念覆盖什么）；Southwest C03把fixed-zero横线如实读成0后一度被拒，表格本身显示Gary C. Kelly只在2021–22年有PEO薪酬，2025年的横线是占位——旧代码靠跳过碰巧读对，现按表格证据放到一边并记录，从未被支付者的横线照常计入。事件阅读的submissions索引原先是未排序glob挑中的哪份算哪份，现取账本最近一次成功的那份（与路线同一份）。D02那份是逐块人工判断记录，没有可复现代码，复核方式是逐块重读。

Issue #47 Paramount前身年份进入五年框架（§7.3，改`normal_history_catalog`/`normal_period_selection`/`historical_annual_input`/`normal_history_plan`等规则文件）：后继CIK 2041610只交过FY2025一份年报，FY2021–FY2024由前身813828提交；现在只在后继完整目录里没有更早年报处才读前身自己的目录，前期不跨注册人拼，前身年份的选择点名`period_registrant`并使用该注册人自己的`CONTINUOUS_PRIMARY`（不授权跨主体合并），注册表没点名的CIK不读。**FY2024跑通**：37个已接线位置全部冻结出公共行、冷读一致（3个数值C02/D01/D02、17个结构性不适用、17个带理由扣留，其中14个卡在前身10-K/A措辞不在已批Part III模式内——两个待定口径决定），每一行点名813828；12个对照结果编号与上一版本逐个相同、行字节不变。**批次在本次改动内抓出四处并已修**：前身期间重推要读后继目录而它不在已准入来源里（第一轮每个指标失败）；共享投影器把`company["primary_cik"]`写进每条已发布行，于是前身年份的已发布行把后继CIK和前身accession放在一行，扣留行却点名前身；零AI路线对已批修订拒绝让尝试失败、Company Facts却出扣留行——同一年同一拒绝两种读法，现统一为扣留；扣留的B03不带B01导致Run依赖集不完整。框架：未发现期间273→117（余下全是JPMorgan），缺原件1170→1287，可达位置429→468。见`docs/evidence/issue47_history/paramount-predecessor-years/`。

Issue #47 剩余工作按"卡在哪里"重排（`docs/evidence/issue47_history/remaining-work-2026-09-25/`，取代按未接线指标计数的`remaining-implementation.json`）：离线实现剩语义路线（B13在范围内/D03/D04，需#47自己的模型调用会话）、六个金融指标适用分支（移植很小，但唯一金融公司JPMorgan每期都卡元数据，排在分片获取后）、D02关键词代理（候选判据被Macy's 1065证伪）；owner决定六项（E01口径、B03 D&A不一致、Part III报表类、Part III措辞、#47 SEC许可、#47模型调用许可），各有证据与推荐。**获取上限由1232重测为1352**：前身年份让年报链+元数据142→150，前身FY2021–FY2023事件窗口多38份8-K（+76次），C02/C03接线后往年代理成为依赖（36份，全仓库已存代理都是2026年申报）。许可提案改由`tools/propose_historical_allowance.py`从声明框架实测产出（旧脚本在临时目录、手写类别清单缺`GOVERNANCE_DISCLOSURE_FILING`，按它批准会让门禁拒掉每一份代理），并过了真实验证器、加宽上限的反例被拒。

Issue #47 D04 历史路线已接线（`historical_semantic_source.py` 第30、`historical_semantic_results.py` 第31个规则文件，非规则的 `historical_model_session.py`），**B13 适用分支与 D03 如实不接**。复审说"需要模型调用不等于无需输入准备、历史接线、已有响应复用核查或记录响应测试"，这一轮做的就是这四件里剩下的两件。**先撤回我自己在本文件里写过的"三条语义路线全部卡在模型额度上"**：卡住 live 调用的不止额度——WB-3 调用控制器按 id 登记 Requirement 世代、不认识 `issue_47_v1`（用例让控制器自己拒绝来测，不写死），`tools/check_provider_egress.py` 固定传输调用方的确切集合，两者都被冻结世代按字节绑定，给它们加 `issue_47_v1` 是改安全边界、要单独审阅；所以 live 会话按名拒绝两次（先缺许可，给了许可再缺出口登记），**授予额度本身不会让调用变成可能**。钉期来源与冻结构建器**逐字节相同**（把冻结的两个"最新年报"读取指向同一份钉期输入做差分，含 `semantic_source_id`；Marriott 2023、带 Part III 10-K/A 的 Paramount 前身 2024、范围内的 Enphase B13 三例全等）。登记是 #47 自己的、按钉期来源作键，**从不消费 #28 的登记**；每个消费者在当前代码下从登记的输出重推每个请求的 Candidate/Evidence 与组装后的评估。RECORDED_TEST_ONLY 只在点名时被消费，批次默认 LIVE，所以测试登记永远进不了批次。**绑定集合是测出来的不是按 import 推的**：新进程跑一遍 D04 案例，列出加载的模块与打开的规则文件（从第一次 import 前开始记录），得到 14 个模块加 7 个数据文件本世代原先未绑定——全是 #28 的原生语义路线，包括决定请求分组边界的 DeepSeek 分词器——已写进 mint 的 `AUTHORITY_ADDITIONS`；代价是 #28 改其中任一文件都会移动本世代 closure。静态 import 闭包给出 12（漏数据文件和函数内导入）或 38（把批次/恢复/发布机器都算进来），两个都不是答案。**不接的两条各有具体理由**：B13 适用分支的请求合同还在 #28 修订（基础请求的分类缺陷已登记、角色/相关性变体未被接受），现在移植等于把调用花在其所有者都没接受的合同上；D03 在普通链路里根本没有原生 Run 路线可移植。覆盖表把 D04 记为已接线，没有登记评估时尝试停在 `HISTORICAL_SEMANTIC_ASSESSMENT_NOT_REGISTERED:LIVE`（`MODEL_REVIEW_NOT_EXECUTED`）——缺输入，不是缺路线、也不是缺披露。运行树端到端（登记→安装→原生 Run→冻结→冷读→公共行）等正在跑的 12 期间批次结束后实测。材料见`docs/evidence/issue47_history/semantic-route-wiring/`。

Issue #47 C02 十个已发布值逐条判读（`docs/evidence/issue47_history/c02-board-read/`），**十个全部撤回**。246 条摘录对照已批定义（"董事人数、独立董事数、委员会信息"）：82 条构成事实、51 条构成政策流程、19 条委员会职能、**94 条在任何口径下都不是董事会构成**——高管/董事薪酬（含"Audit Committee Chair Retainer"这类薪酬表行名）、审计师独立性与预先批准、股权计划条文、履历里的他公司委员会、签名标签。每个值至少三条。**成因是量出来的**：冻结选择器的"委员会信息"标签只要求块里任意处有委员会名、任意处有 'chair of'/'members of'/'serves as' 之类的词，两者不必相关、也不必说本董事会；94 条里 87 条是这个标签放进来的。选择器被 issue_28 各世代冻结、与普通路线共用，普通路线的 C02 同样如此——#47 是继承。**判读绑定路线今天的选择**：`tools/read_c02_board_statements.py` 在当前代码下从已保存字节重推选择，判读不逐块逐文本对上就拒绝出结论。**没做的两件**：漏选方向没读（值已经不能接受，另一方向不改变结论；修复时必须补读）；没修——修复是后继选择器，而目标一半是口径问题（中间两类算不算"委员会信息"），推荐只收构成事实，待所有者定；不论选哪个，94 条都在外面，但去掉它们的规则要先用这 246 条校验，D02 关键词代理那次显而易见的收窄丢掉的正确文本比错误文本多。**顺带纠正本文件先前的一句**："C02 的 29 条摘录内容验收未做"——做了，结论是撤回而不是接受。

Issue #47 Paramount 前身 FY2024 的三个值读完（D01 接受、D02 撤回并修了一半、C02 已随十值撤回）。**D02 两个方向都读**：16 条里 15 条是 Item 3 引入句与被引入的 Note 19「Legal Matters」标题范围，第 2010 块是公司事项附注里的"交易相关项目"段（交易成本与 2022/2023 股东诉讼和解金额），因含 "litigation" 被 `_LEGAL` 关键词代理放进来——与继任年 2108 块同一判断，登记为缺陷；**另一方向查出斜体事项标签"Asbestos"(2877)、"Other"(2885) 被丢**。**先普查再修**：12 份 D02 申报的 Item 3/被引入附注范围内所有未取块，只有页码、目录链接、续页页眉、一个页脚，和**恰好 3 个斜体短标签，全是 Paramount 的**（2025 的 3249 也是，此前的 2025 阅读没看见）。`_note_heading` 现在像读下划线一样从块自身 span 读斜体；差分实测只加这 3 块、不减任何块，其余 10 份 D02 候选逐字节不变，12 份 D03 集合全部不变。这不让任何值变成可接受——两年都还有关键词代理那一块。**D01 查出的是我自己读取器的缺陷**：前身封面写 "For fiscal year ended"（无 "the"），读取器的封面正则要求 "the"，于是跳过封面、匹配到附件清单里对 2021 年报的引用，把一个逐行一致的值报成 DIFFERS——**查不到时落到一个错误匹配，比报"读不到"更糟**，这是本 Issue 第三次这个形状。改为 "the" 可选后既有 15 个阅读用例不变，FY2024 读为 MATCH（51 条标题逐行一致、CIK 813828），接受登记 154→155。

Issue #47 B06 四个已发布值读完并接受（`tools/read_debt_to_equity.py`，登记 155→159）。**先撤回我自己登记里写过的一句**："其中两个是从发布值反解出组成再去找概念、规则无法只从申报陈述（Salesforce 同时带两个债务模型的输入）"。那是**读取顺序**的问题，不是申报说不清：已批定义与各后继 Spec 都是"期末账面债务（含融资租赁）/ 同范围权益"，而融资租赁在不在"债务"行里，**申报自己的租赁附注写着**——Macy's 列在应付与长期租赁负债下、Salesforce 列在应计费用与其他非流动负债下（都在债务行之外，所以要加）、Paramount 的债务表本身列有"Obligations under finance leases 3"（在内，不加）、Enphase 明写"does not have any finance leases"。新读法先按申报分类决定是否加融资租赁、全部数取自该行的 inline XBRL，最后才看发布值：四个读数与发布值逐位相同。除数取母公司股东权益（Paramount 有少数股东权益，11,693 而非 12,887），这是把已批定义的 "shareholders' equity" 读作母公司口径，写进了接受的局限里。**注错用例第一版没打中**：把"Accounts payable and accrued liabilities"替换成债务标签，而 HTML 里这串字被标签切开，替换什么都没改、用例却因别的原因失败；改为只在那张租赁表内替换并先断言替换已生效。

Issue #47 六个金融指标（A03/A04/A09/A11/A12/A13）的适用分支已接通（`historical_financial_results.py` 第 32 个规则文件），**覆盖表的"仅结构适用"随之为空**。复审指出"仅结构适用"其实是一侧的实现缺口：门关着的一侧有答案，门开着的一侧——本帧唯一的银行——没有路线。移植的是普通路线 `resolve_current_financial_metric`：它唯一的期间依赖是"取最新年报"，其后全是准备的函数，所以把准备显式交进来，冻结的 Spec 检查、六个检查器、测量期间规则与记录形状原样导入。**两处有意差异**：钉定准备除三份被读证明外还带修订件与前身目录块的证明，所以按 URL 认那三份（普通解析器要求恰好三份，对带修订件的钉定准备会误判不完整）；交给解析器的是未重标注的 `original_input`。修订件按普通路线回答（扣留），**不**查已批修订政策——普通路线的政策接线没到这一族，放宽接受哪些输入不是移植该做的决定。**验法是替换本身**：同一份准备交给两个解析器，六个指标在 JPMorgan 真实最新年报上每个返回字段都相等且都交付值（A03 1.11、A04 0.025、A09 0.0066、A11 4.791 万亿、A12 4000 万、A13 427.58 亿）；五次注错全被抓。绑定集合是测出来的：新进程跑完六个指标，只有本模块与一个数据文件（A03 检查器经 R4 任务目录读的 PR-B 政策证据，正文已被目录摘要钉住）未绑定，都已写进 mint。**今天移动零个交付值**：JPMorgan 每个期间都停在期间选择（保存的目录不一致），本轮移动的是 30 个位置的"路线存在"那一层。**同轮测出一个跨路线的实现缺口，获取解决不了**：来源集发现只读主 submissions 文档的 recent 块（本路线、accession 路线、Company Facts 与零 AI 报表路线的 `_exact_set` 都如此），而 JPMorgan 主文档只列 2025-08-15 以后的申报、Salesforce 的只到 2023-07-31，所以 50 个目标期间里 6 个（JPMorgan FY2021–FY2024、Salesforce FY2022–FY2023）的目标行只在历史块里——原件与目录全部取得后，这些路线仍会在发现处停下。修法已知（以目标行所在的块作清单，冻结路由器本就接受历史块），但今天没有"原件已存且行在历史块"的真实正例。**同轮还修了两处早已红的测试**：`test_historical_structural_results` 断言"仅结构"集合等于全部 8 个门控 Spec，B10/B11 接通那天起就不成立；`test_historical_coverage` 的 Macy's 无 Run 帧因 C02 坐标级缺陷被计为 1 个撤回——那条是 `_matching_defect` 自己违反了自己的文档（"撤回该位置的结果"，没有结果就没有东西可撤回），修在匹配器上而不是改测试。两处都是 CI 这一层从未跑到绿色终态时漏掉的，这是本文件第三次记同一件事。材料见 `docs/evidence/issue47_history/financial-route/`。

Issue #47 读历史块的钉定期间：**跑通唯一一个银行期间的端到端，撞出两个从未被任何可达期间碰到的缺陷，都已修**。本仓库保存的 JPMorgan 目录不一致，所以它每个期间都停在期间选择；刷新链的记录根（索引按已存六个历史块自身正文重推、其余全是已存字节，`RECORDED_TEST_ONLY`）上 FY2025 能选出来，而且它的选择读了六个历史块——这是本仓库唯一"原件已存且选择读历史块"的期间，所以是下面两件事唯一的真实字节正例。**(1) 安装器从来源根读 Requirement**，普通安装器从运行树读；任何外部来源根（获取会话产出的正是这种根）都在复制之前停在 "Requirement JSON is missing"——从没有调用方传过来源根，所以没人见过。**(2) 钉定输入不带选择读过的本注册人历史块**；修 1 之后，新数据根里的重建重推自己的选择，停在 `Request-ledger locator evidence is invalid`——前身年份补带前身目录块之前撞的就是这一句。修法是选择记录的 `loaded_inventories` 除主文档外每块的证明都随输入携带；12 个可达期间的选择都只读主文档，所以**零变化**（用例断言 Marriott FY2023 仍恰好 3 份证明），受影响的是实测会读块的 Pfizer FY2021、Salesforce FY2022–FY2024 与 JPMorgan——它们今天都缺原件，获取之后才会走到这里，不修就会在安装那一步全停。修后 JPMorgan FY2025 的 A04 走完装入→原生 Run→冻结→公共行→另一进程冷读，值 0.025 与普通路线同一申报上的值相同；A03（季度平均窗口，Run 坐标走结果窗口）与 A09（瞬时）同样跑。两处注错各被对应用例抓住。**仍未修、也没有真实正例的**：目标行本身在历史块里（JPMorgan FY2021–FY2024、Salesforce FY2022–FY2023）时，四条钉定路线的来源集发现仍只看主文档 recent 块。**顺带一个操作教训**：用 `pkill`/`pgrep -f` 找等待进程时我又一次把自己的 shell 杀掉了（模式串出现在自己的命令行里）；按 PID 杀，本文件已写过这条。材料见 `docs/evidence/issue47_history/financial-route/`。

Issue #47 D04 端到端第一次跑通，**先在渲染公共行处失败了一次**，正是"单元全绿、接上去才露"。接线那一轮只在组件层验证（钉期来源逐字节等同冻结构建器、登记/加载/拒绝），运行树端到端留到批次之后；在独立的运行树副本里跑 Marriott FY2023（记录模式，合成输出只是冻结检查器自己推出的关系）：登记、安装、原生 Run 都通过并冻结，然后 `render_historical_run` 在 `result["text_payload"]["items"]` 上抛 `TypeError`——历史渲染器假设每个文本结果都有 payload，而 D04 的"定义范围内无疑虑披露"没有。普通渲染器为此有一个分支 `capacity_run.project_defined_absence`（先重推候选、候选为空才陈述缺失，按审阅过的每份文档出一条范围检查证据行），历史渲染器没移植；**同一处还缺普通渲染器标注"来源分类用的是记录测试响应"的行备注与收据字段**，缺了它，合成评估的行读起来就像模型给的。两处都已补（复用而非重写那个分支；标注只看 Run 输入是否携带登记评估，不问文本组件恰好有没有 `mode` 字段）。修后：默认 LIVE 安装按名拒绝，Run `FROZEN`（`WITHHELD`/`D04_DEFINED_SCOPE_NO_DOUBT_DISCLOSURE`），公共行 `TEXT_QUAL`、1 条范围检查证据、注明记录测试响应，收据 `semantic_assessment_mode: RECORDED_TEST_ONLY`，另一进程冷读同 run_id 同 result_id，零调用；脚本现在把"行与收据标明记录模式"作为通过条件。材料见 `docs/evidence/issue47_history/semantic-route-wiring/recorded-d04-marriott-2023.json`。

Issue #47 12期间批次与两处接上去才露的缺陷：批次（closure `sha256:aa0eff7e…`）442个位置、440个冻结Run与公共行、2个失败（Marriott 2023/2024 C02，代理未保存）；178个值里159个被独立读过、16个被开放缺陷撤回、3个（E01）等8.01决定；八个修复后的结果在本版本下以相同result_id复现，各按本版本登记释放。**(1) 安装器按目录名选规则输入是错的代理**：JPMorgan 记录根扫描里 B13 以 `No such file … 02_指标定义_SEC_10公司单年指标.md` 失败——B13 从数据根读已批定义，定义在仓库根，而安装器照抄父代只复制 `config/`、`catalog/`。改为"非代码、非展示的执行授权输入全装"，两个方向各一用例、两个注错各被抓，同一个根补装后 B13 出公共行。**(2) 申报行在历史块里的目标期间**（`historical_filing_inventory.py`，第33个规则文件）：用 Marriott 已存材料造记录根，把2024-06-30及之前的行整体移进新块——FY2025不变、FY2024上一期行在块里、FY2023本期行在块里。修前FY2023有16个指标以 `Filing accession is absent from submissions` 停下（accession三个、公司事实十一个、B01/B03），FY2024零差异；修后三期39个指标答案逐字段相同。**本轮被实测推翻的两句自我判断**：我先说"事件窗口会跨块而冻结清单只收一个目录"——冻结事件路线本就按窗口读每个重叠块、每块一份清单再汇总；又说"公司事实的上一期也只看主文档"——冻结的 `_prior_filing` 本就跨块并返回列出它的那份目录。所以缺口只在"本期目标申报"这一个角色上，修法就是对本期问冻结代码已为上一期回答的同一个问题。**测量先错了一次**：文本类 Run 输入的 `primary_result` 按设计为空，第一版比较报 D01/D02"相同"是两个相同的 AttributeError；改比 Run 工厂导出的候选后，才看到 FY2023 的 D02 身份不同——它记录了申报行的出处（新块第62行而非主文档第329行），摘录逐条相同，这是溯源不是答案变化。八个注错全部被抓。**JPMorgan FY2025 记录根整期**：38个位置37个出公共行（17个值、12个结构性不适用、8个具名扣留），D04按名停；C01/C04/E01–E05 的第一个阻断都是同一份8-K头文件最近一次请求失败，逐份量过事件遍历自己的材料（20份8-K，正文全存，头文件19份可用，不可用的恰好是那一份），C04 另读的本期与上一期审计师材料（两份 accession 的索引与 XBRL 实例）实测都已存；遍历之后的步骤没有量。材料见 `docs/evidence/issue47_history/block-resident-filings/`、`financial-route/recorded-bank-sweep.json`、`native-run-batch-12-periods/`。

Issue #47 闭包`30a7934b`定向运行（18个位置，不是全量帧，材料见`docs/evidence/issue47_history/targeted-round-30a7934b/`）：D02的Paramount两期与12期间批次逐行比较只增不减、保留行顺序，增的正是斜体修复普查点名的三个事项标签（FY2024 "Asbestos""Other"、FY2025 "Asbestos"），两坐标仍被关键词代理缺陷撤回；FY2024那次创建在Run冻结后被容器重启打断，另起目录重建得到同一run_id与result_id。D04在12个期间全部按名停在`HISTORICAL_SEMANTIC_ASSESSMENT_NOT_REGISTERED:LIVE`、零调用。重分块记录根上的块内目标期（Marriott FY2023）B01/B04/B12/D02走完整条Run链、冷读一致、值与未改动的根相同，四个Run都点名新块；首跑在`ORDINARY_SOURCE_UNREGISTERED_LEDGER`停下，因为那个根由仓库树建、检查点登记在仓库树的信任日志里——信任根按代码树划分是设计，改由运行树自己的会话重建而不是绕过。驱动作为`targeted_runs.py`提交。
