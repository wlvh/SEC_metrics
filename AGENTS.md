...
Issue #47 D02条目上限：**修订机制已建好，但64不是一个界而是两个，所以还没有接通**。第一层是`specs.py`规定任何`TEXT_V1` Spec最多声明64条——该文件字节被14个已冻结世代（`issue_28_v2`–`v14`及`issue_47_v1`）的execution authority点名，改它实测导致`native_request_construction`对`issue_28_v14`开作用域时抛`NATIVE_REQUEST_CONSTRUCTION_RULE_CHANGED:scripts/vnext/specs.py`，13个测试红、整条continuous语义调用路线起不来。该层已解决：新增第16个规则文件`scripts/vnext/historical_spec_revision.py`承担放宽后的上限，`specs.py`字节不动。它不替换编译——冻结`compile_spec`仍做全部解析校验；本模块把后继前置内容按前驱的界编译一次、要求`compiled`与`prompt_bundle`与前驱完全相等，再放回声明值并用冻结哈希函数重算三个哈希。因此这是机械证明“只动了一个数”：改sections/字符界/allowed_source_roles/name/disclosure_group/quality_rule/metric_id的后继一律被拒，且有用例断言`SPEC_FIELDS`全部24个字段都落在两侧之一（否则相等判断有缝）；另有用例断言空applicability/非法renderer由冻结编译器先拒，以证明后继路径只加检查不减检查。代价比第一版小：`issue_47_v1`不再重记任何继承文件，一个数据根可同时满足两者。**第二层未解决，是当前待决项**：64同时是`ORDERED_NEWLINE_V1`文本结果协议的结构常量，硬编码在`text_results.render_text_payload`第86行，与Spec无关。实测一次D02 Run经过该函数122次、5个调用点（`payload_from_observations`/`build_text_result_and_trace`/`validate_text_record`/`verify_text_trace`/`projector._projection_value`）全部超界。仅在进程内换掉该函数（磁盘零改动，所有字节校验照常通过）整条链走到`PUBLIC_ROW`，证明除这一个字面量外没有别的阻断。但`text_results.py`是`issue_28_v11`的new_rule_file（数据根与代码根双向校验）且在v14授权内，所以后继要自带约155行的5个函数，并在`calculator`/`records`/`constraints`/`projector`/`run_store`各加一条路由——注册补丁由3文件9处扩成约6文件14处，其中`validate_text_record`与`verify_text_trace`正是文本结果的完整性检查，复制品静默漂移时照样全绿。因此暂不动手，按既有机制交回决策。影响面已实测：2025-12-31七家可选公司中只有Pfizer超界（92条），次高Ford 53条，另三家因财年/已存历史无法在该期末选出（非D02结论）。`catalog/r6/D02_legal_disclosures_v2.md`（192）与其编译器均已提交并有测试，但`TEXT_SPEC_PATHS["D02"]`**仍指向v1**：让已接通的Spec声明运行时兑现不了的192，是在目录里写假合同。材料见`docs/evidence/issue47_history/d02-item-bound/`。
