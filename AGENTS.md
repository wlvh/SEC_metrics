...
对另外八个D02结果做内容核查（读已通过结果的正文）又发现反向缺陷：被引用附注只在不被其他范围包含时才保留附注身份并整取，落在Item 8内的改按六个关键词过滤，按原文写法点名诉讼的块被丢弃（Pfizer 39块19,918字、Lumen 11块7,689字、Paramount 6块、Salesforce 4块、Marriott 1块）。`referenced_note_candidates`让附注仍按附注整取，去重改为"最内层范围拥有该块"；只有`EXACT_NOTE`解析才整取，Pfizer的Note 16A解析为`WIDER_PARENT_NOTE`（135块、125条超Spec 64条上限）故保留原行为并留在覆盖记录。实测四家增加、零块丢失，其余五家逐字节不变，全部在Spec双上限内。未做：Pfizer带字母子附注的精确定位。
