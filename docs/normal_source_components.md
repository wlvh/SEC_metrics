...
旧22项已全部接到普通已保存来源（包含明确WITHHELD，不表示全部已有数值），输出现有Observation、Trace和Result记录，当前普通入口连同金融、文本和住宿路线共支持36项，仍为未冻结V14的CLI与原生Run草案。实际原件、来源失败和开发缺口逐项保留；当前材料不证明实时SEC最新或正式采纳。
...
- Southwest的有限附件链接更正已经过原件验证并接入普通数值和事件路径。Paramount的B08/B09遵循原目录允许的当前主体/时点，经单独有限修订证明接入；全年和跨期指标仍保留主体范围缺口。结构性不适用与这些限制分开；见`docs/ordinary_instant_balances.md`。
- Salesforce当前财年原文2026与DEI/同申报CF标签2025的冲突由普通输入v2明确保留，当前新候选采用已证明的原文财年标签，实际起止日期不改。历史输入和Run不重签；年标签差异不能直接绕过其他表格路径的年度核对。
