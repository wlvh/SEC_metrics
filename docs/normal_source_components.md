...
本轮把旧22项中19项接到普通已保存来源，输出现有Observation、Trace和Result记录，尚未创建后继Run或扩展12指标CLI。实际原件、来源失败和开发缺口逐项保留；当前材料不证明实时SEC最新或正式采纳。
...
两个入口都只接受数据根、配置公司ID和受支持指标ID（第二个入口一次返回11项）。没有调用者提供的答案、期间、事实或选定申报参数；验证入口从原件完整重建，不接受自洽但改过结果的JSON。数据目录可以不含Git；安装的原始获取基线、来源账本和实际body/header必须仍通过验证。代码未调用旧release准备器或读取历史矩阵答案。
...
作者报告、独立发现、修复后原反例、真实材料、测试和首次失败分别保留在`docs/evidence/issue28_continuous/successor-source-components/`。这些组件尚需进一步独立审阅、A01/A02/B12接线、统一Run、来源更新及完整390坐标验收；费用与最终生产权限仍按总委托分别核实。
