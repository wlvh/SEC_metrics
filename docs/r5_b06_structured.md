...
借款、债券及融资租赁按报表确认账面额计入；经营租赁排除，融资租赁已合并确认的非租赁组分不剥离。允许补齐已证明不重叠的类别，禁止总额与current、已含租赁再次相加。固定原文关系由独立证据绑定，数值与披露精度全部从原生XML重读。Macy的本金加溢价减成本再加租赁由既有Calculator/Trace实际运算，编译器只补具名嵌套算式检查，未提高AST上限。

新集合明确适用于十公司。Pfizer的借款子集合虽可计算，融资租赁完整性仍未证明，和JPM采用同一标准。Ford补充资产负债净额不是已建立的工业归母权益分母。三个缺口分别保留，不能以0替代。没有放行不完整总额或把无标签当零。旧v1/v2 Spec/Run/候选保持原解释。
