...
共同来源输入现可在消费者版本变化后复验：完全相同的原请求和原件、原生成功收据以及当前含义检查必须同时通过，保留原执行/接受ID并登记独立当前复验，不新增模型调用。v3/v4等实际请求/协议变化不能借此互换；原68仍被当前内容规则拒绝。B13/D04复用这一输入和Run机制，D04的原可行性记录仍无原生信用。
