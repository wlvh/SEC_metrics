...
1. 先读 `outputs/validation_run_manifest.json`，确认 mode、result，以及本次真正刷新的 tracked validation/audit artifact；`source_commit` 带 `+dirty` 表示运行时工作树含未提交改动。Golden、矩阵与 evidence 不在这份最小 manifest 中，仍需各自的重跑来源。
2. 再读 `REPORT_十公司财务指标.md`，了解本批次摘要、coverage、例外和 verdict。
3. 在 `outputs/metrics_matrix.csv` 按 company 与 `metric_id` 找到具体结果。
4. 用 `outputs/coverage_matrix.csv` 和 `outputs/exceptions_and_review_items.md` 判断缺口类型。
5. 需要采信非空数值时，在 `outputs/metric_evidence.csv` 核对同一 company/metric 的来源与口径。
6. 最后只核对 manifest `refreshed_artifacts` 中的 `repair_validation_results.csv`、`stratified_audit.csv` 等证据；旧文件存在不代表本次运行已评估。
...
full validation 需要本地 raw evidence、请求日志和 concept inventory 的完整工作区形状，并且仍要以 Golden、repair gate 与 manifest 的实际结果为准。full 关键检查出现 `NOT_EVALUATED_MISSING_EVIDENCE` 时必须 NO-GO。light review 只验证随包范围，`SKIPPED_LIGHT_PACKAGE` 或 `NOT_EVALUATED_MISSING_EVIDENCE` 必须成为显式 caveat；不能将其宣传为 full validation，也不能把“没有发现失败”改写成 PASS。
...
- Golden、P0 validation、workspace 完整性或分层审计出现失败，或 full 关键检查为 `NOT_EVALUATED_MISSING_EVIDENCE`。
