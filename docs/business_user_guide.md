...
未来存在 active publication 后，任何 APPLICABLE/WITHHELD 或不完整 bundle 都不能替换 active。Projector 必须重新加载 persisted FROZEN Run，从仓库 release plan 取得迁移 exact set，并拒绝同一 legacy key 的多个 scope；publisher 必须读取 bundle 内 ProjectionManifest，不能用另一份游离 Result 把 BLOCKED 改成 PUBLISHABLE。publication receipt 必须以无自引用 candidate view 同时绑定 Requirement、Run、ledger、predecessor 和每个非 receipt artifact 的 path/SHA-256/size，并证明 exact required gates 全部通过；rollback 只能回到当前 active 已记录的 committed predecessor。
