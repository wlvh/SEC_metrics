...
O=Path(__file__).parent;(O/'measurement-code-fixed-check.json').write_text(json.dumps(reports,indent=2));print(json.dumps([{'company':r['company'],'error':r.get('error'),'result':r.get('real_fact_result',{}).get('quality'),'counterexample':r.get('function_level_conflicting_CF_carrying_case',{})} for r in reports],indent=2))
