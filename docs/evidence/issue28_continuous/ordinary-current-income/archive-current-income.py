...
R=Path('/Users/lyuhongwang/Developer/SEC_metrics');T=Path('/private/tmp/sec_metrics_issue28_continuous');L=Path('/Users/lyuhongwang/.local/state/sec_metrics/issue28-2026-09-13');E=R/'docs/evidence/issue28_continuous/ordinary-current-income';E.mkdir(exist_ok=True)
folders=['current-income-native-first','current-income-native-attacks','current-income-provider-wiring']
...
with tarfile.open(archive,'w:gz') as tar:
 ...
 selected=set()
 for pattern in ['current-income*.json','current-income*.log','current-income*.csv','successor-stub-calculator-probe.*','successor-financial-amendment-probe.*','successor-income-amendment-scan.*','successor-revenue-period-probe.*']:
...
for name,source in [('provider-summary.json','current-income-provider-wiring/summary.json'),('actual-counts.json','registered-real-final-counts.json')]:
 (E/name).write_bytes((T/source).read_bytes())
coords=[]
for folder in ['current-income-native-first']:
...
assert len(coords)==2
(E/'income-coordinates.json').write_text(json.dumps({'coordinates':coords,'full390_acceptance':False,'production_authorized':False},indent=2)+'\n')
