...
GitHub在180a05e的fast实际失败：96入口中13个原件聚合入口触及每入口30秒，日志无断言失败，完整失败JSON另存。CI并发从4改2以减少原文解析争用，总job上限从5改10分钟以容纳新增范围；每入口30秒不提高，检查及断言均保留。新CI结果另行核对，不能把本地PASS写成远端已通过。
