...
with patch.object(socket.socket,'connect',side_effect=AssertionError('NO_NETWORK')):
 ...
 mismatch=observed[0]!=configuration['registered_update_options'];assert mismatch
 rows.append({'case':'finite_native_preparation_vs_current_run_contract','outcome':'CONFIRMED_MISMATCH','prepare_options':observed[0],'ordinary_run_options':configuration['registered_update_options'],'ensure_return':returned['status'],'boundary_double':'Only prepare_registered_update; real ensure_native_update and _config. No actual source execution or model call.'})
