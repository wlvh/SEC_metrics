...
with patch.object(socket.socket,'connect',side_effect=AssertionError('Network forbidden')),patch.object(socket,'getaddrinfo',side_effect=AssertionError('DNS forbidden')):
 ...
 for label,text in [('nominal-production','Our production volume totaled 80 widgets in the first quarter.'),('nominal-capacity','Our available production capacity amounted to 150 widgets per month.')]:
...
(out/'variations.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print(json.dumps(report,ensure_ascii=False,indent=2))
