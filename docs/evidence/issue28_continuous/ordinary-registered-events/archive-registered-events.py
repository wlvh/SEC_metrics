...
R=Path('/Users/lyuhongwang/Developer/SEC_metrics');T=Path('/private/tmp/sec_metrics_issue28_continuous');L=Path('/Users/lyuhongwang/.local/state/sec_metrics/issue28-2026-09-13');E=R/'docs/evidence/issue28_continuous/ordinary-registered-events';E.mkdir(exist_ok=True)
folders=['registered-events-sec-wiring','registered-events-provider-wiring']
...
with tarfile.open(archive,'w:gz') as tar:
 ...
 selected=set()
 for pattern in ['registered-events*.json','registered-events*.log','registered-events*.csv']:
...
for name,source in [('sec-summary.json','registered-events-sec-wiring/summary.json'),('provider-summary.json','registered-events-provider-wiring/summary.json'),('actual-counts.json','sec-live-thirteen-count-check.json')]:
...
print('VERIFIED',len(files),'file mappings;',len(objects),'stored unique objects;',archive.stat().st_size,'bytes;',req['requirement_closure_hash'])
