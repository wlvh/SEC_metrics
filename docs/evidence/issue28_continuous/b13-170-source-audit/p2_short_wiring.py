...
out=Path('/tmp/issue28_visible_role_wiring_cancel_p2');out.mkdir(exist_ok=False);e=Path(__file__).resolve().parent;started=time.monotonic();r=load_requirement_snapshot(snapshot_dir=ROOT/'requirements/issue_28_v14')
...
with patch.object(socket.socket,'connect',side_effect=AssertionError('NO_NETWORK')),patch.object(socket,'getaddrinfo',side_effect=AssertionError('NO_DNS')),patch('sec_http.urlopen',side_effect=AssertionError('NO_SEC')),patch.object(control,'effective_invocation_policy',side_effect=AssertionError('NO_LEGACY')),request_construction_session(r):
 ...
 good=selected[1];request=json.loads(good.request_bytes);assert not request['required_candidate_assessments']
 ...
 observed=[];results=[]
 for name,obj,wire,expected in [('valid_empty_group',good,positive,'SUCCEEDED'),('original111_current_semantic_rejection',selected[0],(old/'wire/raw-response.bin').read_bytes(),'FAILED_TERMINAL')]:
...
summary={'status':'PASS_CURRENT_B13_FACTORY_OPENER_CONTROLLER_POSITIVE_AND_SOURCE_ROLE_REJECTION','requirement_closure_hash':r['requirement_closure_hash'],'prior_receipt_error':prior_error,'mock_opener_calls':observed,'results':results,'original111_request_identity_unchanged':True,'original111_current_reuse_credit':False,'old111_terminal_changed':False,'complete_company_result':False,'new_calls':[0,0,0],'network_disabled':True,'legacy_default_forbidden':True,'real_request_factory_to_controller_verified':True,'seconds':round(time.monotonic()-started,3)}
(e/'p2-short-wiring-summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary),flush=True)
