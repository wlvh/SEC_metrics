"""Authorized one-shot Enphase then Ford; no redraw, no automatic restart."""
...
out=Path('/tmp/sec_metrics_b13_live_20260922');out.mkdir(exist_ok=False)
...
assert initial['counts']==[60,60,49] and len(initial['rows'])==109 and not initial['stopped_channels']
...
started=time.monotonic();report={'status':'RUNNING','code_requirement':requirement['requirement_closure_hash'],'before':initial['counts'],'companies':[],'allowed_provider_requests':17,'production_authorized':False}
def save():
