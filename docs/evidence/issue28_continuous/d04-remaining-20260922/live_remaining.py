...
out=Path('/tmp/sec_metrics_d04_remaining_live_20260922');out.mkdir(exist_ok=False)
...
def save():
 ...
 (evidence/'live-summary.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
 assert report['new_calls'][0]<=71 and report['new_calls'][2]==0
 return state
companies=[('marriott_international',4),('southwest_airlines',6),('jpmorgan_chase',27),('salesforce',5),('lumen_technologies',8),('macys',5),('paramount_skydance_paramount_global',10),('enphase_energy',6)]
assert sum(n for _,n in companies)==71
save()
try:
 ...
 report['status']='COMPLETE_EIGHT_NATIVE_COORDINATES' if all(r['status']=='COMPLETE_NATIVE_OPEN_AND_PUBLIC_ROWS' for r in report['companies']) else 'BOUNDED_ATTEMPT_COMPLETED_WITH_COORDINATE_GAPS';save()
