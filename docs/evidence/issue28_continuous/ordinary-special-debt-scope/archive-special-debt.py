...
R=Path('/Users/lyuhongwang/Developer/SEC_metrics');T=Path('/private/tmp/sec_metrics_issue28_continuous');L=Path('/Users/lyuhongwang/.local/state/sec_metrics/issue28-2026-09-13');E=R/'docs/evidence/issue28_continuous/ordinary-special-debt-scope';E.mkdir(exist_ok=True)
folders=['special-debt-native-first','special-debt-native-final','special-debt-native-attacks','special-debt-native-attacks-fixed','special-debt-sec-wiring','special-debt-sec-wiring-final','special-debt-provider-wiring','special-debt-provider-wiring-final','special-debt-provider-wiring-bound']
...
with tarfile.open(archive,'w:gz') as tar:
 ...
 selected=set()
 for pattern in ['special-debt*.json','special-debt*.log','special-debt*.csv','ci-4bc3-next.json']:
...
for name,source in [('sec-summary.json','special-debt-sec-wiring-final/summary.json'),('provider-summary.json','special-debt-provider-wiring-bound/summary.json'),('actual-counts.json','sec-live-thirteen-count-check.json')]:
 (E/name).write_bytes((T/source).read_bytes())
coords=[]
for folder in ['special-debt-native-final']:
 summary=json.loads((T/folder/'summary.json').read_text())
 for c in summary['coordinates']:
  assert c['status']=='WITHHELD_CANDIDATE' and c['public_row_status']=='CANDIDATE_ROW_PREPARED'
...
assert len(coords)==2
(E/'scoped-coordinates.json').write_text(json.dumps({'coordinates':coords,'full390_acceptance':False,'production_authorized':False},indent=2)+'\n')
