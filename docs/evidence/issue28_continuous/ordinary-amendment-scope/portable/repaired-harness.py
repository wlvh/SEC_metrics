...
out=pathlib.Path('/tmp/sec_metrics_issue28_continuous/amendment-portable-material-second');out.mkdir(exist_ok=False)
...
except (ValueError,FileNotFoundError,BatchWorkflowError) as e:print('MISSING_AMENDMENT_REJECTED',type(e).__name__,str(e)[:150])
finally:p.write_bytes(original)
p.write_bytes(original+b' ')
try:
 with original_sources_only(): prepare_saved_amendment_input(repo_root=data,company_id='southwest_airlines',input_class='ORIGINAL_STATEMENT_VALUES')
 raise AssertionError('Changed amendment body accepted')
except (ValueError,BatchWorkflowError) as e:print('CHANGED_AMENDMENT_REJECTED',type(e).__name__,str(e)[:150])
finally:p.write_bytes(original)
p=data/POLICY_PATH;original=p.read_bytes();p.write_bytes(original+b' ')
try:
 with original_sources_only(): prepare_saved_amendment_input(repo_root=data,company_id='southwest_airlines',input_class='ORIGINAL_STATEMENT_VALUES')
 raise AssertionError('Changed policy accepted')
except (ValueError,BatchWorkflowError) as e:print('CHANGED_DATA_POLICY_REJECTED',type(e).__name__,str(e))
finally:p.write_bytes(original)
'''
