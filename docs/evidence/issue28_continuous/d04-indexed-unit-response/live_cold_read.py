...
assert r['receipt']['semantic_assessment_mode']=='LIVE'
assert not blocked
out={'status':'PASS_COPIED_OPEN_RUN_COLD_REPLAY','run_id':m['run_id'],'result_id':r['receipt']['result_id'],'record_count':len(records),'review_decisions':len(decisions),'network_or_process_events':blocked,'semantic_assessment_mode':'LIVE','new_provider_paid_sec_calls':[0,0,0],'production_authorized':False}
