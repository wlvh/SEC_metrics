  ...
  for need in missing:
   ...
   try:
    ...
    rows.append({'company_id':cid,'missing_html_url':need['source_url'],'classification':'OPTIONAL_PRIOR_HTML_WITH_VERIFIED_NATIVE_INSTANCE_ALTERNATIVE',
     ...
     'source_period':periods[0],'filing':chosen[0],
     ...
     'file_set':reader.file_sets[-1],'scope':'Native accession identity/annual period and optional-HTML branch only, not a new metric Run','new_sec_calls_required_for_this_alternative':0})
