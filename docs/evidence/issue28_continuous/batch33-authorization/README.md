...
用户随后报告已充值；这只更新了用户对服务状态的说明，未将原172停止自动清除。`continuous_recovery_172.py` 与`config/issue28_recovery_172_v1.json`已准备**默认关闭**的一次性路径：缺确切用户恢复决定和服务器转录评论时，不安装`recovery-172.json`、不产生第173次申领。`recovery172-offline-summary.json`用当前Enphase保存来源验证旧失败、模拟新402、独立测试授权与同摘要成功的原生收集；失败均保留在前缀，只有新成功替代当前信用。36项定向与127项快速测试通过，真实调用新增0。最终权限绑定、限定独审与真实执行必须在明确决定后再核对；录制结果不计真实公司完成。
