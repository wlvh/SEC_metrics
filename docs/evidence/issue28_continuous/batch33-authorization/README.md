...
后继小差异收窄了“本批SEC0”的解释：它禁止把SEC请求记到33/66批次内，**不撤销**原总委托下独立的Issue28 SEC来源获取准入。批次首次D04业务申领后，独立SEC claim仍由原权限、余额和SEC通道停止守卫判断，不带批次授权标记；账本及原生输入的冷读保留它与provider claim的确切前驱顺序。限定独审在补丁`1c6bb186`发现一项P2：保存历史的验证器原可误接纳SEC已停止后的第二条SEC申领，真实账本会拒绝。定向反例先复现，再把SEC停止条件接入冷读，31项定向和127项快速测试通过；修后增量独审仍待完成。第172次新HTTP402仍关闭PROVIDER，不能据此清除或重发；本项代码修正本身没有发SEC请求。
