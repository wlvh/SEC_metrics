"""Persist the real Paramount D04 candidate and independently cold read it."""
import hashlib
import json
import os
import shutil
...
original = Path('/tmp/sec_metrics_issue28_d04_paramount_real_20260924')
destination = Path('/Users/lyuhongwang/.local/state/sec_metrics/issue28-2026-09-13/native-candidate-results-20260924')
target = destination/'paramount_skydance_paramount_global'
...
with (evidence/'cold.log').open('wb') as stream:
    result = subprocess.run(['/tmp/sec_metrics_cold39_20260922/bin/python', '-c', code,
                            ...
                            cwd=empty, env=env)
report = {'record_type':'ISSUE28_D04_PARAMOUNT_PERSISTED_INDEPENDENT_COLD_READ',
          'status':'PASS_PERSISTED_COMPLETE_CANDIDATE_AND_COLD_READ'
              if result.returncode == 0 else 'FAILED_COLD_READ_NO_RETRY',
          'company_id':'paramount_skydance_paramount_global', 'persistent_candidate_root':str(target),
