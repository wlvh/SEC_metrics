"""Finish the six saved real Enphase D04 groups without another provider call."""
import json
import socket
import time
...
base = Path('/tmp/sec_metrics_issue28_d04_enphase_real_20260924')
assert not base.exists(), 'D04_ENPHASE_CURRENT_MATERIAL_ALREADY_EXISTS'
...
assert before['counts'] == [129, 129, 49] and len(before['rows']) == 178
...
with patch.object(socket.socket, 'connect', side_effect=AssertionError('NETWORK_FORBIDDEN')), \
    ...
    assert len(selected) == 6 and ordinals == [173, 174, 175, 176, 177, 178]
    ...
    install_inputs(data_root=data, company_id='enphase_energy', metric_id='D04',
...
assert after['counts'] == before['counts'] and len(after['rows']) == 178
result = created['result']
body = {'record_type':'ISSUE28_D04_ENPHASE_COMPLETE_REAL_NATIVE_CANDIDATE',
        ...
        'recovered_failed_ordinals':[114,172],
        'assessment_input_id':registered['input_record_id'],
