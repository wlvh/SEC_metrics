...
groups = [deepcopy(json.loads((evidence/'initial-group-set.json').read_text())['groups'][6])]
# Test ordinals are deliberately short; the real server-bound manifest still
# points to original113. This recorded ledger proves the lossless same-digest path.
...
with tempfile.TemporaryDirectory(prefix='issue28-batch33-offline-wiring-') as temporary:
    ...
    original113 = Path('/Users/lyuhongwang/.local/state/sec_metrics/issue28-2026-09-13/calls/0113')
    old_request_bytes = (original113/'semantic-request.json').read_bytes()
    old_source_bytes = (original113/'source.json').read_bytes()
    assert old_request_bytes != candidate.request_bytes
    assert old_source_bytes != candidate.source_bytes
    assert request_digest(json.loads(old_request_bytes), policy) == request_digest(request, policy)
    with ledger.locked():
        failed_path, failed_intent = ledger.claim(channel='PROVIDER',
            request_digest=request_digest(request, policy),
            requirement=requirement, plan_id=content_hash(value='recorded old113 plan'),
            purpose='remaining_development_feasibility')
        control._exclusive_write_bytes(path=failed_path/'semantic-request.json',
                                       content=old_request_bytes)
        control._exclusive_write_bytes(path=failed_path/'source.json',
                                       content=old_source_bytes)
        failed_terminal = fail_recorded(ledger, failed_path, failed_intent,
                                        label='old113-lossy-source-failure', error='')
    ...
    with patch.object(socket.socket, 'connect', side_effect=AssertionError('NO_NETWORK')), \
        ...
        selected, report = select_native_request_variants(prepared_requests=prepared,
            ledger=ledger)
        assert len(selected) == 10 and all(x['original_ordinal'] is None for x in report)
        candidate = selected[0]
        request = json.loads(candidate.request_bytes)
    ...
    assert new_intent['batch_group_id'] == 'D04:paramount_skydance_paramount_global:0'
    assert new_intent['batch_resume_171'] is True
    assessment = collect_native_assessments(prepared_requests=selected, ledger=ledger)
    assert len(assessment['completed']) == 1 and len(assessment['missing_request_ids']) == 9
    ...
    summary = {'status': 'PASS_RECORDED_BATCH33_PARAMOUNT113_LOSSLESS_SUCCESSOR',
        ...
        'original113_saved_request_bytes_differ_current_lossless': True,
        'original113_saved_source_bytes_differ_current_lossless': True,
        'original113_business_request_digest_unchanged': True,
        ...
        'partial_native_collection_missing_requests': 9,
...
(evidence/'offline-paramount-113-summary.json').write_text(json.dumps(summary, indent=2)+'\n')
