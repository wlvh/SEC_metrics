...
保存来源的十公司候选批次入口`tools/vnext_normal_candidate.py`现在对**C04指标自动选择**现有四形式后继，调用者无需为每个公司手动传入`c04_event_forms`；批次`request.json`记录该固定选择。`verify_batch_route.py`在原账本保存来源上禁用网络运行一个Paramount C04坐标，结果为`WITHHELD_CANDIDATE`。批次主函数返回2，表示存在未获结果信用的坐标；验证脚本直接调用该函数并核对结果后**进程退出0**，正式CLI才会通过`sys.exit(main())`把2作为进程退出码。`batch-route-summary.json`证明新批次的原生Run ID、Result ID及两份公开格式行哈希与上段既有独立冷读完全相同，仍为`WITHHELD/null / C04_COMPARABLE_AUDITOR_FACTS_MISSING`；原账本读回仍192槽、143/143/49，本次真实调用0。第一次脚本运行仅因`PYTHONPATH`没有包含仓库根，在进入候选构造前失败，原`batch-route.log`保留；修正运行环境后的`batch-route-retry.log`记录一次通过。这个改动缩减了**保存来源候选批次**的逐公司操作，普通共享函数默认、正常新来源发现/获取和完整自动更新尚未改变；不扩大C04的0/1规则或生产权限。

`c067ffd9`的[限定独审](independent-review-batch-route/conclusion.md)为 **PASS_WITH_BOUNDS**：只读核对其他指标仍传原空路线、Paramount没有假0、新旧隔离根Run/Result/公开行字节相同，并明确上述返回值与验证脚本进程状态的区别。审阅没有重跑十公司批次、正常自动更新或长安装冷读；这些验收不能从本单坐标结论外推。
