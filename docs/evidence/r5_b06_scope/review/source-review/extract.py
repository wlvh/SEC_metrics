...
for company,p in companies.items():
 ...
 for n in root:
  ...
  if re.search(r'Debt|Borrow|Lease|Equity|Discount|Premium|IssuanceCost|FinanceCost',tag,re.I) and cref and instant and instant[0] in ['2025-12-31','2026-01-31']:
   facts.append({'concept':n.tag,'value':text,'context_id':cref,'context_xml':c,'unit':n.get('unitRef'),'unit_xml':units.get(n.get('unitRef')),'decimals':n.get('decimals'),'fact_id':n.get('id')})
  if tag.endswith('TextBlock') and re.search(r'Debt|Borrow|Lease|BalanceSheet',tag,re.I):
   ...
   notes.append({'concept':n.tag,'context_id':cref,'text':clean,'table_rows':rows,'fact_id':n.get('id'),'raw_text_sha256':hashlib.sha256(text.encode()).hexdigest()})
