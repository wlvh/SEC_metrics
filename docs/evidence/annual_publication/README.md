...
原交付之后的最终实现补验见 [PR39 收口记录](close/README.md)：fast实际覆盖、
全新根首次prepare、同一实施提交的两个正向集成测试及代码—运行—包身份关联。
最新工作委托允许五项硬条件全部通过后以merge commit合并；实际R3仍不切换。
下文保留原交付身份、原失败与原审阅范围，不能当作本次重新执行的记录。
