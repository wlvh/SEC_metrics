...
Issue #12 的 exact FSD、immutable R2、exact R3 Addendum、Decision Register、baseline、release plan、旧路径 inventory、semantic runtime、`catalog/`、`fixtures/` 与 vNext code/tests/tools 都进入 source-input closure。Candidate binding使canonicalizer semantic version 2→3，source-plan latest verified immutable request-attempt binding使其3→4，旧路径 inventory冻结Git blob binding使projector semantic version 2→3；当前semantic runtime versions hash为`sha256:59a98abb0ccc2fb3625dd140bc159a1a6c2c8e389e34af452e24e579538decd7`。任一 R2/R3/FSD/Decision 或 semantic byte/path 变化都会使旧 Requirement、approval、Run、Batch、publication 与 snapshot失效；GitHub 当前可变 Issue body不是运行时唯一 authority。
