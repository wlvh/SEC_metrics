...
def validate_workflow_authorization(*, authorization, **kwargs):
    return _module(authorization).validate_workflow_authorization(
        authorization=authorization, **kwargs
    )


def write_run_binding(*, authorization, run_dir):
    return _module(authorization).write_run_binding(
        authorization=authorization, run_dir=run_dir
    )
...
def wrap_live_request(*, authorization, request):
    ...
    if type(authorization) is annual_runtime.RuntimeAuthorization:
        return annual_runtime.wrap_live_request(
