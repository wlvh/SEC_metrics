...
def _validate_decision(*, decision: Mapping[str, object]) -> Dict[str, object]:
    """Validate one approved/rejected/superseded decision record.
    ...
    *, decisions: Sequence[Mapping[str, object]]
    ...
    for candidate in decisions:
        ...
        if decision_id not in groups:
            groups[decision_id] = []
        groups[decision_id].append(decision)
    effective: Dict[str, Dict[str, object]] = {}
    for decision_id in groups:
        records = groups[decision_id]
        by_hash = {
...
def load_requirement_snapshot(*, snapshot_dir: Path) -> Dict[str, object]:
    ...
    decisions = effective_decisions(decisions=register["decisions"])
    pending_ids = [
        pending["decision_id"]
        for pending in register["pending_decisions"]
        if isinstance(pending, dict) and "decision_id" in pending
    ]
    if len(pending_ids) != len(register["pending_decisions"]):
        raise RequirementError("Pending decision lacks decision_id")
    hashes = {
