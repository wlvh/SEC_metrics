...
from .canonical import parse_utc_timestamp, sha256_bytes, sha256_file, strict_json_file, strict_json_loads
...
from .sources import raw_blob_record, source_reference_record, load_raw_blob_bytes, resolve_repository_file
...
def _git(*args):
    result = subprocess.run(["git", *args], cwd=CODE_ROOT, env=sanitized_git_environment(),
...
def code_identity():
    ...
    require(not _git("status", "--porcelain", "--untracked-files=all"), "RUNTIME_CLEAN_CODE_REQUIRED")
    paths = _git("ls-tree", "-r", "--name-only", "HEAD", "scripts", "tools", "catalog", "config", "requirements",
                 "docs/evidence/issue_28_annual_runtime_policy.json").splitlines()
    files = {}
    for relative in paths:
        path = resolve_repository_file(repo_root=CODE_ROOT, repo_relative_path=relative)
        files[relative] = {"sha256": sha256_file(path=path), "size": path.stat().st_size}
    return {"exact_head": _git("rev-parse", "HEAD"), "runtime_tree": content_hash(value=files)}
...
def _external(path):
    require(isinstance(path, Path) and path.is_absolute(), "RUNTIME_ABSOLUTE_PATH_REQUIRED")
    for p in (path, *path.parents):
        require(not p.is_symlink(), "RUNTIME_PATH_ALIAS_FORBIDDEN")
        require(not (p / ".git").exists(), "RUNTIME_DATA_MUST_BE_OUTSIDE_GIT")
    result = path.resolve()
    require(result != CODE_ROOT and result not in CODE_ROOT.parents and CODE_ROOT not in result.parents,
            "RUNTIME_DATA_CODE_OVERLAP")
    require(not (result / "outputs/active_publication.json").exists()
            and not (result / "outputs/publication_state.json").exists(), "RUNTIME_ACTIVE_ROOT_FORBIDDEN")
    return result
...
def verify_data_root(data_root, requirement=None):
    ...
    actual = {p.relative_to(data_root).as_posix() for top in {p.split('/')[0] for p in expected}
              for p in (data_root / top).rglob('*') if p.is_file() or p.is_symlink()}
    require(actual == expected, "RUNTIME_AUTHORITY_FILE_SET_CHANGED")
    for relative in sorted(expected):
        copied = resolve_repository_file(repo_root=data_root, repo_relative_path=relative)
        original = resolve_repository_file(repo_root=CODE_ROOT, repo_relative_path=relative)
        require(copied.read_bytes() == original.read_bytes(), "RUNTIME_AUTHORITY_BYTES_CHANGED: " + relative)
...
def _request(prepared, data_root, task_id):
    ...
    raw = raw_blob_record(repo_root=data_root, repo_relative_path=kwargs['source_repo_relative_path'], media_type=kwargs['source_media_type'])
    ...
    grid = build_table_grid(html_bytes=load_raw_blob_bytes(repo_root=data_root, raw_blob=raw),
        parent_raw_asset_ids=[raw['raw_asset_id']], storage_uri='artifacts/vnext/derived/'+raw['raw_asset_id'].split(':')[1]+'.json')
    manifest = build_reader_input_manifest(derived_asset=grid, source_reference_ids=[source['source_reference_id']])
    request = prepare_reader_request(repo_root=CODE_ROOT, task_contract_id=task_id, manifest=manifest, derived_asset=grid)
    parent = load_requirement_snapshot(snapshot_dir=CODE_ROOT/'requirements/issue_15_v1')
    ...
    runtime = load_provider_runtime_authority(repo_root=CODE_ROOT, provider=transport.provider, model=transport.model, api=transport.api)
    ...
    return {'source_reference':source, 'reader_input_manifest_id':manifest['reader_input_manifest_id'],
        ...
        'maximum_payload_bytes':transport.maximum_payload_bytes, 'model_context_tokens_max':runtime['maximum_context_tokens'],
        'provider':transport.provider,'model':transport.model,'api':transport.api}
...
def stage_proposal(*, stage_root, data_root, baseline_run, review_path):
    ...
    require(review['reviewer_kind'] == 'INDEPENDENT_MODEL_SUBTASK' and review['conclusion'] == 'NO_BLOCKING_FINDINGS'
    ...
    body = {'decision':'AUTHORIZE_ANNUAL_RUNTIME_STAGE', 'approval_kind':'USER_DELEGATED_CONDITIONAL_STAGE_APPROVAL',
        ...
        'reviewed_code':identity, 'independent_review':review, 'independent_review_sha256':sha256_file(path=review_path),
        'stage_root':str(root), 'data_root':str(data), 'initial_candidate':baseline,
        'baseline_mode':'ISOLATED_HISTORICAL_START_NOT_PRODUCTION', 'policy':policy,
        'maximum_provider_paid_sec_calls':[1,1,0], 'automatic_retry_count':0,
        'publication_authorized':False,'long_running_schedule_authorized':False}
    return {**body,'stage_id':content_hash(value=body)}


def _validate_stage(stage):
    require(type(stage) is dict and stage.get('stage_id') == content_hash(value={k:v for k,v in stage.items() if k!='stage_id'}),
            'RUNTIME_STAGE_ID_INVALID')
    requirement = _requirement()
    require(stage['decision']=='AUTHORIZE_ANNUAL_RUNTIME_STAGE'
        ...
        and stage['maximum_provider_paid_sec_calls']==[1,1,0] and stage['automatic_retry_count']==0
        and stage['publication_authorized'] is False and stage['long_running_schedule_authorized'] is False,
        'RUNTIME_STAGE_SCOPE_INVALID')
    identity = code_identity()
    require(identity['runtime_tree']==stage['reviewed_code']['runtime_tree'], 'RUNTIME_REVIEWED_CODE_CHANGED')
    review = stage['independent_review']
    require(review['reviewer_kind']=='INDEPENDENT_MODEL_SUBTASK' and review['conclusion']=='NO_BLOCKING_FINDINGS'
        and review['reviewed_head']==stage['reviewed_code']['exact_head'] and review['runtime_tree']==identity['runtime_tree'],
        'RUNTIME_INDEPENDENT_REVIEW_REQUIRED')
    ...
    require(_git('merge-base', stage['reviewed_code']['exact_head'], 'HEAD')==stage['reviewed_code']['exact_head'],
    ...
    require(update.candidate_baseline(company=company,run_dir=Path(old['provenance']['run_directory']))==old,
...
def _binding(authorization):
    require(type(authorization) is RuntimeAuthorization and authorization._factory is _FACTORY,
...
def _paths(plan):
    ...
    return workspace,workspace/'b10', 'run:annual-runtime:'+plan['plan_id'].split(':')[1]
...
def wrap_live_request(*, authorization, request):
    ...
    return _RuntimeLiveRequest(factory=_FACTORY,request=request,authorization=authorization)
...
def validate_runtime_adapter_root(*, adapter, data_root):
    auth=getattr(getattr(adapter,'invocation_context',None),'annual_candidate_authorization',None)
    fields=authorization_fields(auth)
    require(data_root==fields['data_root'],'RUNTIME_ADAPTER_ROOT_MISMATCH')


def validate_workflow_authorization(*, authorization, adapter, repo_root, run_dir, run_id, task_contract_id, **source):
    fields=authorization_fields(authorization); plan=fields['plan']; context=adapter.invocation_context
    require(repo_root==fields['data_root'] and run_dir==fields['run_dir'] and run_id==fields['run_id']
        and task_contract_id==plan['task_contract_id'] and source==plan['prepared_input']['table_input']
        and context.annual_candidate_authorization is authorization and context.qualification_usage_policy is None
        and context.workspace_dir==fields['workspace_dir'] and context.release_input_plan_id==plan['plan_id']
        and context.owner_token==fields['owner_token'],'RUNTIME_WORKFLOW_BINDING_MISMATCH')
    return fields


def write_run_binding(*, run_dir, authorization):
    fields=authorization_fields(authorization)
    require(run_dir==fields['run_dir'] and not (run_dir/BINDING_FILE).exists(),'RUNTIME_BINDING_EXISTS')
...
def validate_run_binding(*, repo_root, run_dir, manifest, records):
    binding=strict_json_file(path=resolve_repository_file(repo_root=run_dir,repo_relative_path=BINDING_FILE))
    auth=RuntimeAuthorization(factory=_FACTORY,binding=binding)
    fields=authorization_fields(auth); plan=fields['plan']
    require(repo_root==fields['data_root'] and run_dir==fields['run_dir'] and manifest['run_id']==fields['run_id']
        and manifest['record_type']=='SUCCESSOR_RUN' and manifest['requirement_id']==REQUIREMENT_ID
        ...
        and manifest['company_id']==plan['prepared_input']['table_input']['company_id']
        and manifest['target_period']==plan['prepared_input']['table_input']['target_period']
        and manifest['source_references']==[plan['request']['source_reference']]
        and manifest['task_contract_bindings']==[plan['task_binding']]
        and manifest.get('qualification_authorization') is None,'RUNTIME_RUN_BINDING_MISMATCH')
    ...
    for attempt in attempts:
        require('qualification_authorization' not in attempt,'RUNTIME_QUALIFICATION_FORBIDDEN')
        require(sha256_file(path=run_dir/attempt['request_body_path'])==plan['request']['provider_request_body_sha256'],
                'RUNTIME_PERSISTED_REQUEST_CHANGED')
        if attempt['status']=='SUCCEEDED':
            require(not prior.usage_error((run_dir/attempt['raw_response_path']).read_bytes()),'RUNTIME_USAGE_INVALID')
...
def _validate_terminal(*, fields, attempt):
    ...
    if not attempt['transport_observation']['egress_attempted']:
        ...
        return
    plan=fields['plan']; root=fields['workspace_dir']/'invocation_control'
    plans=list((root/'plans').glob('*.json'))
    require(len(plans)==1 and not plans[0].is_symlink(),'RUNTIME_CONTROLLER_PLAN_MISSING')
    invocation=strict_json_file(path=plans[0])
    authority=controller.prepare_annual_candidate_invocation_authority(requirement=fields['requirement'],repo_root=CODE_ROOT)
    with controller._successor_plan_context(repo_root=CODE_ROOT,authority=authority):
        ...
        require(invocation['release_input_plan_id']==plan['plan_id']
            ...
            and invocation['selected_representation_hash']==plan['request']['derived_asset_id'], 'RUNTIME_CONTROLLER_PLAN_MISMATCH')
        eid=controller.execution_identity(ai_invocation_plan_id=invocation['ai_invocation_plan_id'],
            owner_token=fields['owner_token'],authorized_at_utc=fields['authorized_at_utc'])
        receipt=controller._load_execution_receipt(root=root,path=controller._execution_path(root=root,execution_id=eid),execution_id=eid)
        markers=controller._egress_markers_for_execution(root=root,execution_id=eid)
        require(len(markers)==1 and markers[0]['attempt_ordinal']==1
        ...
        if attempt['status']=='SUCCEEDED':
            ...
            saved=controller.load_successful_response(workspace_dir=fields['workspace_dir'],plan=invocation)
            require(receipt['status']=='SUCCEEDED' and len(receipt['attempts'])==1
                ...
                and receipt['attempts'][0]['usage']==_controller_usage(raw_response_bytes=(fields['run_dir']/attempt['raw_response_path']).read_bytes()),
                'RUNTIME_RESPONSE_EXECUTION_MISMATCH')
        else:
            require(receipt['status'] in {'UNKNOWN_REMOTE_OUTCOME','FAILED_TERMINAL','FAILED_RETRYABLE_FINAL'},
...
def _exclusive_slot(stage,plan):
    ...
    with os.fdopen(descriptor,'wb') as out:
        out.write(canonical_json_bytes(value={'stage_id':stage['stage_id'],'plan_id':plan['plan_id']}))
...
def run_update(*, approval_url):
    ...
    binding=verify_stage(approval_url=approval_url); stage=binding['stage']; root=Path(stage['stage_root'])
    company=update.supported_company(repo_root=CODE_ROOT)
    published=update.published_baseline(company=company,publication_root=CODE_ROOT)
    old=stage['initial_candidate']; baseline=old
    ...
    if report['status']!='INPUT_READY':
        ...
        return report
    plan=prepare_plan(binding=binding); binding={**binding,'plan':plan}
    ...
    report.update(status='CANDIDATE_UPDATE_SUCCEEDED' if good and success_b01 else 'CANDIDATE_UPDATE_FAILED',
        execution='EXECUTED',new_candidate={'run_directory':str(b10),'run_id':run_id,'run_status':manifest['status'],
        'results':results,'attempts':[r for r in records if r['record_type']=='AI_EXTRACTION_ATTEMPT'],
        'reviewers':[d['reviewer_type'] for d in decisions]},stage_provider_paid_sec_calls=stage_counts(root))
    ...
    if report['status']=='CANDIDATE_UPDATE_SUCCEEDED':
        ...
        atomic_write_json(path=success,value={'run_directory':str(b10),'run_id':run_id,'stage_id':stage['stage_id'],
            'b01_run_directory':str(workspace/'b01')})
