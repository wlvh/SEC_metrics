...
A source adapter, not a second reader. ``_source_plan`` and the
``text_results_v2`` scopes are imported unchanged from the frozen modules, and
the candidate functions this feeds read only the target filing's own bytes.
What this module owns is which filing the text roles mean when the period is
pinned.

The metadata view is the one place that could not be inherited. The frozen
``_current_metadata_context`` scans only ``filings.recent`` and refuses while a
declared history shard could reach the pinned period, which puts thirteen
company periods of this repository's own saved indexes out of reach.
``historical_metadata_context`` keeps that invariant and meets it by reading
those blocks through the existing historical catalog instead.
...
def prepare_historical_business_text_input(*, repo_root: Path, company_id: str, metric_id: str,
    ...
    metadata = historical_metadata_context(repo_root=root, company_id=company_id,
                                           prepared=prepared, inventory=inventory,
                                           reader=reader, metric_id=metric_id)
    ...
    body = {"record_type": RECORD_TYPE, "metric_id": metric_id, "company_id": company_id,
            ...
            "metadata_blocks_read": metadata["loaded_inventories"], "source_proofs": proofs,
