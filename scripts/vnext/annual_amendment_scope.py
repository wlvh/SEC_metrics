...
def _link_note_identity(first, purpose, source):
    if purpose is None:
        return False
    intro = re.fullmatch(POLICY['link_note_intro_pattern'], first[:purpose.start()].strip())
    if intro is None:
        return False
    names = source['document']['registrant_names']
    if not any(_words(intro['registrant']) == _words(name) for name in names):
        return False
    try:
        report_end = datetime.strptime(intro['period_end'], '%B %d, %Y').date().isoformat()
    except ValueError:
        return False
    banners = [b['text'] for b in source['document']['blocks']
               if re.fullmatch(r'Amendment No\. [0-9]+', b['text'], re.I)]
    return (report_end == source['period']['period_end']
...
def prepare_saved_amendment_scopes(*, repo_root: Path, company_id: str):
    ...
    proofs=list(prepared['source_proofs']);sources=[];records=[]
...
def prepare_saved_amendment_input(*, repo_root: Path, company_id: str, input_class: str):
    ...
    verify subject continuity, event-source coverage and the metric's semantics.
    ...
    body = {**packet, 'record_type':'ORDINARY_ANNUAL_AMENDMENT_INPUT', 'schema_version':1,
            'input_class':input_class, 'decision':'WITHHELD' if failures else 'INPUT_PROPERTY_PROVEN',
            'basis':'ALL_SAVED_CURRENT_AMENDMENTS_CHECKED' if packet['scopes'] else 'NO_CURRENT_AMENDMENTS_IN_PINNED_INVENTORY',
            'unresolved_scopes':failures, 'not_covered_metric_ids':POLICY['not_covered_metric_ids'],
            'subject_continuity_proven':False, 'metric_result_created':False,
            'resolver_sha256':sha256_file(path=Path(__file__))}
    body = exact_json_value(body)
    return {**body, 'amendment_input_id':content_hash(value=body)}
