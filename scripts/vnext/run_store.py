...
def _run_validation_artifacts(*, run_dir: Path) -> Dict[str, object]:
    ...
    if manifest["record_type"] == "SUCCESSOR_RUN" and manifest.get("requirement_id") in {"issue_28_v4", "issue_28_v5"}:
...
def _run_transport_policy(*, requirement):
    ...
    if requirement.get("requirement_id") in {"issue_28_v4", "issue_28_v5"}:
