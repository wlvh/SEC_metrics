"""Ordinary B06 native records for a carrying total with finance leases included."""
from .b06_inclusive_table import POLICY, POLICY_PATH, InclusiveDebtError, inspect_inclusive_debt_scope
...
SPEC_PATH = 'catalog/r5/B06_inclusive_table_v6.md'
RESOLVER = 'debt_equity_inclusive_table_v6'
MODEL_ID = 'INCLUSIVE_REPORTED_TOTAL'
...
def _spec(root):
    for path in [SPEC_PATH,POLICY_PATH]:
        _need(sha256_file(path=resolve_repository_file(repo_root=root,repo_relative_path=path)) == sha256_file(path=ROOT/path),
              'ORDINARY_INCLUSIVE_DEBT_INSTALLED_AUTHORITY_CHANGED')
    ...
    _need(rule['resolver'] == RESOLVER and rule['source_policy'] == POLICY_PATH
          and rule['source_policy_sha256'] == sha256_file(path=root/POLICY_PATH), 'ORDINARY_INCLUSIVE_DEBT_SPEC_BINDING_CHANGED')
    ...
    _need(rule['original_metric_spec_sha256'] == sha256_file(path=root/rule['original_metric_spec'])
          ...
          'ORDINARY_INCLUSIVE_DEBT_ECONOMIC_DEFINITION_CHANGED')
    return spec


def prepare_inclusive_debt_case(*, repo_root, company_id):
    ...
    if not all(local.casefold() in available for local in POLICY['note_concepts']+POLICY['extension_note_concepts']): return None
    ...
    try:
        ...
        if company['industry_profile'] == 'financial_institution' or any(any(str(m).split(':')[-1] in restricted for m in c['dimensions'].values())
            ...
            raise InclusiveDebtError('INCLUSIVE_DEBT_SEPARATE_BANK_OR_INDUSTRIAL_SCOPE_REQUIRED')
        source_proof = inspect_inclusive_debt_scope(primary=preparation['primary'],xml=preparation['xml'],prepared=original)
        path = repo_root/rule['debt_set_registry']; _need(sha256_file(path=path) == rule['debt_set_registry_sha256'],
                                                         'ORDINARY_INCLUSIVE_DEBT_REGISTRY_CHANGED')
        ...
        for check in model['checks']:
            ...
            _need(left == right,'ORDINARY_INCLUSIVE_DEBT_DECLARED_RECONCILIATION_FAILED')
        ...
        _need(amount == Decimal(source_proof['proven_composition_amount']),'ORDINARY_INCLUSIVE_DEBT_PROVED_AMOUNT_CHANGED')
        ...
        for p in precision:
            ...
            _need(current and all(f['unit']=='USD' and f['value'] in {v['value'] for v in p['all_reports']} for f in current),
                  'ORDINARY_INCLUSIVE_DEBT_COMPANYFACTS_CONFLICT_OR_MISSING:'+p['concept'])
        ...
        selection = {'classification':'SOURCE_RECONCILED_TOTAL_WITH_FINANCE_LEASE_INCLUDED','audit':audit}
    except InclusiveDebtError as error:
    ...
    binding = exact_json_value({'record_type':'ORDINARY_INCLUSIVE_DEBT_INPUT','original_source_input':preparation['input_binding'],
