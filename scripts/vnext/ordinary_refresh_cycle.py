...
def refresh_and_process(*, session, state_root, company_ids=None, metric_ids=None, max_sec_requests, max_provider_requests=0, recorded_native_wire_factory=None):
    ...
    for company in selected:
        ...
        try:
            updates = run_company(state_root=Path(state_root) / company, source_root=session.data_root,
                company_id=company, metric_ids=requested,
                native_assessment_mode='LIVE' if session.ledger.live else 'RECORDED_TEST_ONLY',native_assessment_ledger=session.ledger) if requested else {'metrics': [], 'status': 'UPDATES_INCOMPLETE'}
        except Exception as error:
