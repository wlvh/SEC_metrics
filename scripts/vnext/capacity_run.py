...
from .capacity_assessment_input import EXPORT_PATH, load_registered_input
...
def prepare_case(*, data_root, company_id, assessment_mode=None, assessment_input_id=None):
    ...
    need(assessment['proposed_branch'] in {'TEXT_QUAL_PROPOSAL_REQUIRES_NATIVE_REVIEW',
         ...
         'B13_NATIVE_NON_TEXT_BRANCH_REQUIRES_IMPLEMENTATION:' + assessment['proposed_branch'])
    ...
    binding = {'company_id': company_id, 'source_id': source['semantic_source_id'],
               ...
               'mode': registered['mode'], 'export_path': EXPORT_PATH, 'production_authorized': False}
    return {'kind': 'TEXT', 'primary_metric_id': 'B13', 'input_binding': binding,
        ...
        'selection': {'status': ('NOT_AVAILABLE_SEC' if assessment['proposed_branch'] ==
...
def install_inputs(*, data_root, company_id, source_root=ROOT, assessment_mode=None, assessment_input_id=None):
    ...
    case = prepare_case(data_root=source_root, company_id=company_id, assessment_mode=assessment_mode,
                        assessment_input_id=assessment_input_id)
    requirement = load_requirement_snapshot(snapshot_dir=ROOT / 'requirements' / REQUIREMENT_ID)
    extra = ({EXPORT_PATH: canonical_json_bytes(value=case['registered_input'])}
    ...
    rebuilt = prepare_case(data_root=data_root, company_id=company_id)
...
def create_run(*, data_root, run_dir, company_id):
    ...
    case = prepare_case(data_root=Path(data_root), company_id=company_id)
    requirement = load_requirement_snapshot(snapshot_dir=Path(data_root) / 'requirements' / REQUIREMENT_ID)
    return _create_case_run(data_root=data_root, run_dir=run_dir, company_id=company_id, metric_id='B13',
...
def validate_run_authority(*, repo_root, manifest, records, compiled_specs):
    ...
    need(manifest['requirement_id'] == REQUIREMENT_ID and manifest['run_id'].startswith(PREFIX)
         and (compiled_specs is None or set(compiled_specs) == {'B13'}), 'B13_NATIVE_RUN_ROUTE_REQUIRED')
    case = prepare_case(data_root=repo_root, company_id=manifest['company_id'])
    ...
    need((compiled_specs is None or compiled_specs == case['compiled_specs']) and manifest['source_references'] == case['references']
         ...
         and manifest['spec_file_hashes'] == {SPEC_PATH: sha256_file(path=repo_root / SPEC_PATH)},
...
def project_defined_absence(*, case, result, row, company):
    ...
    need(result['reason_code'] == 'B13_DEFINED_SCOPE_NO_RELEVANT_DISCLOSURE'
         and result['value'] is None and result['publication'] == 'WITHHELD'
         and case['selection']['status'] == 'NOT_AVAILABLE_SEC'
    ...
    row.update(status='NOT_AVAILABLE_SEC', value='',
    ...
    for document in source['documents']:
        ...
        item.update(company=company['display_name'], cik=company['primary_cik'], metric_id='B13',
            ...
            extraction_method='complete_native_capacity_assessment', parser_version='capacity_run')
