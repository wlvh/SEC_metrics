...
`scripts/vnext/table_context_attestation.py`读取合并后的Stage C-B pointer→packet→plan/evidence/marker/raw response链，逐个重算content ID与raw usage，再从current Requirement、source ledger、full table grid、task、serializer和provider policy离线重建同一个provider body。attestation同时保存historical measurement closure与current admission closure；任何source/task/prompt/schema/provider request或protected byte drift都会在egress前失效。`table_qualification_freeze.py`用同一full SourceReference为每个task生成request binding，exact attestation只匹配attested task；sibling task因request/task hash不同继续blocked。future qualification adapter在WB-3 plan/request equality之外检查qualification usage policy，missing/excess usage在success/acceptance持久化前转为terminal；同一ordinal crash recovery从controller attempt usage receipt恢复证明，不把assistant payload当作raw provider envelope。
