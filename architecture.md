...
| release plan / ProjectionManifest / PublicationManifest | release plan 定义迁移 exact set；Projector 从 verified Run 生成投影事实；publisher 从 bundle 内 manifest 决定 candidate 状态 | repository release config、legacy inputs、Requirement、Run、ledger prefix、receipts、bundle files |
...
- AI 网络只允许由仓库代码注册的 provider transport 所有；`run_ai_attempt` 只接受 adapter、Reader factory 生成并联合绑定 manifest/task/request/Spec identity 的 prepared request 和 clock，temperature 固定为 0，响应固定进入严格 Reader validator，调用方不能传 sampling mapping 或 no-op callback。`ApprovedTransportAdapter` 不接受 caller policy、root 或 transport，而是从自身模块路径推导固定 repository root，验证 Requirement closure，把唯一 effective APPROVED D-01 的十个 `choice` 字段编译成不可变 `TransportPolicy`，再按 provider 选择仓库 factory。每次 outbound request 前还会重新加载 policy/closure；已构造 adapter 遇到 D-01 撤回或变化时不得调用 transport。factory 返回的 transport 必须在 payload 前证明自己绑定 exact policy，adapter 先执行 payload 上限；transport 返回 `TransportObservation` 的实际 egress、provider/model/host、region、timeout/retry、retention/data-use、payload 与 request size，attempt 顶层标签从该 observation 构造。观察与批准不一致、带完整 observation 的 timeout/transport failure 都形成独立 FAILED attempt；异常或旧式 tuple 等结果缺少 observation 时直接失败，不能猜测并写入获批 host。freeze/load 对磁盘重读的每条 SUCCEEDED attempt 都重新执行 response schema 和 recorded/effective D-01 验证，即使没有 Candidate 引用也不能绕过。D-01 pending/absent/rejected 或 provider factory 未实现时构造失败；recorded adapter 明确记录 no-egress 且不开网络。
...
Projector 的公共入口只接收 `repo_root`、`run_dir`、`legacy_snapshot_dir` 与 `staging_dir` 四个稳定 locator；内部执行 `load_frozen_run()`，并从固定目录重读 legacy/candidate/gate exact file set，不能相信调用方拼出的 `status=FROZEN` mapping 或 hash/status/metric facts。migrated metric exact set 完整取自仓库 `config/vnext_release_plan.json`，再与 Run Result 做 exact-set 互验；不能从当前 Result 或 Spec closure 缩小 release 范围，因为同一 B01 Spec 既可能只是 B03 replay dependency，也可能同时是必须发布的顶层输出。投影必须从完整 legacy matrix/evidence 输入开始，原位替换 migrated keys 并逐字段保留非迁移 rows；同一 legacy `(company_id, metric_id)` 只能有一条结果，不允许两个 scope grain 留给行层隐式覆盖。B03 component evidence 以一个 source binding 一行输出，并由 reconciliation receipt 精确重建冻结的 `;`/`+` 聚合。任何 APPLICABLE/WITHHELD 使 projection candidate 为 BLOCKED。
...
Publication preparation 必须语义解析 bundle 内的 ProjectionManifest，并以其中的 PUBLISHABLE 状态、Run content/audit、Requirement、Result/Trace/Observation/Review/DerivedAsset identities 为 candidate 事实；公共入口不再接受游离 `migrated_results` 覆盖 bundle 状态。它只准备 exact required file set 与 PASSED validation receipt 绑定的 immutable bundle；receipt 的无自引用 staging view 必须同时绑定非 receipt artifact bytes、Requirement、FROZEN Run/review/trace、ledger prefix 与 prepared predecessor，required gate 名称集合必须精确且全部 PASS，并按 path/SHA-256/size 覆盖全部非 receipt artifact。Requirement/Run/ledger nested binding 在 preparation 与 read-back 两端都校验 digest 类型、非负 exact row count、唯一有序 identity 和 predecessor 形状。bundle namespace 必须只有声明的 regular files/directories，不接受 symlink、额外文件或大小写别名。commit、rollback、recovery、PublicationView 与 latest writer 只接受单一 `publication_root`，内部固定派生 immutable storage、pointer、lock、status 与 compatibility mirrors；调用方不能分别命名一套互相矛盾的路径。commit 使用 POSIX lock、expected active ID CAS、已验证 bundle manifest 与唯一 active pointer。rollback 只允许当前 pointer 已证明的 committed predecessor，不接受 prepared-only sibling。固定根 mirror 在 pointer 前准备，失败时恢复上一 bytes；崩溃后的 mirror 可从 active pointer 重建。`PublicationView` 只解析一次 pointer 并 pin 一个 bundle ID；`scripts/vnext/report.py` 只从一个 view 读取 report inputs，不开网络、不 repair、不写 authoritative artifact。latest run status 独立于 active；writer 只接收 persisted Run directory 或 publication ID locator，在同一个 pointer lock 内加载真实 Run/bundle并派生 Run status、candidate status、active ID 与 `active_is_latest_success`，不接受调用方自报枚举、boolean、view 或 manifest。若同一 Run identity 已 active，却由 locator 解析为 FAILED/BLOCKED，状态写入必须失败。
