...
`catalog/table_task_contracts.json`静态声明single-role task；共享Reader/Evidence/Projector不按metric/company/table ID选择目标表。lodging继续使用原有schema v3和一个完整compact request；financial schema v4则只在provider request边界把同一完整compact parent机械切为连续shards。分包算法只读取原始table order、serializer-v2 compact entry bytes和固定task/provider envelope：不读metric literal来找表、不做semantic prefilter/selector。每个shard仍携带完整`ReaderInputManifest`，并绑定shard set/index/count/range、exact `examined_table_ids`及`CANDIDATE_PRESENT`或`NO_CANDIDATE_IN_SHARD`；后者只产生`WITHHELD / SHARD_NO_CANDIDATE`，不构成指标缺失结论。Candidate、Evidence、ReviewUnit、attempt、Run和WB-3 authorization共同哈希同一shard binding；cycle gate只有在全部shards FROZEN、679个table IDs逐一且只出现一次、至少一个candidate shard Evidence PASS且多个candidate不冲突时才形成一个task qualification terminal。single shared locator contract仍规定`caption`只接受`derived_asset_id/table_id`，`cell/header/row/label`只接受含row/column/origin/span的八字段cell locator；这与Reader/Evidence既有重取语义一致，不搜索raw text、不补坐标。

`config/table_qualification_matrix.json`与content-addressed freeze冻结source/second-layout/holdout、task/schema/prompt、11组round trip、WB-3 regression、pre-R3 active/root与protected closure。freeze schema-v4在`readiness_by_family`之外保留`readiness_by_task_request`：每个development source×task使用与Workflow相同的完整SourceReference形成exact request authority；financial row另外保存全部child request SHA、estimated tokens、连续range和full coverage set。JPM development source离线组合为每task 27 shards、679 tables，最大单片199912，全部低于inclusive 200000；BOA/Citi immutable attempts分别拥有独立完整parent与机械连续shards。local parser始终先完整materialize，current cap以BOA最大值精确为200229，不做local shards。SECOND_LAYOUT/HOLDOUT plan还把full-document table count、ordered shape与ordered header text/geometry hash和different-issuer identity绑定为pre-egress layout proof。普通bound与exact-attestation path互斥，exact PASS只去掉本request的estimated blocker。shared drift仍传播dependent families，matrix/task/MetricSpec/attestation等local drift只阻断owner family。唯一qualification adapter把task/request context binding与response-origin/usage policy放入opaque authorization；generic reusable或measurement response的`egress_attempted=false`无法成为qualification evidence。每个真实shard response必须有provider usage且actual prompt<=200000，否则controller写`CONTEXT_LIMIT` terminal、零retry并停止该task后续shards/ordinal。既有WB-3 marker/execution/attempt/success/UNKNOWN、Run/ledger/evidence exact-set与幂等recovery不变量保持不变。十个lodging qualification terminals和R3 formal publication继续作为已提交的无关family证据，不因financial exact-head push重签。financial current freeze/Stage-A、D-07 concrete scope、exact-head审核和live qualification仍须按新head完成。
...
`tools/benchmark_jpm_full_materialization.py`保留Darwin `NOT_RUN_RSS_GUARD_UNAVAILABLE`历史receipt，并新增同一入口的pinned Linux execution：官方Python image按digest固定，repository read-only挂载，cgroup-v2 memory与swap均硬限536870912 bytes，parent wall watchdog为120秒，容器`network=none`、无IPv4与non-loopback IPv6 route、drop-all capabilities且不挂Docker socket。child仍只把自己的`table_grid.RESOURCE_LIMITS.max_total_cells`临时设为187142，调用同一production parser→DerivedAsset→canonical JSON；production cap和root在measurement before/after保持100000及逐byte相等。一次局部变量错误的`FAILED_CHILD` receipt保留后，committed fix完成JPM 679 tables/124761 cells：process peak RSS 282877952、cgroup peak 288043008、wall 5.10483s、canonical JSON 22174348 bytes、DerivedAsset `694e1764…8110`，三类egress0/0/0。历史D-35据此完成100000→124761。后继BOA exact characterization在同样512 MiB/120s/network-none边界完成369 tables/200229 cells：process peak 366530560、cgroup peak 371380224、wall 4.330166s、canonical 34204694 bytes、DerivedAsset `5417fee3…8519`，owner选择A完成current 124761→200229且仍拒绝parser/materialization shards。provider request连续分包属于另一边界；current validator同时重验immutable JPM receipt、两段D-35 transition与resource/tool authority，历史R3 bundle继续用pre-resource D-35闭包验证。
