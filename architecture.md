...
`scripts/vnext/table_context_attestation.py`保留各代plan/evidence/marker/raw链并逐个重算content ID与raw usage；same-ID D-07 successor从不改写旧attestation。schema v3改变provider request/schema/task hashes后，两份上一代attestation仍是immutable historical evidence，但current binding必须失败，freeze据此把lodging恢复为not-ready。新的Occupancy/RevPAR terminal将继续使用同一`TABLE_CONTEXT_FEASIBILITY_ATTESTATION` schema直接绑定plan/marker/raw/evidence，不新增packet或record type；acceptance successor只把这两份新ID设为current。`table_qualification_freeze.py`按request task选择并重验各自attestation，只有两个required request都通过才派生lodging family ready。future qualification adapter仍在WB-3 plan/request equality之外检查新execution usage，missing/excess usage在success/acceptance持久化前转为terminal；measurement response不可复用。
