...
R2新增的14个财务deterministic指标由`catalog/deterministic_metrics.json`声明concept priority、current/prior accession与period role、维度、formula branch、applicability、entity continuity和legacy projection。source plan只从pinned submissions发现当前/上一份original 10-K，再从catalog候选SEC facts机械派生target period；财务producer函数不接收legacy row/evidence/expected value。旧matrix只在全部Result生成后进入strict compatibility verifier，不再反向选择claim。
