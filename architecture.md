...
Requirement dispatch分为两层。`ai_first_v3_3_1`和`issue_15_v1`保留历史adapter，以原规则解释不可变R1–R3 bytes；其他snapshot只有在baseline明确声明`PROFILE_DRIVEN_V1`、五文件exact set、parent closure及`requirement_profile.py` validator identity全部匹配时才进入通用路径。通用Decision resolver拒绝duplicate、detached、fork、cycle和effective `SUPERSEDED`；`invariant_profile.json`只保存`invariant_id → decision_id`，具体policy值只存在于Decision Register。Python仅实现closed typed invariant kinds，不执行任意path/expression DSL。
...
Successor Run在同一record subtype上采用generation-specific identity：历史Run保留hash-only形状；profile-driven Run只有同时持久化`requirement_id`、`requirement_closure_hash`和`requirement_hashes`才可继续freeze/replay，缺失或伪造字段不能回退legacy。
