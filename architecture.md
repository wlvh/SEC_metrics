...
Stage C-A的`scripts/vnext/table_context_measurement.py`建立独立于qualification的窄执行边界，Stage D继续复用同一边界而非复制executor。当前offline plan从latest same-ID D-07 RevPAR exception、历史content-addressed Stage-C lineage、matrix-owned Marriott immutable attempt、`lodging_revpar_table_v2`、serializer v2、current prompt/schema/provider envelope及protected source closure重建；392438 estimated只在该exact measurement plan中越过普通200000门。真实capability必须由`AUTHORIZE_ONE_TOKEN_MEASUREMENT`、当前clean exact HEAD、独立PR comment URL和该comment绑定的provider request SHA共同签发；authorization每次使用前重建HEAD/tree、protected closure、family/task/source/prompt/schema/request/provider identity。`ai_adapter.py`仍拥有唯一socket opener，measurement transport只能由opaque authorization factory构造。socket open立即前的marker使用`O_CREAT|O_EXCL`独占claim并fsync文件与父目录；任何existing marker都在opener前稳定返回`TABLE_CONTEXT_MEASUREMENT_AUTHORIZATION_CONSUMED`。marker前失败不消费，marker后成功、HTTP、usage缺失或UNKNOWN都永久消费且绝不重试。schema-v2 marker/evidence增加authorized HEAD/tree/review URL与RevPAR task binding；usage缺prompt/input token为`FAILED_USAGE_UNAVAILABLE`，不会猜值。该record type仍不进入通用Run/Candidate/Evidence/Review/Observation/publication状态机，qualification/publication/reuse credit固定为false；历史occupancy plan/marker/evidence原字节不改写。
...
`scripts/vnext/table_context_attestation.py`保留historical Stage C-B pointer→packet→plan/evidence/marker/raw occupancy链并逐个重算content ID与raw usage。D-07 additive successor不重签occupancy attestation：validator要求其exact content ID，离线重建当前occupancy provider body，且只允许baseline/decision register/Requirement loader/同一measurement executor四个文件发生该successor所需的受控变化；任何其他protected drift或任一request语义字段变化仍失败。RevPAR成功terminal则以同一`TABLE_CONTEXT_FEASIBILITY_ATTESTATION` schema直接绑定plan/marker/raw/evidence，`source_stage_c_b_packet_*`为null且不新建packet/pointer。`table_qualification_freeze.py`按request task选择并重验各自attestation，只有两个required request都通过才派生lodging family ready。future qualification adapter仍在WB-3 plan/request equality之外检查新execution usage，missing/excess usage在success/acceptance持久化前转为terminal；measurement response不可复用。
