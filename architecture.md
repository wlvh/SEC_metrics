...
RevPAR one-shot随后在review-bound exact head上形成160928/535/161463 provider usage与第二个exact attestation；该authorization已永久消费，历史comparison/Stage-C packet不重签。latest D-07 qualification-only successor把live scope严格限于`lodging_kpi_table`的两个现有task。`qualification.py`仍使用同一task plan/opaque authorization/WB-3/Run/Evidence/Review路径，但增加matrix-owned sample phase：second layout从Hilton fixture manifest机械解析到immutable attempt，post-freeze holdout同理解析Hyatt，fresh stability回到Marriott development source。caller只选择phase/task/ordinal，不能传source/company/period/provider。每个plan保存exact source hash、provider request SHA、estimated tokens和context basis；Hilton走ordinary estimated bound，Marriott走各自attestation，Hyatt略超estimated bound时只允许独立exact-head review绑定其两个request，并在新qualification response持久化前以actual usage<=200000 terminal gate裁决。
