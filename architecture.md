...
| Validation snapshot 与 source/artifact bytes 绑定 | stage 11/12 开始时使旧 provenance sidecar 失效；stage 12 只有在 source-input closure clean、terminal manifest 成功、关键 artifact SHA-256/size sidecar 原子发布并从磁盘自验成功后才返回零。无 Git light 包仍要求全部显式 singleton source 文件存在 | 用旧 success sidecar证明新报告；只比较 commit 字符串；删除治理/契约文件缩小 light source closure；运行后修改 source 或关键 artifact；postflight 失败仍保留 PASSED/GO | `tests/test_validation_provenance.py`、`tests/test_validation_provenance_light_package.py`、`tools/check_validation_snapshot.py` | source/tree/file-count、manifest identity、artifact key/hash/size 或 publication 任一失配均使 snapshot 不可验收；postflight 尝试降为 FAILED/NO-GO并非零退出 |
...
| `scripts/validation_provenance.py` | 定义 source/artifact closure，捕获 tree identity，原子发布/验证 sidecar，并执行 terminal fail-closed rewrite | 指标业务正确性、外部签名或 WORM | Git workspace、Python 标准库、manifest/report/artifact files |
...
- Snapshot provenance 层：成功 stage 12 的 `validation_snapshot_provenance.json` 记录 source checkout 状态、source commit、deterministic source tree digest/file count，以及 manifest/report/README、核心业务 artifact、request ledger 和本轮 refreshed validation artifact 的 SHA-256/size。它与 manifest 职责分离。
...
| stage 12 source-input closure dirty、显式 singleton source 文件缺失、symlink 或 Git metadata 不可信 | 在运行主 gate 前失败；无 Git 仅允许显式 light package，且不能通过删文件缩小 source closure |
...
- 新增会影响运行或验收的 source path：同步 `validation_provenance.py` 的 closure、专项文档与缺失/篡改负例。
