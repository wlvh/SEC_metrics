...
schema-v3两份proof被接受后，首个Hilton Occupancy qualification通过provider schema，但机械Evidence发现`caption`文本并非来自已选目标表：目标表的supplied `caption_raw_text`为空，response却借用了另一张表或邻近正文，因此以`EVIDENCE_FAILURE / SCOPE_LABEL_TEXT_MISMATCH`终态停止且不产生Candidate/Review/Result。latest D-07只追加最小prompt绑定：`caption`仅在目标表自身caption raw text非空时可用并逐字复制；否则必须从同一目标表的一格复制cell/header/row/label八字段locator及exact raw text，禁止跨表或借邻近正文。schema仍为v3，MetricSpec/source/serializer/provider/model/API、全表原序与no-selector/no-prefilter均不变。该prompt变化再次使161282/161263 attestation成为historical；当前两个新exact measurement plan estimated为394837/394828，qualification关闭，只有独立exact-head review后才可各执行一次usage-only、retry=0 measurement。
