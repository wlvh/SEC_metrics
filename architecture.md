...
RevPAR one-shot随后在review-bound exact head上形成160928/535/161463 provider usage与第二个exact attestation；该authorization已永久消费，历史comparison/Stage-C packet不重签。latest D-07 qualification-only successor把live scope严格限于`lodging_kpi_table`的两个现有task。`qualification.py`仍使用同一task plan/opaque authorization/WB-3/Run/Evidence/Review路径，但增加matrix-owned sample phase：second layout现从owner批准的Marriott FY2024 fixture manifest机械解析到immutable SEC attempt，post-freeze holdout解析Hyatt，fresh stability回到Marriott FY2025 development source。FY2024 source只通过既有`SecHttpClient`获取一次且无模型调用；离线重建证明`table_000011`在同一目标表内包含全部冻结scope literals，并以29x39、不同grid hash区别于FY2025的27x39目标表。caller只选择phase/task/ordinal，不能传source/company/period/provider。每个plan保存exact source hash、provider request SHA、estimated tokens和context basis；FY2024 second-layout与Hyatt holdout的estimated request均超过200000，只能在独立exact-head review逐plan/request绑定后走既有`EXACT_REVIEWED_QUALIFICATION_REQUEST_WITH_TERMINAL_USAGE`路径，并在各自新的qualification response持久化前以provider-reported actual usage<=200000裁决。缺usage或超限均terminal、零重试并停止后续lodging plans；不新增measurement，也不复用历史response。Marriott FY2025 fresh仍只使用两份exact attestation。
